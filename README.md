...
`--affected` deploys only the Workers whose inputs the release changed — judged **per Worker against the commit it is serving** (`build.commit` on its `/healthz`; the last `v*` tag as the fallback; no base at all → "unsure" → deployed). A Worker's inputs are the import closure of its `worker.ts`, its own directory, and the *production* half of its lockfile (a `vitest` bump deploys nothing; an `@cloudflare/sandbox` bump deploys the resident); the bot's also include everything its Dockerfile copies. Tests, docs, CI and the deploy tooling are an explicit inert list; a path no rule claims makes the whole fleet deploy and names itself. The release PR carries this plan as a sticky comment before anyone merges it, and every PR's `deploy targets` check shows what its own diff would deploy. Contract: [features/release-and-deploy.md](features/release-and-deploy.md). Manual deploys (a Worker whose release deploy failed, a deliberate full roll) go through the same workflow: `gh workflow run deploy-production.yml --ref main -f targets=all` (or `-f targets=bot,resident`, `-f force=true`) — never from a laptop.
