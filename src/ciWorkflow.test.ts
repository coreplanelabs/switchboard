...
describe("the release publishes the npm package", () => {
  ...
  // The dist-tag follows the line (item 28): `latest` on main, `next` on v1.2.
  const PUBLISH_LINE =
    "npm publish --workspace packages/switchboard --access public ${{ github.event.repository.private == false && '--provenance' || '' }} --tag ${{ github.ref == 'refs/heads/main' && 'latest' || 'next' }}";
  const publish = Object.entries(workflow.jobs).filter(([, job]) =>
