...
describe("the release publishes the npm package", () => {
    ...
    expect(commands).toEqual([
      {
        ...
        id: "publish-npm",
        line: "npm publish --workspace packages/switchboard --provenance --access public --tag ${{ github.ref == 'refs/heads/main' && 'latest' || 'next' }}",
      },
    ]);
  ...
  it("installs from the lockfile, builds the package with its own script, and publishes it public with provenance", () => {
    expect(runs()).toEqual([
      ...
      "npm publish --workspace packages/switchboard --provenance --access public --tag ${{ github.ref == 'refs/heads/main' && 'latest' || 'next' }}",
    ]);
