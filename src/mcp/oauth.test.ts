...
describe("detectAuth", () => {
  it("2xx → none; 401 with discoverable OAuth → oauth (with the discovery); 401 without → bearer; anything else → error naming --auth; unreachable → error", async () => {
    ...
    expect(await detectAuth(oauth.fetch, SERVER)).toMatchObject({ auth: "oauth", discovery: { tokenEndpoint: "https://as.example.com/oauth/token" } });
  ...
  it("reads the WWW-Authenticate resource_metadata hint, quoted or bare", () => {
    expect(resourceMetadataHint('Bearer error="invalid_token", resource_metadata="https://mcp.example.com/.well-known/oauth-protected-resource"')).toBe("https://mcp.example.com/.well-known/oauth-protected-resource");
    expect(resourceMetadataHint("Bearer resource_metadata=https://x.example/.well-known/oauth-protected-resource, scope=a")).toBe("https://x.example/.well-known/oauth-protected-resource");
...
describe("discover", () => {
  it("resource metadata: the hinted URL first, then the path-inserted and root well-known forms; the authorization server's RFC 8414 document with the path inserted after the host (Vanta's shape); scopes from the resource", async () => {
    ...
    expect(urls.indexOf("https://mcp.example.com/.well-known/oauth-protected-resource/mcp")).toBeLessThan(urls.indexOf("https://mcp.example.com/.well-known/oauth-protected-resource"));
  ...
  it("refuses an authorization server without PKCE S256 or the authorization_code grant, http endpoints, and blocked hosts", async () => {
    await expect(discover(fakeAuthorizationServer({ server: SERVER, codeChallengeMethods: ["plain"] }).fetch, SERVER)).rejects.toThrow(/PKCE S256/);
    await expect(discover(fakeAuthorizationServer({ server: SERVER, grantTypes: ["client_credentials"] }).fetch, SERVER)).rejects.toThrow(/authorization_code/);
    await expect(discover(fakeAuthorizationServer({ server: SERVER, tokenEndpoint: "http://as.example.com/oauth/token" }).fetch, SERVER)).rejects.toThrow(/must be https/);
    // Loopback gets no https exemption: the SSRF guard refuses it before the scheme is looked at.
    await expect(discover(fakeAuthorizationServer({ server: SERVER, tokenEndpoint: "http://localhost:8080/oauth/token" }).fetch, SERVER)).rejects.toThrow(/token_endpoint: blocked host/);
    await expect(discover(fakeAuthorizationServer({ server: SERVER, tokenEndpoint: "http://127.0.0.1/oauth/token" }).fetch, SERVER)).rejects.toThrow(/token_endpoint: blocked address/);
    await expect(discover(fakeAuthorizationServer({ server: SERVER, authorizationServers: ["https://169.254.169.254/"] }).fetch, SERVER)).rejects.toThrow(OAuthError);
  });
});
...
describe("registerClient + PKCE + the authorization URL", () => {
  it("registers a public client with our callback as the only redirect URI and both grants; the authorization URL carries S256, state, resource and scope", async () => {
    ...
    expect(as.registrations[0]).toMatchObject({ client_name: "Switchboard", client_uri: "https://switchboard.test", redirect_uris: [CALLBACK], grant_types: ["authorization_code", "refresh_token"], response_types: ["code"], token_endpoint_auth_method: "none", scope: "mcp-api.all:write" });
    ...
    const pending: OAuthPending = { state, codeVerifier: verifier, clientId: client.clientId, tokenEndpoint: d.tokenEndpoint, redirectUri: CALLBACK, resource: d.resource, scope: "mcp-api.all:write" };
    const u = new URL(authorizationUrl(d, pending, challenge));
    expect(u.origin + u.pathname).toBe("https://as.example.com/oauth/authorize");
    expect(Object.fromEntries(u.searchParams)).toEqual({ response_type: "code", client_id: "client-1", redirect_uri: CALLBACK, state, code_challenge: challenge, code_challenge_method: "S256", resource: "https://mcp.example.com/mcp", scope: "mcp-api.all:write" });
  });
  ...
  it("no registration endpoint → a sentence; a refused registration names the status", async () => {
    ...
    await expect(registerClient(refusing.fetch, await discover(refusing.fetch, SERVER), CALLBACK)).rejects.toThrow(/refused.*HTTP 400/);
  });
});

describe("exchangeCode + refreshCredential", () => {
  const VERIFIER = "v".repeat(43);
  const pending = (): OAuthPending => ({ state: `${"n".repeat(24)}.abc`, codeVerifier: VERIFIER, clientId: "client-1", tokenEndpoint: "https://as.example.com/oauth/token", redirectUri: CALLBACK, resource: "https://mcp.example.com/mcp", scope: "mcp-api.all:write" });
  ...
  it("exchanges the code with the verifier, client id, redirect URI and resource; the credential carries an absolute expiry, the refresh token, the scope", async () => {
    ...
    expect(cred).toEqual({ kind: "oauth", accessToken: "at-1", refreshToken: "rt-1", expiresAt: NOW + 3600_000, clientId: "client-1", tokenEndpoint: "https://as.example.com/oauth/token", resource: "https://mcp.example.com/mcp", scope: "mcp-api.all:write" });
    expect(as.tokenRequests[0]).toEqual({ grant_type: "authorization_code", code, code_verifier: VERIFIER, client_id: "client-1", redirect_uri: CALLBACK, resource: "https://mcp.example.com/mcp" });
  ...
  it("a wrong verifier, a used code, or a non-bearer token type is refused with the server's error; nothing is invented", async () => {
    const as = fakeAuthorizationServer({ server: SERVER });
    const code = await as.codeFor(VERIFIER);
    await expect(exchangeCode(as.fetch, { ...pending(), codeVerifier: "x".repeat(43) }, code, NOW)).rejects.toThrow(/invalid_grant/);
  ...
  it("expires_in is range-checked: zero or negative is an already-dead token and refused; an absurd value is clamped to the cap; a non-numeric one is ignored", async () => {
    ...
    await expect(exchangeCode(zero.fetch, pending(), await zero.codeFor(VERIFIER), NOW)).rejects.toThrow(/already expired/);
    const negative = fakeAuthorizationServer({ server: SERVER, expiresIn: -5 });
    await expect(exchangeCode(negative.fetch, pending(), await negative.codeFor(VERIFIER), NOW)).rejects.toThrow(/already expired/);
    const absurd = fakeAuthorizationServer({ server: SERVER, expiresIn: 1e15 });
    expect((await exchangeCode(absurd.fetch, pending(), await absurd.codeFor(VERIFIER), NOW)).expiresAt).toBe(NOW + OAUTH_MAX_EXPIRES_IN_MS);
  ...
  it("needsRefresh is true inside the skew before expiry and never without an expiry; refresh keeps the old refresh token unless the server rotates it; no refresh token or a revoked one is a sentence", async () => {
    ...
    expect(as.tokenRequests.at(-1)).toEqual({ grant_type: "refresh_token", refresh_token: "rt-1", client_id: "client-1", resource: "https://mcp.example.com/mcp" });
    ...
    await expect(refreshCredential(as.fetch, { ...cred, refreshToken: undefined }, later)).rejects.toThrow(/no refresh token/);
    await expect(refreshCredential(as.fetch, { ...cred, refreshToken: "revoked" }, later)).rejects.toThrow(/invalid_grant/);
  });
});
...
describe("parseStoredCredential", () => {
  it("a raw string is a bearer token (even one starting with `{` that is not our JSON); our JSON is the OAuth set", () => {
    ...
    const cred = { kind: "oauth", accessToken: "a", clientId: "c", tokenEndpoint: "https://as.example.com/oauth/token", resource: "https://mcp.example.com/mcp" };
