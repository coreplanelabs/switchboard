...
import { FileMcpSecretStore, InMemoryMcpSecretStore, WorkerMcpSecretStore, type McpSecretStore } from "./secretStore.js";
...
const ticket: McpTicket = { nonce: "n".repeat(24), serverId: "user:slack:U1/vanta", requesterId: "slack:U1", createdAt: 1, expiresAt: Date.now() + 600_000, state: "pending" };
...
contract("FileMcpSecretStore", () => new FileMcpSecretStore(join(mkdtempSync(join(tmpdir(), "swb-mcp-secrets-")), "mcp-secrets.json")));
...
describe("WorkerMcpSecretStore (the ConfigDO secrets/tickets client)", () => {
  function fake(routes: Record<string, (body: Record<string, unknown>) => unknown>, status = 200) {
    ...
    return { calls, store: new WorkerMcpSecretStore({ baseUrl: "https://memory.test/", token: "tok", fetch: fetchImpl }) };
  }
  ...
  it("speaks the route contract with the bearer and validates every answer", async () => {
    ...
    expect(calls.map((c) => c.path)).toEqual(["/config/secrets/put", "/config/secrets/get", "/config/secrets/delete", "/config/tickets/put", "/config/tickets/get", "/config/tickets/transition", "/config/tickets/transition"]);
  ...
  it("a malformed answer is null, not a crash; non-2xx and transport failures throw naming the store", async () => {
    const { store } = fake({ "/config/secrets/get": () => ({ sealed: { serverId: "x" } }), "/config/tickets/get": () => ({ ticket: "nope" }) });
    expect(await store.getCredential("x")).toBeNull();
    expect(await store.getTicket(ticket.nonce)).toBeNull();
    await expect(fake({ "/config/tickets/get": () => ({}) }, 500).store.getTicket(ticket.nonce)).rejects.toThrow(/MCP secret store answered HTTP 500/);
