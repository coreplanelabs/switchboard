...
import { identityMatches, newTicket, planCallback, planComplete, planOpen, planStart, refusalMessage } from "./connect.js";
import { isMcpServerEntry, isMcpTicket, MCP_TICKET_STATES, MCP_TICKET_TTL_MS, MCP_TOKEN_MAX_CHARS } from "./registry.js";
...
describe("connect tickets (features/mcp-tools.md item 15)", () => {
  ...
  it("every refusal has a human sentence that names the next step", () => {
    ...
    expect(refusalMessage({ kind: "oauth_failed", reason: "no code" })).toBe("Sign-in with the server did not complete: no code.");
  });
});
...
describe("OAuth transitions (features/mcp-tools.md item 18)", () => {
  ...
  it("start: planOpen's identity rules (binding an unbound ticket to the starter), then `authorizing` with the sealed record attached", () => {
    ...
    expect(first.ok && first.bound).toBe(true);
    expect((first as { ticket: typeof t }).ticket).toMatchObject({ state: "authorizing", oauth: sealed, openedBy: { sub: "cf-2" } });
    const bound = (first as { ticket: typeof t }).ticket;
    ...
    expect(planStart({ ...t, state: "completed" }, other, sealed, NOW)).toEqual({ ok: false, refusal: { kind: "used" } });
    expect(planStart(t, other, sealed, NOW + MCP_TICKET_TTL_MS + 1)).toEqual({ ok: false, refusal: { kind: "expired" } });
  });
  ...
  it("callback: only an `authorizing` ticket, only its owner, once — a pending/opened ticket is `not_authorizing`, a completed one `used`", () => {
    ...
    expect(planCallback((done as { ticket: typeof t }).ticket, justin, NOW + 2)).toEqual({ ok: false, refusal: { kind: "used" } });
    expect(planCallback(started, justin, NOW + MCP_TICKET_TTL_MS + 1)).toEqual({ ok: false, refusal: { kind: "expired" } });
