...
export { StaticMcpToolSource, CompositeMcpToolSource, DiscoveringMcpToolSource, mcpGuidanceBlock, MCP_TOOLS_CACHE_TTL_MS, type McpToolSource, type McpToolsForRun, type McpServerOutcome } from "./source.js";
export { parseMcpSettings, DEFAULT_MCP_KEY_ENV, MCP_SERVER_NAME_PATTERN, type McpSettings } from "./config.js";
export { InMemoryMcpSecretStore, FileMcpSecretStore, WorkerMcpSecretStore, type McpSecretStore } from "./secretStore.js";
export { McpService, McpServiceError, ConfigMcpToolSource, MCP_OFF_MESSAGE, type McpActor, type McpTarget } from "./service.js";
export { importCredentialKey, sealCredential, openCredential, generateCredentialKeyBase64, randomNonce, type CredentialKey } from "./sealed.js";
...
export function buildMcp(
  ...
  opts: { publicBaseUrl?: string; secretsPath?: string; resolveEmail?: (userId: string) => Promise<string | undefined>; warn?: (m: string) => void; fetch?: typeof fetch } = {},
): McpWiring {
  ...
  if (worker) {
    ...
    secrets = new WorkerMcpSecretStore({ baseUrl: worker.baseUrl, token, ...(opts.fetch ? { fetch: opts.fetch } : {}) });
  } else {
