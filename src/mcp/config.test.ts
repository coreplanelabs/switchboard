...
describe("parseMcpSettings (the `mcp` block — docs/reference/specs/mcp-tools.md item 13)", () => {
  it("absent → undefined (MCP off); `mcp: {}` → the defaults", () => {
    ...
    expect(parseMcpSettings({ credentialKeyEnv: "MY_KEY", secretsPath: "./data/x.json" })).toEqual({
      credentialKeyEnv: "MY_KEY",
      secretsPath: "./data/x.json",
    });
  });
  ...
  it("refuses a non-mapping, a bad env name, a bad path — and the moved `servers` list, naming where servers live now", () => {
    ...
    expect(() => parseMcpSettings({ servers: [] })).toThrow(
      /mcp\.servers moved: declare servers as `defaults\.mcpServers`/,
    );
  });
});
