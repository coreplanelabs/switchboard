...
describe("the incarnation boundary in the resident Worker — minted at a runtime swap, never at a lifecycle transition", () => {
  // The Worker is untested by convention; this reads its source. The id is
  // minted exactly twice: the field initializer (isolate start) and
  // `swapIncarnation` (runtime replaced under the object). `setResidentState`
  // clears the memos and must not swap: the cycle that flips the state to
  // `refreshing` holds a lease of THIS incarnation and holds it afterwards —
  // a swap there made every cycle's own lease dead the moment it began.
  const worker = readFileSync(new URL("../../deploy/cloudflare-resident/worker.ts", import.meta.url), "utf8");

  it("mintIncarnationId is called at the field initializer and in swapIncarnation, nowhere else", () => {
    const mints = worker.match(/mintIncarnationId\(\)/g) ?? [];
    expect(mints).toHaveLength(2);
    expect(worker).toMatch(/private incarnation = mintIncarnationId\(\);/);
    expect(worker).toMatch(/private swapIncarnation\(\): void \{\n\s*this\.incarnation = mintIncarnationId\(\);/);
  });
  ...
  it("setResidentState clears the memos without swapping the incarnation, and the memo clear itself mints nothing", () => {
    const transition = methodBody("async setResidentState(");
    expect(transition).toContain("this.clearIncarnationMemos();");
    expect(transition).not.toContain("swapIncarnation");
    expect(transition).not.toContain("mintIncarnationId");
    expect(methodBody("private clearIncarnationMemos(): void {")).not.toContain("mintIncarnationId");
  });
});
