...
export async function rewriteRunCommits(input: RewriteInput): Promise<RewriteResult> {
  ...
  // The listed commits split into the start state (never judged, never
  // rebuilt) and the run's own. A `known` state subtracts by sha; a
  // `boundary` state cuts the list at the branch's pre-push head — and a
  // range that no longer holds that head is unreadable: the history below it
  // is not the run's to rewrite, and it cannot be told from the run's commits.
  const splitAtStart = (
    commits: ComparedCommit[],
  ): { start: StartCommit[]; run: ComparedCommit[] } | { unreadable: string } => {
    if (startState.kind === "boundary") {
      const sha = startState.sha.toLowerCase();
      const matches = commits.filter((c) => c.sha.toLowerCase().startsWith(sha));
      if (matches.length !== 1)
        return {
          unreadable:
            matches.length === 0
              ? `the branch's pre-push head ${sha} is not in the compare — the history below it is not the run's to rewrite`
              : `the branch's pre-push head ${sha} matches ${matches.length} commits in the compare`,
        };
      const at = commits.indexOf(matches[0]);
      return {
        start: commits.slice(0, at + 1).map((c) => ({
          sha: c.sha,
          author: { name: c.author.name, email: c.author.email },
          date: c.author.date,
          message: c.message,
        })),
        run: commits.slice(at + 1),
      };
    }
    const startShas = new Set(startState.commits.map((c) => c.sha));
    return { start: startState.commits, run: commits.filter((c) => !startShas.has(c.sha)) };
  };
  ...
  for (let read = 1; read <= MAX_READS; read += 1) {
    ...
    const split = splitAtStart(compare.commits);
    if ("unreadable" in split) return { kind: "unreadable", reason: split.unreadable };
    const allowed = {
      bot: input.bot,
