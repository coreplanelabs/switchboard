...
describe("githubRepoInspector", () => {
  it("reads the tree and package.json with the bearer, returning entries + the deciding fields only", async () => {
    ...
    expect(r).toEqual({
      ok: true,
      facts: {
        entries: ["package.json", "pnpm-lock.yaml", "apps"],
        packageJson: { scripts: { test: "turbo test" }, packageManager: "pnpm@10.10.0" },
      },
    });
  ...
  it("a 404 names the two causes (outside the installation / no such ref); other statuses carry the code", async () => {
    const r404 = await githubRepoInspector({ fetch: fakeFetch(() => ({ status: 404 })).fetch, token })(
      "acme/api",
      "develop",
    );
    expect(r404).toEqual({
      ok: false,
      reason:
        "GitHub answered 404 for acme/api@develop — the repo is outside the App installation, or the ref does not exist",
    });
    expect(
      await githubRepoInspector({ fetch: fakeFetch(() => ({ status: 502 })).fetch, token })("acme/api", "main"),
    ).toEqual({ ok: false, reason: "GitHub tree lookup failed: HTTP 502" });
  });
  ...
  it("no credential → a named reason, no request; a throwing fetch → named too", async () => {
    ...
    expect(await githubRepoInspector({ fetch, token: async () => null })("acme/api", "main")).toEqual({
      ok: false,
    ...
    expect(await githubRepoInspector({ fetch: boom, token })("acme/api", "main")).toEqual({
      ok: false,
      reason: "GitHub request failed (ECONNRESET)",
    });
  });

  it("an unparseable package.json is reported as null, not a failure", async () => {
    const { fetch } = fakeFetch((url) =>
      url.includes("/git/trees/") ? { status: 200, body: TREE } : { status: 200, text: "{ not json" },
    );
    expect(await githubRepoInspector({ fetch, token })("acme/api", "main")).toEqual({
      ok: true,
      facts: { entries: ["package.json", "pnpm-lock.yaml", "apps"], packageJson: null },
    });
  });
