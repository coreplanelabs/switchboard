...
// (docs/reference/specs/execution.md item 27): requests are held in memory and
// answered together by the ONE publish the lifecycle transition makes — no
// polling, no retry timer.
describe("RestoreWaiters — the ledger of held /await-restore requests (item 27)", () => {
  ...
  it("hold() registers before the caller reads the state; a withdrawn hold leaves the ledger and is not answered by a later publish, and withdrawing after a publish is a no-op", async () => {
    const ledger = new RestoreWaiters();
    const kept = ledger.hold();
    const withdrawn = ledger.hold();
    expect(ledger.size).toBe(2);
    withdrawn.withdraw();
    expect(ledger.size).toBe(1);
    let answered: unknown;
    void withdrawn.outcome.then((o) => (answered = o));
    ledger.publish({ state: "warm", reason: "" });
    await expect(kept.outcome).resolves.toEqual({ state: "warm", reason: "" });
    await Promise.resolve();
    expect(answered).toBeUndefined();
    expect(() => kept.withdraw()).not.toThrow();
    expect(ledger.size).toBe(0);
  });
});
