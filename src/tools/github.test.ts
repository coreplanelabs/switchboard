...
describe("toolset wiring", () => {
  ...
  it("conductor holds spawn_run, send_to_run, await_runs, list_runs, get_run_status, web_fetch, update_status, request_input and the GitHub reads — no shell, no files, no submit_*, no issue writes; no other toolset holds a run tool", () => {
    const runTools = ["spawn_run", "send_to_run", "await_runs", "list_runs", "get_run_status"];
    expect(names("conductor").sort()).toEqual(
      [...runTools, "web_fetch", "update_status", "request_input", ...reads, ...workReads].sort(),
    );
    for (const key of Object.keys(TOOLSETS).filter((k) => k !== "conductor"))
      for (const t of runTools) expect(names(key), `${key} ${t}`).not.toContain(t);
  });

  it("assistant has no shell, no file writes, no verdict/PR submission — GitHub, work tracking, web_fetch, status and questions only", () => {
    expect(names("assistant").sort()).toEqual(
      ["web_fetch", "update_status", "request_input", ...reads, ...writes, ...workReads, ...workWrites].sort(),
    );
  });
  ...
  it("explore relays update_status, request_input, web_fetch, web_search, the skill tools, the session tools and the GitHub reads — no submit_*, no issue writes, and none of pi's own workspace tools", () => {
