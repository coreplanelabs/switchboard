...
import { distillDiffStats, type DigestReport } from "../core/diffDigest.js";
...
// risky-file flags — NOT the raw diff. The coding agent shapes its PR
// description from it; the review agent orients with it. The parse/render lives
// in the pure distillDiffStats (src/core/diffDigest.ts); this tool only bridges
// the Executor, and reports the digest's totals to the run through
// `ctx.onDigest` so the review post-step can hold them against the PR's.
//
