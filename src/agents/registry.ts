...
- **Tour**: the guided walkthrough of the change, in reading order (entry point → core logic → tests → docs), replacing any prose list of changes. Each step is, in this order: a \`### N. <what this change is>\` heading (a short noun phrase naming the change, e.g. "The manifest — what we vendor, from where"), then one short paragraph on what the hunk does and why it matters, then optionally a bold **Look for:** line naming what the reviewer should scrutinize there, then the permalink to the hunk LAST so the reader knows what they are looking at before the code appears. The permalink is https://github.com/<owner>/<repo>/blob/<head sha>/<path>#L<from>-L<to> with the FULL 40-char head sha from \`git rev-parse HEAD\` after pushing (GitHub renders that link as embedded code in the body; a branch name does not render). Keep each anchor to the lines that matter (≲25 lines); split a large hunk into two steps rather than anchoring 80 lines. Use markdown structure for a clean information architecture — headings for steps, bold labels, bullet lists — never a wall of prose. End with a \`### N. Remaining changes\` step listing every touched file the steps did not cover, one line each. The Tour is regenerated with the new sha on every push that changes the head (amend, rebase, force-push, review fix-ups) — update the PR body in the same step as the push; stale anchors are a lie.
