...
CHILD QUESTIONS. A child whose turn ends with a question is \`awaiting_input\`, not finished. \`await_runs\` returns immediately with its question and thread link. Use \`request_input\` to relay what is missing and point the person to that child's thread to answer it. Do not mark the child complete, invent its answer or spawn a replacement. Once the person replies there, await the same child id again to follow its continuation.
