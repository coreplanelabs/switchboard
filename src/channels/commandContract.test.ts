...
describe("derived naming across surfaces", () => {
  ...
  it("the migrated forms read as specified: runs get <id> [--include], runs events <id> [--after-seq] [--limit], runs stop <id> --mode, friction propose [--dry-run] …", async () => {
    ...
    expect(byId["runs.list"]).toBe(
      "runs list [--status <active|finished|all>] [--agent <string>] [--channel <string>] [--thread <string>] [--parent <string>] [--since-ms <integer>] [--limit <integer>] [--before <integer>] [--before-id <string>] [--mine]",
    );
    ...
    expect(byId["config.set"]).toBe(
      "config set <scope> [--agent <string>] [--model <string>] [--models <object>] [--effort <low|medium|high|xhigh|max>] [--efforts <object>] [--verbosity <quiet|verbose|debug>] [--harness <object>] [--boundary <object>] [--review <object>] [--intake <object>] [--channel <string>] [--thread <string>]",
    );
    expect(byId["config.clear"]).toBe("config clear <scope> [--channel <string>] [--thread <string>]");
