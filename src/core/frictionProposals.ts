...
function proposalBody(p: FrictionPattern, runsAnalyzed: number): string {
  ...
  return [
    ...
    "## Provenance",
    "",
    `Generated by \`friction propose\` from the friction ledger: each run's event stream was diagnosed by \`analyzeRunFriction\` ([docs/reference/specs/run-friction.md](https://github.com/coreplanelabs/switchboard/blob/main/docs/reference/specs/run-friction.md)) and the diagnoses clustered across runs ([docs/reference/specs/self-improvement.md](https://github.com/coreplanelabs/switchboard/blob/main/docs/reference/specs/self-improvement.md)). Finding text is redacted at the source. Dedupe key \`${p.key}\`: re-running the pass will not refile this pattern while this issue is open.`,
  ].join("\n");
}
...
function suggestedFix(p: FrictionPattern): string {
  ...
  switch (p.kind) {
    ...
    case "wrap_up":
      return `Runs keep reaching the wrap-up warning (${p.runIds.length} runs; ${formatDuration(p.durationMs, "report")} spent winding down). Either the affected agent's ask (\`ASKS\` in \`src/core/budgets.ts\`) is too tight for this shape of work, or the prompt should push batching (fewer, larger tool calls) — the evidence rows say which agent and how close to the deadline each run got.`;
    case "budget_hit":
      return p.signature === "turns"
        ...
        : `Runs exhaust the TIME budget and are cut off mid-work. Raise the affected agent's ask (\`ASKS\` in \`src/core/budgets.ts\`; the fit check says what else must move), or split the task shape that triggers it — a run that is forced to write up findings is a run whose work was wasted.`;
    case "infra_failure":
