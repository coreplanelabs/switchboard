...
const echo = define({
  id: "demo.echo",
  options: z.object({
    status: z.enum(["active", "finished", "all"]).describe("which"),
    limit: z.coerce.number().int().positive().optional().describe("how many"),
  }),
  action: "runs:read",
  effect: "read",
...
const whoami = define({
  ...
  handler: async ({ caller }) => ({
    kind: caller.kind,
    id: caller.id,
    actor: caller.actor ? `${caller.actor.kind} ${caller.actor.id}` : null,
  }),
});
...
describe("parseChatCommand", () => {
  ...
  it("recognizes a registered `<group> <verb>` and binds --kebab flags (value, =value, quoted) into camelCase options", () => {
    expect(parseChatCommand("demo echo --status all --limit 5", registry)).toEqual({
      kind: "invoke",
      id: "demo.echo",
      input: { args: [], options: { status: "all", limit: "5" } },
    });
    expect(parseChatCommand('  demo echo --status="all"  ', registry)).toEqual({
      kind: "invoke",
      id: "demo.echo",
      input: { args: [], options: { status: "all" } },
    });
    expect(parseChatCommand("demo echo --status 'all'", registry)).toEqual({
      kind: "invoke",
      id: "demo.echo",
    ...
    expect(parseChatCommand("demo echo", registry)).toEqual({
      kind: "invoke",
      id: "demo.echo",
      input: { args: [], options: {} },
    });
  ...
  it("unwraps Slack link markup before binding: `<url>`, auto-link `<url|label>` and a custom-labelled link all bind the bare url, with `&amp;`/`&lt;`/`&gt;` restored; a mention stays out of the tail", () => {
    ...
    expect(parseChatCommand("demo url --url <https://mcp.example.com/mcp>", registry)).toEqual({
      kind: "invoke",
      id: "demo.url",
      input: { args: [], options: { url: "https://mcp.example.com/mcp" } },
    });
    expect(parseChatCommand("demo url --url <https://mcp.example.com/mcp|mcp.example.com/mcp>", registry)).toEqual({
      kind: "invoke",
      id: "demo.url",
      input: { args: [], options: { url: "https://mcp.example.com/mcp" } },
    });
    expect(parseChatCommand("demo url --url <https://mcp.example.com/mcp?a=1&amp;b=2|the server>", registry)).toEqual({
      kind: "invoke",
      id: "demo.url",
      input: { args: [], options: { url: "https://mcp.example.com/mcp?a=1&b=2" } },
    });
    // All three entities Slack escapes in message text are restored, not just `&amp;`.
    expect(parseChatCommand("demo url --url <https://mcp.example.com/mcp?q=a&lt;b&gt;c&amp;d>", registry)).toEqual({
      kind: "invoke",
      id: "demo.url",
      input: { args: [], options: { url: "https://mcp.example.com/mcp?q=a<b>c&d" } },
    });
    // Only http(s) links unwrap; a leading mention (or any other `<…>`) is left to the grammar as before.
    expect(parseChatCommand("demo say me see <@U123> and <#C456|general>", registry)).toEqual({
      kind: "invoke",
      id: "demo.say",
      input: { args: ["me", "see <@U123> and <#C456|general>"], options: {} },
    });
  });

  it("binds positionals, including free text with Slack smart quotes", () => {
    expect(parseChatCommand("demo say me be terse, always", registry)).toEqual({
      kind: "invoke",
      id: "demo.say",
      input: { args: ["me", "be terse, always"], options: {} },
    });
    expect(parseChatCommand("demo say channel “quoted  text”", registry)).toEqual({
      kind: "invoke",
      id: "demo.say",
      input: { args: ["channel", "quoted  text"], options: {} },
    });
    expect(parseChatCommand("demo missing abc", registry)).toEqual({
      kind: "invoke",
      id: "demo.missing",
      input: { args: ["abc"], options: {} },
    });
  });
  ...
  it("prose is never a command: a mid-sentence mention, an unknown verb or group, a chat-hidden id → null; no form is reserved for a legacy parser any more", () => {
    ...
    expect(parseChatCommand("config show", registry)).toEqual({
      kind: "invoke",
      id: "config.show",
      input: { args: [], options: {} },
    });
  ...
  it("the bare word `help` is `help show` when it is registered and chat-exposed — else prose (KTD25)", () => {
    ...
    withHelp.register(
      define({ id: HELP_COMMAND_ID, action: "help:read", effect: "read", describe: "help", handler: async () => ({}) }),
    );
    expect(parseChatCommand("help", withHelp)).toEqual({
      kind: "invoke",
      id: "help.show",
      input: { args: [], options: {} },
    });
    ...
    expect(parseChatCommand("help show", withHelp)).toEqual({
      kind: "invoke",
      id: "help.show",
      input: { args: [], options: {} },
    });
  ...
  it("a recognized command with a malformed tail is an `invalid_input` reply (the registry's own code, one vocabulary) naming the flag/argument — never a model turn, never the value", () => {
    expect(parseChatCommand("demo echo please", registry)).toEqual({
      kind: "reply",
      error: "invalid_input",
      text: "⚠️ `demo echo`: demo echo takes no arguments\nusage: demo echo --status <active|finished|all> [--limit <integer>]",
    });
    expect(parseChatCommand("demo echo --status", registry)).toMatchObject({
      kind: "reply",
      error: "invalid_input",
      text: expect.stringContaining("option --status needs a value"),
    });
    expect(parseChatCommand("demo echo --bogus s3cret", registry)).toMatchObject({
      kind: "reply",
      error: "invalid_input",
      text: expect.stringContaining("unknown option --bogus"),
    });
    expect(JSON.stringify(parseChatCommand("demo echo --bogus s3cret", registry))).not.toContain("s3cret");
    expect(parseChatCommand("demo echo status=all", registry)).toMatchObject({
      kind: "reply",
      error: "invalid_input",
      text: expect.stringContaining("takes no arguments"),
    });
    expect(parseChatCommand("demo missing", registry)).toMatchObject({
      kind: "reply",
      error: "invalid_input",
      text: expect.stringContaining("missing argument <id>"),
    });
    expect(parseChatCommand('demo say me "open quote', registry)).toEqual({
      kind: "reply",
      error: "invalid_input",
      text: "⚠️ `demo say`: unterminated quote",
    });
  ...
  it("help is derived and chat-shaped: `<group> <verb> --help` gives the command's help, `<group> help` lists the group's chat commands — bullets and code spans, never padded columns", () => {
    expect(parseChatCommand("demo echo --help", registry)).toEqual({
      kind: "reply",
      text: [
        "echoes its parsed options",
        "usage: `demo echo --status <active|finished|all> [--limit <integer>]`",
        "*options*",
        "• `--status <active|finished|all>` — which (required)",
  ...
  it("a group's verbs are recognized independently: a registered verb binds, an unregistered sibling is prose", () => {
    ...
    repo.register(
      define({ id: "repo.list", action: "repo:read", effect: "read", describe: "list", handler: async () => ({}) }),
    );
    expect(parseChatCommand("repo list", repo)).toEqual({
      kind: "invoke",
      id: "repo.list",
      input: { args: [], options: {} },
    });
...
describe("chatCallerFor", () => {
  ...
  it("carries the message's user as the Actor the table decides on (plan U2/U4): admin → everything, a plain user → the open chat commands + the unrestricted agents, a machine channel's user → a service actor", async () => {
    ...
    const config = configStore(ADMIN_YAML);
    expect(chatCallerFor(msg("x", "slack:UADMIN"), config).actor).toEqual({
      kind: "user",
      id: "slack:UADMIN",
      grants: ALL_GRANTS,
    ...
    expect(machine.actor).toMatchObject({
      kind: "service",
      id: "http:cron",
    ...
    expect(schedule.actor).toMatchObject({
      kind: "schedule",
      id: "schedule:self-improvement",
      grants: { actions: new Set(["friction:read", "friction:write"]), channels: "all" },
    });
  });
});
...
describe("handleChatCommand", () => {
  ...
  it("not_found → one line with the handler's message; a rejected/help parse is replied as-is without invoking, the rejection carrying `invalid_input` like a registry refusal would", async () => {
    ...
    const rejected = await invokeChatCommand({
      commands,
      parsed: parseChatCommand("demo echo please", commands)!,
      msg: msg("demo echo please", "slack:UX"),
      config,
    });
    ...
    const help = await invokeChatCommand({
      commands,
      parsed: parseChatCommand("demo echo --help", commands)!,
      msg: msg("demo echo --help", "slack:UX"),
      config,
    });
  ...
  it("every chat caller carries its Actor and no channel pin: a machine credential speaking as text (`http:`/`mcp:`) is a service actor whose grants — not the channel it speaks in — decide what it may run and see (authorization.md item 7)", async () => {
    ...
    const via = (channelId: string, userId: string) =>
      handleChatCommand({ commands, parsed, msg: { channelId, userId, threadKey: `${channelId}:t` }, config });
...
describe("runs list on chat (KTD18)", () => {
  ...
  async function setup() {
    ...
    reg.create("review · acme/api <!channel>", {
      agent: "review",
      channelId: "slack:D0PRIVATE",
      userId: "slack:UOWNER",
      threadKey: "slack:D0PRIVATE:t",
    });
  ...
  it("an admin's `runs list --status all` renders short id · agent · status · duration only — no channel, user, thread, label, or token", async () => {
    ...
    const reply = await handleChatCommand({
      commands,
      parsed: parsed!,
      msg: msg("runs list --status all", "slack:UADMIN"),
      config,
      now: NOW,
    });
    expect(reply.split("\n")).toEqual([
  ...
  it("`runs get/events/friction <id>` are not chat commands (opt-out); `runs stop <id> --mode soft` is", async () => {
    ...
    expect(parseChatCommand("runs stop live0001 --mode soft", commands)).toEqual({
      kind: "invoke",
      id: "runs.stop",
