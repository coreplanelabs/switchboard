 ...
 *  serves; the block names it so the model knows whose gateway it is. */
export function selfDescriptionBlock(
  ...
  const agentLines = Object.values(agents)
    ...
    .join("; ");
  return [
    `${SELF_DESCRIPTION_HEADER} you are Switchboard, the agent gateway of the ${organization} organization. A message arrives on a channel (Slack — an @-mention, thread follow-ups stay on the same agent — plus a CLI, an HTTP /api and an MCP surface), is routed to an agent, runs on a configured provider/model, and executes tools through an executor.`,
