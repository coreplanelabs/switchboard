...
### 1. The template — Tour replaces Changes and How to review

The shared `PR_DESCRIPTION_TEMPLATE` (both coding prompts embed it). Every section is a `##` heading, the TL;DR included. The Tour bullet fixes the step shape — `### N. <what this change is>` heading → one paragraph → optional **Look for:** → permalink last — in reading order (entry point → core logic → tests → docs), anchors ≲25 lines, a `Remaining changes` catch-all, markdown structure over prose, and the repush rule: the sha comes from `git rev-parse HEAD` after pushing and the Tour is regenerated on every push that moves the head.
...
- **Author-written, not generated.** Inspect generates its Tour with a second cheap model call at PR-open. Ours is written by the coding agent inside the same run: strictly more context, zero extra cost. The trade-off is that it depends on the agent following the template — covered by the `[agent]` criterion, not by code.
- **Heading → explanation → code, not code first.** Inspect shows the hunk then the note. A reader who sees code before knowing what it is for has to read it twice; the heading names the change, the paragraph says why, and the permalink confirms it.
- **Full 40-char sha, spelled out.** A branch ref renders as a bare link; the full sha always embeds. (Reviewer nit on the first push: unambiguous short shas do embed too — the full sha is still the safer instruction.)
- **Repush regeneration is in the template, not a post-step — for now.** The agent already does the push; "update the PR body in the same step" is the smallest correct change. The proper fix is the data pipeline: anchors stored as (path, from, to) and rendered against the head sha, so a repush is a re-render — that is what `renderPrDescriptionMarkdown` does, and the roadmap item tracks wiring the agent to it.
- **Dropped "How to review" rather than keeping both.** The Tour *is* the ordered review guide; two sections saying where to start would drift.

## Risks & implications

Prompt-only change to `PR_DESCRIPTION_TEMPLATE`; no runtime code paths change. Blast radius: the shape of every PR body the coding agent opens from the next bot deploy. Existing PRs untouched. If the agent anchors badly (wrong lines after a rebase) the body degrades to a link that still opens the file — no worse than the prose it replaces.

## Validation

`npm run typecheck` clean; `npm test` 107 files, 2458 passed, 2 skipped.

| Criterion | Proof |
|---|---|
| Both coding prompts carry the six sections incl. `**Tour**`; honesty rules unchanged | `[unit]` `registry.test.ts::coding prompts: templated PR description by default::both coding prompts include every PR-description section`, `::both prompts state the rules…` |
| `**Changes**` / `**How to review**` are gone from both prompts | `[unit]` `::the Tour supersedes the prose Changes / How-to-review sections` (red before: both present) |
| Every section is a `##` heading, `## TL;DR` first | `[unit]` `::every section is a \\`##\\` heading, the TL;DR included (no headingless opening paragraph)` |
| Permalink shape `blob/<head sha>/<path>#L<from>-L<to>`, reading order, Remaining-changes catch-all | `[unit]` `::Tour steps are anchored to head-sha line permalinks in reading order, with a catch-all step` |
