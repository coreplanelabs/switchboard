...
describe("tokenize", () => {
  it("splits on whitespace, honors double/single quotes (also inside a token), normalizes smart quotes", () => {
    expect(tokenize("runs stop abc --mode soft")).toEqual({
      ok: true,
      tokens: ["runs", "stop", "abc", "--mode", "soft"],
    });
    expect(tokenize(`a "b c" 'd e' --repo="x/y" f`)).toEqual({
      ok: true,
      tokens: ["a", "b c", "d e", "--repo=x/y", "f"],
    });
    expect(tokenize("config instructions me “be terse, always”")).toEqual({
      ok: true,
      tokens: ["config", "instructions", "me", "be terse, always"],
    });
...
describe("parseInvocation — the one grammar", () => {
  it("binds positionals to args in order and --kebab flags (value, =value) to camelCase options", () => {
    expect(parseInvocation(stop, ["abc", "--mode", "soft"])).toEqual({
      kind: "invoke",
      input: { args: ["abc"], options: { mode: "soft" } },
    });
    expect(parseInvocation(stop, ["--mode=hard", "abc"])).toEqual({
      kind: "invoke",
      input: { args: ["abc"], options: { mode: "hard" } },
    });
  ...
  it("boolean options: --flag → true, --no-flag → false, --flag=false → the string the schema coerces", () => {
    expect(parseInvocation(propose, ["--dry-run"])).toMatchObject({
      kind: "invoke",
      input: { options: { dryRun: true } },
    });
    expect(parseInvocation(propose, ["--no-dry-run"])).toMatchObject({
      kind: "invoke",
      input: { options: { dryRun: false } },
    });
    expect(parseInvocation(propose, ["--dry-run=false"])).toMatchObject({
      kind: "invoke",
      input: { options: { dryRun: "false" } },
    });
    // a boolean flag never swallows the next token
    expect(parseInvocation(propose, ["--dry-run", "--top", "2"])).toMatchObject({
      kind: "invoke",
      input: { options: { dryRun: true, top: "2" } },
    });
    expect(parseInvocation(propose, ["--no-dry-run=x"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringContaining("takes no value"),
    });
  });
  ...
  it("dotted keys nest: --models.coding x --models.review y → { models: { coding, review } }", () => {
    ...
    expect(parseInvocation(propose, ["--models", "x", "--models.coding", "y"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
  ...
  it("a trailing rest argument takes every remaining positional joined with single spaces", () => {
    expect(parseInvocation(instructions, ["me", "be", "terse,", "always"])).toEqual({
      kind: "invoke",
      input: { args: ["me", "be terse, always"], options: {} },
    });
    expect(parseInvocation(instructions, ["me", "be terse", "please"])).toEqual({
      kind: "invoke",
      input: { args: ["me", "be terse please"], options: {} },
    });
    expect(parseInvocation(instructions, ["channel"])).toEqual({
      kind: "invoke",
      input: { args: ["channel"], options: {} },
    });
    // `--` ends options: a leading dash inside the free text is fine after it
    expect(parseInvocation(instructions, ["me", "--", "--not-a-flag", "x"])).toEqual({
      kind: "invoke",
      input: { args: ["me", "--not-a-flag x"], options: {} },
    });
  });
  ...
  it("a rejected tail is the registry's own `invalid_input` (one error vocabulary); the message names the flag or the argument, never a value, and ends with the usage line", () => {
    ...
    expect(parseInvocation(stop, ["--mode", "soft"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringMatching(/^missing argument <id>/),
    });
    expect(parseInvocation(stop, ["abc", "--mode"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringMatching(/^option --mode needs a value/),
    });
    expect(parseInvocation(stop, ["abc", "extra-secret", "--mode", "soft"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringMatching(/^unexpected argument: runs stop takes at most 1/),
    });
    expect(parseInvocation(propose, ["please"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringMatching(/^friction propose takes no arguments/),
    });
    expect(parseInvocation(stop, ["abc", "--mode", "soft", "--mode", "hard"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringContaining("given twice"),
    });
    ...
    expect(parseInvocation(stop, ["abc", "-m", "soft"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringContaining("unknown option -m"),
    });
    expect(parseInvocation(stop, ["abc", "--Mode", "soft"])).toMatchObject({
      kind: "invalid",
      code: "invalid_input",
      error: expect.stringContaining("unknown option --Mode"),
    });
  });
  ...
  it("--help / -h anywhere asks for help; a negative number is a positional, not a flag", () => {
    ...
    expect(parseInvocation(instructions, ["me", "-5", "degrees"])).toMatchObject({
      kind: "invoke",
      input: { args: ["me", "-5 degrees"] },
    });
  });
});
...
describe("namedToInput — the JSON surfaces address arguments and options by name", () => {
  it("splits a flat object into positional args (declared order) and options; kebab query keys map to camelCase; dotted keys nest", () => {
    expect(namedToInput(stop, { id: "abc", mode: "soft" }, "camel")).toEqual({
      args: ["abc"],
      options: { mode: "soft" },
    });
    expect(namedToInput(propose, { "min-runs": "2", "dry-run": "true", "models.coding": "m" }, "kebab")).toEqual({
      args: [],
      options: { minRuns: "2", dryRun: "true", models: { coding: "m" } },
    });
    expect(namedToInput(propose, { minRuns: 2, models: { coding: "m" } }, "camel")).toEqual({
      args: [],
      options: { minRuns: 2, models: { coding: "m" } },
    });
  ...
  it("an unknown name lands in options (the registry names it as unexpected); a nesting conflict is an error", () => {
    ...
    expect(namedToInput(propose, { models: "x", "models.coding": "y" }, "camel")).toEqual({
      error: "option models.coding given twice",
    });
  });
...
describe("jsonSchemaFor — arguments (by name) + options merged", () => {
  it("lists args and options as properties, required = non-optional args + required options, no extra properties", () => {
    const schema = jsonSchemaFor(stop) as {
      type: string;
    ...
    expect(schema.properties.mode).toMatchObject({
      enum: ["soft", "hard"],
      description: "soft = finish the step; hard = abort",
    });
  ...
  it("an optional rest argument is a plain string property, not required; a boolean flag accepts boolean or 'true'/'false'; coerced ints are integers", () => {
    const s = jsonSchemaFor(instructions) as {
      properties: Record<string, Record<string, unknown>>;
      required?: string[];
    };
    ...
    const p = jsonSchemaFor(propose) as {
...
describe("help", () => {
  ...
  it("catalogueText is one aligned line per command", () => {
    expect(catalogueText([stop, propose]).split("\n")).toEqual([
      "  runs stop         — Stop a live run.",
      "  friction propose  — File proposals.",
    ]);
  });
  ...
  it("chatHelpText: description, usage in a code span, one bullet per argument and option, no column padding", () => {
    ...
    const bare = define({
      id: "x.y",
      options: z.object({ quiet: flag.optional() }),
      action: "x:read",
      effect: "read",
      describe: "x",
      handler: async () => ({}),
