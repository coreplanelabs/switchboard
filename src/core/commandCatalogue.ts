...
export function buildCoreCommands(
  ...
  const deps: CoreCommandDeps = {
    ...
    config: {
      ...
      setOrgOverride: async (p) => (await cfg()).setOrgOverride(p),
      setRepoOverride: async (r, p) => (await cfg()).setRepoOverride(r, p),
      clearRepoOverride: async (r) => (await cfg()).clearRepoOverride(r),
