...
describe("mcp.* commands", () => {
  ...
  it("MCP off → `unavailable` with the standard sentence on every command", async () => {
    ...
    for (const [id, input] of [
      ...
      expect(await inv.invoke(id, input, cli)).toMatchObject({ ok: false, error: "unavailable", message: MCP_OFF_MESSAGE });
    }
  });
  ...
  it("add for yourself lands in your config scope; the reply carries the one-time link and never a token; the empty list points at the command", async () => {
    ...
    const inv = bind(svc);
    expect(await text(inv, "mcp.list", {}, chat(ALICE))).toContain("No MCP servers reach your runs here. Add one for yourself with `mcp add <name> --url <url>`");
    const added = await text(inv, "mcp.add", { args: ["vanta"], options: { url: "https://mcp.vanta.com/mcp" } }, chat(ALICE));
    expect(added).toContain("`vanta` (user) ⏳ awaiting credential");
    expect(added).toContain("agents: general, research · auth: bearer");
    expect(added).toContain("open this link and paste the server's token (only you can complete it; it expires in 10 min): https://switchboard.test/mcp/connect/nonce-00000000000000000001");
    expect(backing.document?.users[ALICE].mcpServers?.vanta).toMatchObject({ url: "https://mcp.vanta.com/mcp", auth: "bearer", addedBy: ALICE });
    expect(await text(inv, "mcp.list", {}, chat(ALICE))).toContain("`vanta` (user) ⏳ awaiting credential — https://mcp.vanta.com/mcp");
  ...
  it("channel and org scopes are decided by the data: channel needs the channelConfig gate, org an admin / cli / machine mcp:write; refusals name `--scope me`", async () => {
    ...
    const deniedOrg = await inv.invoke("mcp.add", { args: ["linear"], options: { url, scope: "org", auth: "none" } }, chat(ALICE));
    ...
    expect(await inv.invoke("mcp.add", { args: ["linear"], options: { url, scope: "org", auth: "none", agents: "general,coding" } }, chat(ADMIN))).toMatchObject({ ok: true });
    expect(await inv.invoke("mcp.add", { args: ["notion"], options: { url: "https://mcp.notion.so/mcp", scope: "org", auth: "none" } }, machine(["mcp:write"]))).toMatchObject({ ok: true });
    expect(await inv.invoke("mcp.add", { args: ["x"], options: { url, scope: "org", auth: "none" } }, machine(["mcp:read"]))).toMatchObject({ ok: false, error: "unauthorized", decidedBy: "registry" });
    const deniedChannel = await inv.invoke("mcp.add", { args: ["hubspot"], options: { url: "https://mcp.hubspot.com/mcp", scope: "channel", auth: "none" } }, chat(NOBODY, { channelConfig: false }));
    expect(deniedChannel).toMatchObject({ ok: false, error: "unauthorized", decidedBy: "handler" });
    expect(await inv.invoke("mcp.add", { args: ["hubspot"], options: { url: "https://mcp.hubspot.com/mcp", scope: "channel", auth: "none" } }, chat(ALICE))).toMatchObject({ ok: true });
    // A machine caller must name the channel.
    expect(await inv.invoke("mcp.add", { args: ["h2"], options: { url: "https://h2.example/mcp", scope: "channel", auth: "none" } }, machine(["mcp:write"]))).toMatchObject({ ok: false, error: "invalid_input" });
    ...
    expect(await inv.invoke("mcp.remove", { args: ["linear"], options: { scope: "org" } }, chat(ALICE))).toMatchObject({ ok: false, error: "unauthorized" });
    expect(await inv.invoke("mcp.remove", { args: ["linear"], options: { scope: "org" } }, chat(ADMIN))).toMatchObject({ ok: true, value: { removed: true, name: "linear", scope: "org" } });
  });

  it("semantic refusals carry the service's codes; grammar refusals the registry's", async () => {
    const inv = bind(service().svc);
    expect(await inv.invoke("mcp.add", { args: ["v"], options: { url: "https://x.example/mcp", agents: "coding" } }, chat(ALICE))).toMatchObject({ ok: false, error: "invalid_input", decidedBy: "handler" });
    await inv.invoke("mcp.add", { args: ["v"], options: { url: "https://x.example/mcp", auth: "none" } }, chat(ALICE));
    expect(await inv.invoke("mcp.add", { args: ["v"], options: { url: "https://x.example/mcp", auth: "none" } }, chat(ALICE))).toMatchObject({ ok: false, error: "conflict" });
    expect(await inv.invoke("mcp.show", { args: ["nope"] }, chat(ALICE))).toMatchObject({ ok: false, error: "not_found" });
    expect(await inv.invoke("mcp.connect", { args: ["v"] }, chat(ALICE))).toMatchObject({ ok: false, error: "invalid_input" }); // auth none needs no credential
    expect(await inv.invoke("mcp.add", { args: ["Bad Name"], options: { url: "https://x.example/mcp" } }, chat(ALICE))).toMatchObject({ ok: false, error: "invalid_input", decidedBy: "registry" });
    expect(await inv.invoke("mcp.add", { args: ["ok"], options: { url: "ftp://x" } }, chat(ALICE))).toMatchObject({ ok: false, error: "invalid_input", decidedBy: "registry" });
  });
