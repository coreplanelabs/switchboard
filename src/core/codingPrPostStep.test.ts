...
// The open-PR lookup for a description-less push: no branch heads an open PR
...
describe("runCodingPrPostStep (callable with explicit inputs)", () => {
  ...
  // A follow-up on an existing PR: the run repushed the PR's OWN head branch
...
describe("runCodingPrPostStep — the identity rewrite before the open (record 0062)", () => {
  ...
  it("a pushed branch other than the one the start state was read for is judged over an EMPTY start state — the run cut it itself", async () => {
    const spy = openSpy();
