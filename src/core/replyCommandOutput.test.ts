...
// has one — lead line as the message, the whole text as `<group>-<verb>.txt`.
...
describe("replyCommandOutput", () => {
  ...
  it("a long reply on a channel with attach: the first line leads, the whole text is the file, named after the command", async () => {
    ...
    expect(file.name).toBe("mcp-show.txt");
    expect(file.text).toBe(long);
  ...
  it("a short reply is a plain reply even with attach; a long one without attach is a plain reply; a help reply attaches as `command.txt`", async () => {
    const short = io(true);
    ...
    expect(usage.attach.mock.calls[0][0].name).toBe("command.txt");
  });
});
