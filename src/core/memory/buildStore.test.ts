...
// Feature: docs/reference/specs/memory.md — process-startup store selection:
...
describe("buildMemoryStore", () => {
  it("memory disabled/absent → undefined (the dispatcher then uses NullMemoryStore)", () => {
    ...
    expect(buildMemoryStore(undefined, secretsFrom({}), (m) => warnings.push(m))).toBeUndefined();
    expect(
      buildMemoryStore({ enabled: false, worker: { baseUrl: "https://m" } }, secretsFrom({ MEMORY_TOKEN: "t" }), (m) =>
        warnings.push(m),
      ),
    ).toBeUndefined();
  ...
  it("an out-of-range or non-integer memory.maxRecordsPerScope warns and falls back to the default instead of evicting everything / 400ing every write", async () => {
    for (const bad of [0, -5, 2.5, 10_001, Number.NaN]) {
      const warnings: string[] = [];
      const store = buildMemoryStore({ enabled: true, maxRecordsPerScope: bad }, secretsFrom({}), (m) =>
        warnings.push(m),
      )!;
  ...
  it("enabled without a worker → in-process store and a warning naming the restart loss", () => {
    const warnings: string[] = [];
    const store = buildMemoryStore({ enabled: true }, secretsFrom({}), (m) => warnings.push(m));
