...
import { reclaimAtBoot } from "./boot.js";
...
describe("reclaimAtBoot", () => {
  it("closes an expired run `interrupted` with a record built from the ledger's events, meta and identity; the row, steps and transcript go; the completeness verdict is the reason", async () => {
    ...
    expect(outcome.closed).toEqual([
      {
        ...
        status: "interrupted",
        from: "live",
        why: expect.stringMatching(/^resumable: the transcript's 2 turns match the last step record/),
    ...
    expect(logs.some((l) => l.includes("r1 slack:C1:1.0 closed interrupted (from live; resumable"))).toBe(true);
  });
  ...
  it("a row with no step record was killed before its conversation was stored: interrupted, and the reason says so; a handed-off row names its phase", async () => {
    const { ledger, run } = harness();
    ...
    expect(byId.handed).toMatchObject({
      status: "interrupted",
      from: "handoff",
      why: expect.stringMatching(/^resumable/),
    });
  });

  it("a partial step write (one turn past the record) is named as such", async () => {
    const { ledger, run } = harness();
    await ledger.claim(claim("partial", "slack:C1:1.0"));
    await ledger.seed("partial", "g1", [{ idx: 0, message: user("go") }]);
    await ledger.step("partial", "g1", seedRecord(1), []);
    await ledger.seed("partial", "g1", [{ idx: 1, message: assistant("half") }]); // one turn, no record
    const outcome = await run();
    expect(outcome.closed[0].why).toMatch(/partial step write/);
  });
  ...
  it("a ledger without the routes, or one that cannot be reached, is a warning and an empty outcome — the bot boots", async () => {
    ...
    expect(await missing.run()).toEqual({ closed: [], liveElsewhere: [], failed: [] });
    ...
    expect(await down.run()).toEqual({ closed: [], liveElsewhere: [], failed: [] });
