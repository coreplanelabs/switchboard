...
describe("resident repo dispatch", () => {
  ...
  it("a resident fallback note appears in the status frames and on the run's stream as a cold_sandbox note (named, never silent)", async () => {
    ...
    vi.stubEnv("GITHUB_APP_ID", "");
    // The /status probe answers restoring; the /await-restore subscription
    // answers restoring too (execution.md item 23), so the run falls to the
    // per-thread backend with the wait named (the coding PR post-step's
    // head/branch probe goes to the sandbox backend).
    const fetchSpy = vi.fn(
      async (_url: unknown) =>
    ...
    expect(
      statuses.some((s) =>
        /resident restoring \(rehydrating\) after waiting \d+s for the resident's restore — using fresh sandbox/.test(
          s.title,
        ),
      ),
    ).toBe(true);
    ...
    const loopAt = events.findIndex((e) => e.type === "span_start" && e.name === "run.agent");
    expect(String((events[noteAt] as { summary?: string }).summary)).toMatch(
