...
const VOCAB: LegacyVocabulary = {
  ...
  commandGroups: ["runs", "friction", "repo", "config"],
};
const translate = (
  p: Parameters<typeof translateLegacyConfig>[0],
  tokens?: Parameters<typeof translateLegacyConfig>[1],
  svc?: Parameters<typeof translateLegacyConfig>[2],
  vocab = NONE,
) => translateLegacyConfig(p, tokens, svc, vocab);
...
describe("translateLegacyConfig — the KTD6 table, key by key", () => {
  ...
  it("permissions.operators → every <group>:read and <group>:write of every registered group (never :exec), over every channel (OQ1: fleet-wide today)", () => {
    ...
    expect(t.get("access:op-1")).toEqual(
      grants({
        actions: set(
          "runs:read",
          "runs:write",
  ...
  it("the `open` chat gate → CHAT_OPEN_ACTIONS for everyone: the reads every Slack user had, plus memory:write and mcp:write; never runs, deploy, env, repo:write, friction:write, or an exec", () => {
    ...
    expect(CHAT_OPEN_ACTIONS).toEqual([
      "help:read",
      "config:read",
      "repo:read",
      "friction:read",
    ...
    for (const closed of [
      ...
      expect(everyone.actions, closed).not.toContain(closed);
  });
  ...
  it("permissions.agents.<name>: [users] → agent:run:<name> for those users only", () => {
    const { grants: t, everyone } = translate(
      { agents: { coding: ["slack:UDEV"], review: ["slack:UDEV", "slack:UREV"] } },
      undefined,
      undefined,
      VOCAB,
    );
  ...
  it("an agent with NO allowlist → agent:run:<name> for everyone (canRunAgent is true for anyone); every agent restricted → everyone holds the open commands only", () => {
    expect(translate({}, undefined, undefined, VOCAB).everyone).toEqual(
      grants({ actions: set(...OPEN, "agent:run:general", "agent:run:coding", "agent:run:review") }),
    );
    expect(
      translate({ agents: { general: [], coding: [], review: [] } }, undefined, undefined, VOCAB).everyone,
    ).toEqual(grants({ actions: set(...OPEN) }));
  ...
  it("permissions.repos → repos per listed user (slugs as given — validateConfig lowercases them at load)", () => {
    const { grants: t, reposOpen } = translate({
      repos: { "acme/api": ["slack:UDEV"], "acme/web": ["slack:UDEV", "slack:UWEB"] },
    });
  ...
  it("two tokens for one subject union their scopes and pins", () => {
    const { grants: t } = translate(undefined, {
      a: { subject: "ci", channel: "ops", scopes: ["dispatch"] },
      b: { subject: "ci", channel: "dev", scopes: ["runs:read"] },
    });
    expect(t.get("http:ci")).toEqual(
      grants({ actions: set("dispatch", "runs:read"), channels: set("http:ops", "http:dev") }),
    );
  });
  ...
  it("Access service tokens (common_name → scopes) → access:svc:<cn> with those actions over every channel; malformed scope lists yield nothing", () => {
    const { grants: t } = translate(undefined, undefined, {
      "reader-bot": ["runs:read"],
      "ops-bot": ["runs:read", "runs:write", "friction:read"],
    });
    expect(t.get("access:svc:reader-bot")).toEqual(grants({ actions: set("runs:read"), channels: "all" }));
    expect(t.get("access:svc:ops-bot")).toEqual(
      grants({ actions: set("runs:read", "runs:write", "friction:read"), channels: "all" }),
    );
  ...
  it("an actor named by several keys gets the UNION of its grants; `all` absorbs a list", () => {
    const { grants: t } = translate({
      repoManagement: ["slack:U1"],
      channelConfig: ["slack:U1"],
      agents: { coding: ["slack:U1"] },
      repos: { "acme/api": ["slack:U1"] },
    });
    expect(t.get("slack:U1")).toEqual(
      grants({
        actions: set("repo:write", "friction:write", "config:write", "agent:run:coding"),
        repos: set("acme/api"),
      }),
    );
  ...
  it("fromPermissions names the ids the `permissions.*` keys (serviceTokens included) grant — never an ingress token's, whose entry is its credential", () => {
    const t = translate(
      { admins: ["slack:UADMIN"], repoManagement: ["http:cron"] },
      { tok: { subject: "ci", scopes: ["dispatch"] } },
      { "reader-bot": ["runs:read"] },
    );
...
describe("the legacy baselines — what an id inherits by its namespace (KTD5/KTD6/KTD10)", () => {
  const table = {
    everyone: grants({ actions: set(...OPEN) }),
  ...
  it("slack: → everyone; access:<sub> → every group's read; access:svc:, http:, mcp:, schedule: → nothing", () => {
    ...
    for (const id of ["access:svc:ci", "http:ci", "mcp:ci", "schedule:x", "cli:local"])
      expect(legacyBaseline(id, table), id).toBe(NO_GRANTS);
  });
  ...
  it("a legacy-listed Access identity has its reads unioned in; an unlisted one holds the reads alone; a service token never inherits them", () => {
    const source = {
      permissions: { repoManagement: ["access:bob"], serviceTokens: { bot: ["runs:write"] } },
      commandGroups: ["runs", "repo"],
    };
    expect(grantsFor("access:bob", source)).toEqual(
      grants({ actions: set("repo:write", "friction:write", "runs:read", "repo:read") }),
    );
...
describe("the native/legacy DIFFERENTIAL — one deployment written both ways resolves to identical Grants for every actor", () => {
  ...
  it("grantsFor agrees actor by actor", () => {
    ...
    for (const id of parsed.grants.keys())
      expect(grantsFor(id, { grants: parsed.grants }), id).toEqual(grantsFor(id, legacy));
  });
  ...
  it("an actor neither shape names: an unlisted slack: user gets what everyone holds (the open commands, the unrestricted agents); a native-only deployment, being explicit, adds nothing — not even config:write", () => {
    expect(grantsFor("slack:UNOBODY", legacy)).toEqual(
    ...
    expect(
      grantsFor("slack:UNOBODY", {
        ...legacy,
        permissions: { ...legacy.permissions, agents: { general: [], coding: [], review: [] } },
      }),
    ).toEqual(grants({ actions: set(...CHAT_OPEN_ACTIONS) }));
  });
  ...
  it("the everyone baseline is for slack: users; a browser session inherits the reads; an unlisted schedule, service token, or ingress subject is NO_GRANTS (fail-closed)", () => {
    for (const id of ["schedule:x", "access:svc:x", "http:stranger", "mcp:stranger"])
      expect(grantsFor(id, legacy), id).toBe(NO_GRANTS);
    ...
    expect(grantsFor("slack:UMGR", legacy).actions).toEqual(
      set(...CHAT_OPEN_ACTIONS, "repo:write", "friction:write", "agent:run:general"),
    );
  });
});
...
describe("grantsTable / grantsIn / grantsFor — the merged lookup", () => {
  ...
  it("the overlap reported is what config can delete: a serviceTokens entry beside a native one is; an ingress token's id beside its native entry (#453) is not, though native still wins", () => {
    const parsed = parseGrantsConfig({
      "http:ci": { actions: ["dispatch", "runs:read"], channels: "all" },
    ...
    const table = grantsTable({
      ...
      permissions: { serviceTokens: { ops: ["runs:read", "runs:write"] } },
    });
  ...
  it("a schedule actor's grants are the registry's declared ones (R9) unless the native block names the id — then config wins, without an overlap warning; no legacy key names schedules", () => {
    ...
    expect(
      grantsFor("schedule:self-improvement", {
        schedules,
        permissions: { admins: ["slack:U1"], repoManagement: ["http:cron"] },
      }),
    ).toEqual(declared);
    ...
    expect(
      grantsFor("schedule:self-improvement", {
        schedules,
        permissions: { repoManagement: ["schedule:self-improvement"] },
      }),
    ).toEqual(grants({ actions: set("friction:read", "friction:write", "repo:write"), channels: "all" }));
    const parsed = parseGrantsConfig({
      "schedule:self-improvement": { actions: ["friction:write"], channels: ["slack:C1"] },
    });
    if (!parsed.ok) throw new Error(parsed.errors.join("; "));
