...
| `src/channels/frictionTrigger.ts` | `POST /friction/propose` — the scheduled trigger of the self-improvement pass (#84), behind the dedicated `FRICTION_TRIGGER_TOKEN` bearer | Fail-closed (503 without the token); the Worker shim's weekly cron (`deploy/cloudflare/worker.ts`, Mondays 14:00 UTC) is the caller; answers the report JSON |
| `src/channels/liveView.ts` | Live-view surface: token-gated `GET /runs/:id` (HTML) + `/runs/:id/events` (SSE) + `POST /runs/:id/stop?mode=soft\|hard` (run control, #101) | Capability-URL auth (not bearer) + Access at the edge; consumes the run-visibility stream; see `features/live-view.md` |
...
| `src/runner.ts` | Provider-blind agent loop (complete → run tools → append → repeat) | Turn budgets on the agent def; honors `RunOptions.control` (soft = finale, hard = abort in-flight via `AbortSignal`, #101) |
...
| `deploy/cloudflare/` | Worker+Container shim, mirrors `coreplanelabs/infrastructure` `terrateam/` pattern | Recommended deploy target; two crons — per-minute keep-alive and the Monday 14:00 UTC self-improvement pass (`POST /friction/propose`) |
...
- Container sizing (2026-08-28): thread-sandbox Worker `standard-3` (8 GiB — lives only for one run, so cents), resident Worker `standard-1` (4 GiB — effectively always-on, billed 24/7 per repo; 4 GiB fits 1-2 simultaneous test runs, step to standard-2 only on real OOMs). `basic` (1 GiB) died running switchboard's vitest suite during a cold review. The resident pool is 16 users and runs release theirs via `/detach` on finish — `features/resident-repos.md` item 16a; the hourly sweep releases clean idle trees, a deploy that grows the pool self-applies via `reconcileImage`, and unused residents park their refresh so the container sleeps (item 16b). Residency GC (#50, items 45–46): every refresh cycle reclaims worktrees whose ref is finished (branch pruned from the mirror, or PR merged/closed — a poll, since the GitHub App has webhooks off), and an over-cap `repo onboard … --evict-coldest` offboards the coldest eligible warm resident instead of 429 (admin opt-in per request, default off).
