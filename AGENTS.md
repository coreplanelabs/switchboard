...
| `src/core/memory/` | Cross-session memory (#85): `MemoryStore` seam, read path (types, keyword+recency scorer, scope deriver, `Null`/`InMemory` stores) and write path (`reflection.ts`: post-reply async distillation on `memory.model`, validated + redacted, dedup/supersede on write). Flag-gated OFF by default | `NullMemoryStore` when disabled → model input byte-identical to memory-off, nothing written; reflections are fire-and-forget, awaited only by the `index.ts` drain; see `features/memory.md` |
