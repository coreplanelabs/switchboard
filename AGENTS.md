...
| `src/channels/residentsView.ts` | Residents dash: Access-gated `GET /residents` (index) + `/residents/<owner>/<name>` (detail) — the browser twin of `repo list` | Reads the resident admin `/residents` route live per request; never caches, never renders the bearer; see `features/resident-repos.md` item 33 |
