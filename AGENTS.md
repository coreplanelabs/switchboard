...
2. **A failing test, then the code.** Unit tests are the default proof. The loop is `npx vitest run --changed origin/<base>` (the PR's base; `origin/main` for most branches) or the touched test files — and for a pipeline agent on the shared resident, that is also the criterion: the full suite is CI's gate and CI is the only place it runs. Local human session: `npm test` before pushing.
