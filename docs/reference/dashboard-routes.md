...
| `GET /costs/<group>` | Spend for one group | `?view=users` opens the By user tab; `?stream=1` is the page's status feed (SSE: the snapshot's stamp, a take in flight, the next due time — the page repaints its figures when a new snapshot lands) |
