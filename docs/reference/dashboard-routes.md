...
The header lists only the surfaces this installation has: **Residents** appears when resident environments are configured (`execution.resident`), **Costs** when `costs` is configured with its analytics token, **Delivery** when a GitHub credential is set, the **Scheduled** tab when `schedules.worker` records firings. **Settings** is always there (every installation has channel scopes); its MCPs tab only with an `mcp` block. The docs link is always there: it opens the project's published site. The routes themselves still answer without their subsystem — with a `503` naming the config that turns them on.
