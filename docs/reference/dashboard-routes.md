...
| `GET /runs?mine=1` | Only the runs you requested (combines with `all=1`) | "You" is the Slack user your sign-in email names; a sign-in with no linked Slack user has no runs of its own, and the toolbar's **Show mine** is disabled for it |
...
| `GET /runs/unit/<pipeline>:<unit>` | One ship unit's story: its one thread's runs in round order, each opening to its timeline, with a search over the whole thread | What `runs unit` answers, as a page; a unit you may not see is the same 404 an unknown run gives. The pipeline's own run page lists its units, and a conductor's run page lists the runs it spawned |
...
| `GET /costs/<group>` | Spend for one group | `?view=users\|threads\|channels\|agents\|models` opens that dimension's tab — the same dollars laid against who started the runs, the thread or channel they ran in, the agent, or the model; `?stream=1` is the page's status feed (SSE: the snapshot's stamp, a take in flight, the next due time — the page repaints its figures when a new snapshot lands) |
...
| `GET /costs/<group>/<view>.json` | One dimension's report, machine-readable | `<view>` is `users`, `threads`, `channels`, `agents` or `models`: the rows the tab shows (`key`, `runs`, `turns`, `llmUsd`, `cloudUsd`, `totalUsd`, `unpricedTokens`, `byModel`), the coverage and the reconciliation; the same as `costs by <dimension>` on the command surfaces; 503 with `Retry-After` before the first snapshot |
...
| `GET /plane` | What is happening: every live and recently ended run, every tracked pull request and every ship unit, each with its requester and its health | The page twin of `/api/plane.show` below — the same table the `plane show` command answers |
| `GET /settings` | The settings page: MCPs where the capability is on, else Channels | An adapter over the command routes below ([settings-page.md](specs/settings-page.md)): what it lists is `mcp list` / `config overrides` / `config show --channel` invoked as you; every button is one `POST /api/…`; the org and channel scopes only — personal settings are set in chat |
| `GET /settings/mcps` | Every MCP server your runs can reach, by scope, with an add form; `?channel=<id>` lists that channel's scope beside the org's | Add and Connect hand back the one-time link below; a credential never passes through the page |
...
| `/api/runs.get` | `GET`, `POST` | `runs:read` | One run's record, its cost in dollars per model (or unpriced) included; `--include messages` adds its events with free text wrapped as untrusted content. |
...
| `/api/runs.stop` | `POST` | `runs:write` | Request a live run to stop (`--mode soft` = finish the current step; `hard` = abort now). A pipeline refuses soft — `--mode hard` seals it failed and releases its thread. Records the caller as the requester. |
| `/api/runs.unit` | `GET`, `POST` | `runs:read` | A ship unit's runs in round order — its thread's, live and finished, each with its round — from one read. |
...
| `/api/runs.findings` | `GET`, `POST` | `runs:read` | A pull request's findings ledger — every review finding by id with its severity, where it was raised, what the coding run recorded against it and whether the next review agreed — read from the run records alone. |
...
| `/api/mcp.promote` | `POST` | `mcp:write` | Promote a person's MCP server into the org tier (admins): the same name, URL and auth, added by you; a bearer/oauth server gets a fresh org connect link for you to complete — the person's credential is never copied. |
...
| `/api/metrics.trend` | `GET`, `POST` | `metrics:read` | The run trend from the metrics dataset: runs, failure rate, p50/p95 wall and dollars per day and per agent over the range, weighted for sampling — the /metrics page's report as text or JSON; nothing written. |
