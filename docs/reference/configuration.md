...
| `harness` | Which loop drives each preset's runs: `<preset>: native` (the in-process loop) or `pi` (the pi coding agent in the run's own container, behind the model proxy) | every preset on the harness it declares — `native` for all of them today |
