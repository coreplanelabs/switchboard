...
| `routing` | The request router: `auto: true` lets a plain message — no directive, no sticky preset, no channel or user `agent` — pick its preset through the fast model (`model`, default `defaults.models.general`); `ship` is never routed — it holds the merge grant | off — every plain message runs `defaults.agent` |
