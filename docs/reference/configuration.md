...
| `delivery` | `/delivery` page and `delivery report`: the repositories the page serves and the identities the indicators judge by (the review agent's login, agent logins, agent co-author names); needs a GitHub credential | `/delivery` answers 503 naming the block; `delivery report --repo owner/name` still works |
