...
| `grants` | What each actor holds — actions (commands, `agent:run:<name>`, `dispatch`), channels whose runs it may read, repos it may use — keyed by platform-namespaced id | nobody is an admin; Slack users hold the open chat commands and every unrestricted agent, browser sessions every read, credentials nothing — see [reference: authorization](authorization.md) |
