...
| `routing` | The request router: a plain message — no directive, no sticky preset, no channel or user `agent` — picks its preset through the fast model (`model`, default `defaults.models.general`) and the card says why; `auto: false` turns it off; `ship` is never routed — it holds the merge grant | on — a plain message routes; add `routing: { auto: false }` to keep every plain message on `defaults.agent` |
