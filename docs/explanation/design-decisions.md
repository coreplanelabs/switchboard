...
| 0033 | [Run artifacts move by reference through one private R2 bucket — containers and the bot's Worker move the bytes over URLs the bot signs, in both directions, and the run record keeps the key](../decisions/0033-artifacts-move-by-reference-through-r2.md) | Presigned-URL upload (a browser writing to S3 while the app server only signs), applied to both directions of a run, with the run record as catalogue and access list | proposed | 2026-09-14 |
