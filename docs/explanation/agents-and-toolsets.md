...
The budget is the wall clock. Each agent also carries a turn cap, but it is derived, not chosen: six turns a minute over the wall clock (270 for `coding`, 150 for `review`), a pace only a run stuck re-issuing the same call sustains. A run that reaches it is told so — `Stopped after 270 model turns in 38 minutes — that pace looks like a loop` — and writes up what it has; a run doing real work is ended by the clock, never by the count ([the pi harness](../reference/specs/harness-pi.md) item 15).
