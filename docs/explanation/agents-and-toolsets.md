...
`agent:<name>` in a message forces one of them. With the router on (`routing.auto`, [Turn features on and off](../how-to/turn-features-on-and-off.md)), a plain message picks its own preset through the fast model and the card says why; `ship` is never routed, and the `conductor` is reached only as a compound request with several independent parts ([routing-and-config item 21](../reference/specs/routing-and-config.md)).
