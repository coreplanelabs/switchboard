...
| `ship` | Coding, review, fixes, until LGTM; on a plan branch the runner merges, elsewhere a person does. | Never sent to a model; the plan runner runs each round as a child `coding` or `review` run in a thread of its own. | `repo-resident` | `write` | 240 min (`ship.maxMinutes` in config); each round clipped to what remains |
