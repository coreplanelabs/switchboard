...
| `ship` | Coding, review, fixes, until LGTM. A person merges. | Never sent to a model; each round runs `coding` or `review`. | `repo-resident` | `write` | 120 min (`ship.maxMinutes` in config); each round clipped to what remains |
