...
| Provider | `Provider` (`src/core/provider.ts`) and the config's `providers` blocks | A run's pi calls the model proxy, which forwards the two wire shapes — Anthropic Messages, OpenAI Chat Completions (OpenAI, Groq, Ollama, vLLM) — with the deployment's key; the same blocks on pi's model library (`PiAiProviders`) for the router's and reflection's calls | Slack, authorization, where tools run |
...
A `ship` request is the one that runs other runs. The pipeline (a Workflow in the bot's shim Worker, with no model turn of its own) opens a thread per unit, then dispatches ordinary runs into it as the requester: a coding run with the unit's contract, a review run at the pull request's head in the same thread on a log of its own, and, after a verdict that requests changes, the review's findings as a message into the unit thread with `agent:coding` on it, so the agent that wrote the code answers its own review. The pipeline matches the dispositions that run records to the review's findings, dispatches the re-review into the same thread, and on a plan branch merges the approved head ([agent-ship spec](../reference/specs/agent-ship.md); [decision 0034](../decisions/0034-one-agent-per-unit-a-run-continues-a-transcript.md)).
