...
Two gaps remain open and are tracked rather than papered over: a re-attach reads pi's log from the transport's last read position, which can sit past a tool result pi wrote before the death, so that result is missing from the ledger's copy; and a resume re-provisions the workspace as a fresh run would, which on the resident can replace the tree pi was working in. Both are in [Known limits](known-limits.md).
