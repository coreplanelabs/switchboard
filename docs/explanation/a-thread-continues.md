...
On the resident, where a thread's worktree is bound to one branch, the thread follows its own work: the resident remembers the branches the thread's runs pushed, and a thread bound to the repository's default branch only because its first message named none moves onto the branch its own pull request lives on. The move is a decision about the binding alone — the run then starts from a clean tree at that branch, because a run always starts from a clean tree at its bound ref: what a run wants kept, it commits and pushes, and what it leaves uncommitted or unpushed is discarded at its end and said on its record and its card. Once that pull request has merged and its branch is gone, the thread returns to the default branch and may follow its next pull request the same way — a thread whose own branch is gone returns to the default whether a rebind put it there or a ship unit's coding child bound it there by name and pushed it; the card names each move ([resident-repos spec](../reference/specs/resident-repos.md) items 16 and 17).
