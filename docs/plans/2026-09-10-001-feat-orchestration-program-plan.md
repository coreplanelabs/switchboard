...
- R26. The program is the **Switchboard: v1.2** board (project 4): one parent issue per plan and one sub-issue per unit, with Status `Todo | In Progress | Done`; the board also carries the sibling V2 items this plan does not own (record 0028's image manifest; 0026 slice two, the router) so the version line has one view.
...
**In scope**: the three tracks above and the program hygiene; record 0026 slice one and spawn/await Phase 1 and 2 as the ship track's prerequisites (U20 to U24); record 0032's replacement series (U25 to U29); spec rows for every changed behaviour; the live receipts each unit names.
...
- Record 0026 slice two (the router behind `defaults.models.router`, the sticky card label) and record 0028 (the image manifest): v1.2 board items with their own plans.
...
Eight phases, in dependency order; the harness track runs in parallel with the resident track once the `v1.2` line exists, the ship prerequisites (F) run in parallel with both, and the harness series (G) follows the adoption record one gated step at a time.
...
| Generated docs match sources | `npm run docs:check` | U1, U10, U21 (`config set` gained options), U22, U23 (the specs index) | `ok — 5 file(s) match…` |
...
| Live receipts | `[agent]` rows in the touched specs, receipts on the tracker | U2, U7, U9, U13, U14, U21, U22, U23, U24, U25 to U29 | posted, linked from the spec row |
...
| Splitting a scope's `agent` into a force and a default (a deployment that set one never routes in that scope) | record 0026, Routing | Out of scope; slice two's concern. |
