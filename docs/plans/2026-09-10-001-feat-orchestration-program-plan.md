...
- **Baseline**: release v1.13.0 (`902ecd50`, cut 2026-09-10; receipts on [#809](https://github.com/coreplanelabs/switchboard/issues/809)) is the frozen 1.x baseline every V2 unit re-baselines against; record 0026 is accepted against it ([#815](https://github.com/coreplanelabs/switchboard/pull/815)). The `v2` branch is cut from it.
- **Gates it waits on, not units it owns**: the public flip's history rewrite ([#750](https://github.com/coreplanelabs/switchboard/issues/750)), which the `v2` branch is cut after or rebased across; record 0026 slice one (a 1.x item that lands on `main` and reaches `v2` at the next sync); spawn/await ([#108](https://github.com/coreplanelabs/switchboard/issues/108) Phase 1 and 2, a V2 item); the distribution series ([#792](https://github.com/coreplanelabs/switchboard/issues/792), merged through PR C, with its live receipts still owed).
...
- **Execution profile**: every PR targets `v2` and squash-merges there; `v2` takes `main` regularly and lands on `main` once as 2.0.0 at the release moment (KTD12); each phase is its own PR series through the pr-lifecycle review loop; every unit is an issue on the v1.2 board with the plan as its parent (KTD13); the two seam tests (U2) are the first receipts and gate every resident unit; specs change in the same PR as the code they describe (`resident-repos.md`, `execution.md`, `agent-ship.md`, `agent-coding.md`, `costs.md`, `tracing.md`, `run-history.md`, `release-and-deploy.md`).
- **Tail ownership**: the `v2` branch's sync cadence and the 2.0.0 release moment are Justin's calls; staging and production experiments that deploy the resident Worker mid-cycle are Justin's call on timing; the pi adoption record (U10) is Justin's decision; everything else ships autonomously.
...
- R24. V2 is the 1.2 line: a long-lived `v2` branch cut from the v1.13.0 baseline; every V2 PR targets `v2`; release-please on `v2` cuts 1.200.0 first (a `Release-As: 1.200.0` footer on the branch's first release commit) and minor releases from there (1.201.0, 1.202.0, …), called "1.2" in public; `main` stays the 1.1x line, keeps releasing patches and the work in flight, and never reaches minor 200; `v2` takes `main` at a regular cadence; V2 lands on `main` once, at Justin's release moment, as 2.0.0 (`feat!:` plus `Release-As: 2.0.0`) with the `## 2.0.0` migration section naming every change an installation must act on.
- R25. `v2` has its own CI and release: pull requests to `v2` run the same checks as `main`; pushes to `v2` run release-please for that branch; when npm publishing is on, 1.2xx releases publish under the `next` dist-tag so `@latest` stays the 1.1x line; a `v2-staging` installation profile deploys the 1.2xx releases so every live receipt in this plan runs against the 1.2 line, never production 1.1x.
...
- Record 0026 slice one and two (machine classes, boundaries, `explore`, the router) and record 0028 (the image manifest): v1.2 board items with their own plans, landing on `main` (slice one, for the A7 comparison) or `v2` (their owners' call).
...
- KTD12. **`v2` squash-merges its PRs, releases as 1.200 and up, takes `main` by merge at a regular cadence, and lands on `main` once as 2.0.0 at the release moment.** (session-settled: user-directed — Justin asked for a 2.x branch kept on top of 1.x, versioned 1.2xx so minors are free for it and "1.2" is the public name, with 2.0.0 saved for a big release moment; chosen over developing V2 on `main` behind flags, which would put V2's churn in every 1.1x patch, and over 2.0.0 prereleases, which spend the major early.) Versioning: the first release commit on `v2` carries `Release-As: 1.200.0`; release-please's `node` strategy then bumps the minor per `feat` (1.201.0, …) and the patch per `fix`; `main` stays below minor 200 by construction (it is at 1.13 and gains a minor per release). Recommendation inside that direction, Justin's call: sync `v2` with `main` by merge, not rebase, once more than one PR has merged into `v2`, because a rebase rewrites shared history and invalidates every open PR against the branch; a rebase is fine only while `v2` is linear and one stream; the `.release-please-manifest.json` conflict at every sync resolves to `v2`'s version. Landing: a merge of `v2` into `main` whose merge commit is `feat!: Switchboard 2.0.0` with `Release-As: 2.0.0`, so release-please cuts 2.0.0 with the `## 2.0.0` migration section (CONTRIBUTING's breaking-change rule) regardless of the two manifests. Releases from `v2`: a second release-please job with `target-branch: v2` and the same config; the publish job tags npm `next` for a 1.2xx version when publishing is on; the reusable deploy workflow deploys the release to `v2-staging`. CI: `ci.yml` runs on every pull request already; `push.branches` gains `v2` for the release-please, codeql and scorecard jobs.
...
- The `v2` branch can be cut before the public flip's history rewrite and rebased across it (a rewrite that changes only history, not tree contents, leaves the rebase mechanical); if the rewrite reshapes the tree, `v2` is cut after it and U0 waits.
...
| Sync `v2` from `main` by merge (recommended once several PRs have landed) or by rebase (Justin's stated preference)? | Justin | KTD12's trade-off; decided at the first sync | U2 |
...
Five phases, in dependency order; the harness track runs in parallel with the resident track once the `v2` line exists.

| Phase | Units | Waits on | Receipt that closes it |
|---|---|---|---|
| 0. The 1.2 line | U0 | the v1.13.0 baseline (cut) | `v2` exists with CI green, release 1.200.0 cut from it and the `v2-staging` installation live on it |
...
- **Files**: the `v2` branch (cut from `main` at or after `902ecd50`); `.github/workflows/ci.yml`, `codeql.yml`, `scorecard.yml` (`push.branches: [main, v2]`); `.github/workflows/release-please.yml` (a second job or matrix leg with `target-branch: v2` and the same config; the publish job's npm dist-tag `next` for a 1.2xx version; the deploy call targeting `v2-staging`'s profile variable); `docs/reference/migrations.md` (the `## 2.0.0` section, opened now and filled as V2 accumulates changes an installation must act on, per CONTRIBUTING); `src/ciWorkflow.test.ts` (the branch-list assertion); `scripts/check-pr-title.mjs` or a sibling check (refuses `!` on a PR to `v2`; the major is the landing's); the `v2-staging` installation profile and config in the infrastructure repository (a second account or zone, `images: registry`, its own bearers); `docs/how-to/ship-a-release.md` (one paragraph: the 1.2 line, its versions and how it lands as 2.0.0).
- **Approach**:
  1. Cut `v2`; protect it like `main` (required checks `bot`, `web`, `docs`, `workers`, `image`, `package`, `title`); PRs squash-merge into it.
  2. Add `v2` to the push triggers; add the `v2` release-please leg; the first release commit on `v2` carries `Release-As: 1.200.0`, so the first release is 1.200.0 and later ones bump the minor.
  3. Create the `v2-staging` installation from the profile shape in `deploy/profile.example.json` and deploy 1.200.0 through the reusable workflow; its `/healthz` build commit is the receipt.
  4. Sync policy (KTD12): a `chore(main): sync main into v2` PR at the cadence Justin sets, the manifest resolving to `v2`'s version; the landing PR is `feat!: Switchboard 2.0.0` with `Release-As: 2.0.0` and the migration section complete.
...
- **Test scenarios**:
  - `src/ciWorkflow.test.ts`: every workflow that triggers on push to `main` also triggers on `v2`; the release-please `v2` leg targets `v2` with the same config; the publish job tags a 1.2xx version `next`.
  - The title check refuses a `!` title on a PR whose base is `v2`, and accepts the landing title with `!` on `main` only when `docs/reference/migrations.md` has the `## 2.0.0` section.
  - Human-gated: a PR to `v2` runs the full CI matrix; release 1.200.0 appears from `v2`; `v2-staging`'s `/healthz` reports its commit; a 1.1x patch releases from `main` afterwards with its own minor untouched.
- **Verification**: the three human-gated checks receipted on the parent issue; a 1.x patch released from `main` after `v2` exists without touching it.
...
- **Dependencies**: U1; the `v2-staging` installation (U0).
...
- **Approach**:
  1. Onboard one resident on `v2-staging`. Define a one-step Workflow whose step calls the resident Worker binding to run today's install command on it (10-minute budget). Time it. Assert no platform timeout below the step's own.
...
- **Requirements**: R18
- **Dependencies**: U8; a resident on `v2-staging`.
...
| Workflow branch lists | `npm test -- src/ciWorkflow.test.ts` | U0 | green with `v2` in every push trigger |
| Live receipts | `[agent]` rows in the touched specs, receipts on the tracker | U2, U7, U9, U13, U14 | posted, linked from the spec row |

## Definition of Done

- The 1.2 line exists: CI green on `v2`, 1.2xx releases cut from it, `v2-staging` deployed from them, 1.1x still releasing from `main`.
...
- Every V2 PR merged into `v2` and released as 1.2xx; V2 reached `main` once, at the release moment, as 2.0.0 with its migration notes.
