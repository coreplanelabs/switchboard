...
**In scope**: the three tracks above and the program hygiene; record 0026 slice one and spawn/await Phase 1 and 2 as the ship track's prerequisites (U20 to U24); spec rows for every changed behaviour; the live receipts each unit names.
...
Seven phases, in dependency order; the harness track runs in parallel with the resident track once the `v1.2` line exists, and the ship prerequisites (F) run in parallel with both.
...
| C. Harness | U8 to U11 | U1 (U11 also U20 to U22) | the adoption record, Justin's decision, with the spike's table |
...
| Bot project tests | `npm test` (root; `deploy:gen` runs first) | U3, U4, U11, U12, U13, U14, U15, U20 to U24 | all shards green |
...
| Live receipts | `[agent]` rows in the touched specs, receipts on the tracker | U2, U7, U9, U13, U14, U21, U22, U23, U24 | posted, linked from the spec row |
...
| The `harness` property on presets | harness exploration Phase 3 | Absorbed into U11, gated on U10 and U22. |
