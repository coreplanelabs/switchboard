...
A single provider boundary maps status and structured body to `ProviderFailure.cause`. The cause union is closed: `transient`, `rate-limited`, `credit-or-quota-exhausted`, `key-absent`, `key-invalid`, `model-unknown`, `permanent`. Consumers act on that field. `transient`, `rate-limited` and `credit-or-quota-exhausted` are provider-down; the proxy parks the current run and the harness holds its turn. Every person-facing surface calls one renderer that returns one sentence and receives no payload or URL.
...
- R1. `ProviderFailure.cause` and `PROVIDER_FAILURE_CAUSES` contain exactly the seven accepted causes. `classifyProviderFailure` is total over status, structured body and adapter transport errors. Unknown input is `permanent`.
- R2. `src/core/providerConformance.test.ts` contains the failure matrix with at least one provider answer per cause, including the 21:24Z OpenRouter 402 body with `metadata.limit_source: openrouter_key_limit`.
...
- R6. Intake stores the typed cause on its receipt and returns the one sentence. The live Slack door posts it once before returning without dispatch. It never includes the provider body or URL.
...
| Criterion | Proof |
| --- | --- |
| The seven causes are closed and every fixture answer maps once | `src/core/providerConformance.test.ts::ProviderFailure — the closed failure matrix::*` |
...
| Intake carries and renders one typed sentence instead of failing silent | `src/core/intake.test.ts::decideIntake — the verdict from one forced tool call (routing-and-config item 27)::a provider failure carries its typed cause and one safe sentence…`; `src/channels/slack.test.ts::receiveSlackMessage — the intake gate (docs/reference/specs/slack-channel.md item 15)::an intake provider failure is not silent…` |
...
- Every model-call failure reaches a consumer as one of seven causes.
