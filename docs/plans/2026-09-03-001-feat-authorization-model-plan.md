...
`RunsService` and `RunStoreFrictionLedger` take `predicateFor`; `runs.ts` and `friction.ts` lose their hand-written channel comparisons; `Predicate` gains the `visibility-in` leaf (`member-of` compiles to `or(channels-in(grants.channels), visibility-in(["public"]))`, filtering on the run's stamped `channelVisibility`) — the contract addition lands in this unit since `types.ts` is already on main; the Access-routed `/api/runs.*` path is covered because it flows through the same handlers.
