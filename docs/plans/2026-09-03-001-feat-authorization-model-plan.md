...
- **Stop conditions**: a requirement that needs an external policy engine (Cedar, OPA) to express; a Slack API limit that makes channel membership unknowable within a request budget; any unit that cannot keep its behavior-preserving tests green except at the four deliberate changes named in R12.
...
- R12. Every unit is behavior-preserving under the existing test suite except four deliberate changes, each with a red-first test: (a) the cron actor analyzes the fleet; (b) Access callers and Slack users are bound by channel membership on run reads; (c) reflection cannot write private-channel facts to `org`; (d) an ingress token without a `channel` key holds no channel grant and reads no run (public runs and its own aside) — today's per-request pin to the channel it speaks in is gone (OQ4, option a; production keeps its fleet readers through native `grants` entries).
...
- KTD1. **Policy is data with a closed condition vocabulary.** A rule is `{ actorKind?, grant?, action, resource, when: Condition[] }` where `Condition ∈ { has-grant(g), member-of(resource.channelId), is-self(resource.userId), owner-of(resource.repo), all-channels }`. Because conditions are closed, every rule can be evaluated against one resource AND compiled to a store predicate (`channelId IN (...)`, `userId = ...`, no constraint). Arbitrary predicates are not allowed in rules; a need for one is a stop condition to revisit the vocabulary. `Grants` has exactly three axes — `actions`, `channels`, `repos` — each a set or `"all"`; which agents an actor may run is not an axis but the action `agent:run:<name>` (rule: `has-grant(agent:run:<name>)` on resource `agent { name }`).
...
- KTD6. **Backward-compatible config translation.** At load, `permissions.admins` → grants `{ actions: all, channels: all, repos: all }`; `permissions.operators` → `{ actions: every <group>:read + every <group>:write, never :exec; channels: all }` — today's fleet-wide operator, preserved (see OQ1); an Access browser session holds every `<group>:read` implicitly; `permissions.repoManagement` → `{ repo:write, friction:write }`; `permissions.channelConfig` → `{ config:write }` (key absent → every user holds it); `permissions.agents.<name>` → the ACTION `agent:run:<name>` in `actions` (an agent with no allowlist → everyone; admins hold `agent:run:*`); `permissions.repos.<owner/name>` → the listed ids' `repos` (map or key absent → `all`, open-when-absent preserved); `permissions.serviceTokens.<cn>` → `access:svc:<cn>` `actions` of the listed scopes; token `scopes` → `actions` of the same names; token `channel` → `channels: { that channel }` (namespace pin unchanged). The translation table is a unit-tested pure function and is documented in `features/authorization.md`; the old keys are deleted in U7 after one release with both accepted.
...
`RunsService` and `RunStoreFrictionLedger` take `predicateFor`; `runs.ts` and `friction.ts` lose their hand-written channel comparisons and `Caller.channel` is deleted; `Predicate` gains the `visibility-in` leaf (`member-of` compiles to `or(channels-in(grants.channels), visibility-in(["public"]))`, filtering on the run's stamped `channelVisibility`) — the contract addition lands in this unit since `types.ts` is already on main; the Access-routed `/api/runs.*` path is covered because it flows through the same handlers. The stamp lands here too (KTD7 brought forward): `RunMeta`/`RunRecord`/`RunListItem` carry `channelVisibility`, dispatch asks a `ChannelDirectory` once per run — the static id-based first cut (`http:`/`mcp:` → machine, `slack:D…` → dm, `slack:G…` → private, `slack:C…` → unknown until U5's Slack directory) — and the run-history DO stores the column, migrates old tables to `unknown`, and compiles the predicate's wire form (`RunListOptions.visibleTo`) to indexed SQL. The schedule registry declares each run schedule's grants (`self-improvement`: `friction:read`, `friction:write`, `channels: all`) as the floor `grantsFor` serves for its `schedule:` actor; the shim still fires as `http:cron`, which `config.production.yaml` grants the same reach natively.

- Red-first tests: (a) the `schedule` actor for `self-improvement` sees runs from every channel (fails today); (b) an Access operator without `all-channels` gets `not_found` on a run from a channel they are not a member of (fails today); (c) → R12 (d): an unpinned token holds no channel and reads no run (fails today: it saw the channel it spoke in); (d) a pinned token still sees only its channel, now through its channel grant.
...
- Known limit of this cut: the `/runs` HTML index and the tokenless finished-run page (`liveView.ts`) list with an explicit `visibleTo: all` and read without `authorize` — Access gates who reaches them, not which runs they show; binding them needs the page to carry an actor (frontend work on hold) and is a spec `[gap]`.
...
- OQ4 (resolved 2026-09-04, Justin: option a). How does an ingress token WITHOUT a `channel` key translate, given today's per-request pin ("the channel it speaks in") has no grants spelling? (a) no channel grants — channels are channels, no machine-channel vocabulary; (b) a `"machine-channels"` sentinel on the channels axis meaning every `http:*`/`mcp:*` channel. Decision: (a). Consequence: R12 (d); an unpinned token that must read runs is pinned or given a native `grants` entry, and `config.production.yaml` carries the fleet readers (`http:cron`, `schedule:self-improvement`, the ops service token) so production keeps working through the cut.
