...
**Amendment, 2026-09-18, while proposed: the units after the seam cut move.** Record 0057 supersedes this record's deterministic-first ordering. U1 and U2 continue here; U3 lands as-is and its near-match helper becomes evidence in the operator's turn; U4's Yes button stays as record 0044's; U5 to U9 are absorbed by [the operator plan](2026-09-18-002-feat-the-operator-is-the-one-door-plan.md) (the answer row and words as an answer become the operator's question and answer binds, the router's question its question outcome, the remaining guess sites its evidence lines, the repository cards its briefs). Nothing from U5 onward is seeded from this plan.
...
- **Execution profile**: nine units in two cuts, each one pull request through the review loop, in dependency order. Tests first in every unit. U1 to U4 are the **seam cut**: U1 and U2 change no sentence a person reads; U3 is the first visible change and fixes the trace; U4 adds the Yes button. U9 and U5 to U8 are the rest of the bundle: U9 gives every repository a README-derived card so the thread knows its repositories by what they are; U5 measures the router on the answer task before U6 gives it the task; U7 lets the router ask; U8 finishes the guess sites. After the amendment below moved those to the operator plan, one unit still runs here after the cut: U11 makes every refusal a run record, the count's sink. The units are seedable to the plan runner one at a time (`agent:ship in <owner/repo>: plan <this path> units U<n>`), which holds the merge grant on a plan branch; a unit run by a person merges under `merge: person`.
- **Stop conditions**: a unit that cannot pass `npm run verify` within its listed files hands back a deviation. Nothing here adds a Worker, a table or a credential; the one store change is a new row kind in an existing table. A unit stops and asks if it would: run a write from anything but a button showing the exact line (a typed yes, a router answer, a guess); change a sentence a person reads during U1 or U2 other than by moving it into the renderer unchanged; introduce a word the person must type to answer; let a `system` refusal carry a Yes; or run a proposal as anyone but the person whose message it corrects. In code comments, spec prose and commit text a unit names itself by its title or cites record 0054, never by a plan-unit token such as the two-character unit id: the public-hygiene ratchet's plan-id class fails `verify` on a new imprint, which is what turned the second unit's first head red.
...
- R26. The router's user turn carries, after the thread's repository line, the cards of the repositories the thread has touched and then the residents' cards, capped at twenty briefs, the thread's own first, as the operator plan keeps it. When the ask names no repository and its subject matches one card, the router may bind that repository; the bind is a bind like any other under records 0039 and 0044: a read runs with its receipt naming the repository, a write meets the offer, and a doubt is U7's `clarify` with the candidate cards as the question's list.
...
- **Execution note**: this is the largest diff of the plan and changes no sentence; split the pull request by mechanism if review asks, in the order above. The unit's first hand-off named two follow-ups for a later round: per-site span assertions for the hand-off's `plan_*` codes (a named gap row), and the tracing spec's stale `ship_preflight` token now that the preflight's nine results carry codes of their own.
...
### U11. Every refusal is a run record

- **Goal**: every refusal the door makes, including a gate refusal before any command is bound and before admission, is a run record with its code and cause, so `npm run load -- door` counts refusals per day, cause and code from the run store alone; the trace attributes stay as they are. This is the one unit after the cut that runs under this plan: the count the record's first unit promised has no sink in production (issue 1727), and the operator plan's volume line reads the same rows.
- **Requirements**: R4 as amended (a refusal is always a run record), R23 (run-history item 2, load-harness item 19, routing-and-config item 4).
- **Dependencies**: U1, U2.
- **Files**: `src/core/dispatch/commandRun.ts` (`recordRefusal(deps, msg, io, refusal, ending, trace)` beside `recordRoutedDecision`: agent `door`, label the code, status `completed`, `announce: false`, no thread claim, the redacted request as its `input` event and one `refusal` event carrying `code`, `cause` and the redacted sentence capped like a receipt); `src/core/runEvents.ts` (the `refusal` event); `src/core/dispatcher.ts` (both `refuse` helpers and the catch-all call `recordRefusal` after `renderRefusal`, on the same span; a refusal that already recorded through `recordRoutedDecision` records once); `src/load/doorReport.ts` (counts `door` records and `refused` route outcomes together; the footer about recordless refusals goes); the run page shows the record through its generic event row, and any new page vocabulary is a check-in, not part of this unit; tests `commandRun.test.ts`, `dispatcher.test.ts`, `doorReport.test.ts`; the run-history spec's record shape and event list.
- **Approach**:
  1. Tests first: in `dispatcher.test.ts` a gate refusal (the agent allowlist, a repository that is not a resident) leaves one `door` record, status `completed`, whose `refusal` event carries the code and cause, with no thread claim, no `runStarted` signal and the reply unchanged; a refused click still leaves exactly one record; the catch-all's `uncaught` refusal records with cause `system`; `doorReport.test.ts` counts a fixture store of `door` and `command` records by day, cause and code and prints no footer.
  2. Add the event and `recordRefusal`; call it from the two `refuse` helpers and the catch-all; teach the report to read both record kinds.
  3. Spec rows: run-history item 2 (the `door` agent and the `refusal` event), load-harness item 19 (the report counts every refusal), routing-and-config item 4 (a gate refusal is a run record).
- **Execution note**: no sentence a person reads changes. In comments, spec prose and commit text name this unit by its title or cite record 0054, never a plan-unit token; run the memory Worker's own checks only if a file under deploy/ is touched, which this unit should not need.
- **Patterns to follow**: `recordRoutedDecision` and `announce: false` on `runInlineCommandRun`; the `route` event's `outcome: "refused"` and `refusalCode`; the door report's refusal rows.
- **Test scenarios**: as in the approach, plus a refusal on a message with no thread (a DM) records with the channel as its thread key, and a record whose code the table does not know counts as `unknown`.
- **Verification**: the test files green, red first; `npm run specs:check`; `npm run verify`; `npm run load -- door --since <date>` prints refusals per day, cause and code including gate refusals, posted on the receipts tracker after one week.
...
| The count | `npm run load -- door --since <date>` prints refusals per day, per cause and per code from the run store, gate refusals included once U11 lands; the root-span query is not available in production (issue 1727) | U1, U11 |
...
- U9 and U5 to U8 are the operator plan's, per the amendment in the goal capsule; U11 merged here and the count posted after one week.
