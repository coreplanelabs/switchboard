...
**Amended 2026-09-16 — D4 reopened; the seeded tier is taken on.** The deferral recorded in [#712](https://github.com/coreplanelabs/switchboard/issues/712) named four signals that would reopen the decision; one day produced three of them. (1) Seat and disk refusals: the largest repository's resident refused attaches on `disk-pressure` from 05:00Z to past 21:00Z — partly a stale staging reserve ([#1405](https://github.com/coreplanelabs/switchboard/pull/1405)), but underneath it the ceiling this plan already names: one 20 GB container per repository whose deps alone are 6.9 GiB holds ten to fifteen trees at best, and no trimming makes it hold a hundred (the ordered relief is [#1402](https://github.com/coreplanelabs/switchboard/issues/1402); this is its step 3). (2) Cold attaches: every refused run fell to a fresh sandbox, which on the 0.13 line died at its first command until [#1393](https://github.com/coreplanelabs/switchboard/pull/1393) and, since, clones and installs from zero — minutes before the first command on that repository. (3) `fleet-busy` waits: the cold fleet sat at `max_instances` the whole day, through an idle leak ([#1354](https://github.com/coreplanelabs/switchboard/issues/1354), fixed in [#1355](https://github.com/coreplanelabs/switchboard/pull/1355)) rather than load, but the refusals it produced are the shape point 5 describes. Two preconditions the deferral named are met since: the sandbox Worker runs the resident's SDK line and image (1.230.0, [#1251](https://github.com/coreplanelabs/switchboard/pull/1251)), so the restore API D4 needs exists on the cold path; and a cold sandbox's life is now bounded on its own terms (the idle deadline of [#1355](https://github.com/coreplanelabs/switchboard/pull/1355), the start gate of [#1369](https://github.com/coreplanelabs/switchboard/pull/1369)), so seeding more of them does not seed a leak. Order of work: Phase 0's `load:seeded` at N = 1 on the current cold path first — the one harness slice the gate reads; the rest of Phase 0 is not a precondition for the seed — then Phase 3.3 (the seed), then the gate at N = 24 on the largest repository. D4's shape stands as recommended (seeded sandboxes; replicas remain the named alternative behind the gate); D6 is unchanged.
