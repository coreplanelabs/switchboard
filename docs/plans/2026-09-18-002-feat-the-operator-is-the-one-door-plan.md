...
- **Test scenarios**:
  ...
  - The volume line prints refusals per day from the door run records; a run record of another agent counts none.
...
## Amended 2026-09-20 — record 0069 absorbs U12, narrows U13 and narrows U16

[Record 0069](../decisions/0069-the-one-door-has-one-execution-path-a-bind-runs-clicks-or-routes-a-violation-is-re-asked-a-disagreement-floors-and-no-chat-surface-hands-back-a-line-to-retype.md) and [its plan](2026-09-20-001-feat-one-execution-path-plan.md) take three of this plan's remaining units. U12 (the directive words leave) is absorbed: the dispatcher's directive short-circuit and stage A's directive read are decide sites the one execution table collapses, and the deletion lands as that plan's loop unit, gated by the directive replay row plus the day's two directive fixtures. U13 (the typed command line leaves stage A) is narrowed: a line the person's own grammar parses is permanently a deterministic row of the table — run as typed, never re-spelled — and what leaves is stage A as a separate render path, the parse moving behind the table's owner. U16 (the confirm default moves) is narrowed to one cell: whether the write row's chat outcome is `run` or `click`, moved under the same gates U16 already names, since under the table there is no write hand-back left to relax. This plan's unit table is not renumbered; the other units stand as written.
