...
- R9. When the operator is `on`, it runs before admission; a thread admits one live run per unit; a bind of `steer` folds into the named run at its next boundary; a reply with no live run the decision steers is a bind.
...
- R13. The thread session is keyed by the thread alone; the operator reads and writes it; every connector's event the intake gate admits, every child's report, every question and answer is a turn in it.
- R14. A working session is keyed `<instance>:<unit>:<lane>`; every coding round of a unit continues `<instance>:<unit>:coding` and every review round `<instance>:<unit>:review`; a re-issue of the same plan id continues the prior instance's lanes; a child's first turn is the parent's brief followed by the thread session's tail within the seed budget.
- R15. The session object gains an idempotent keyed append; a fold's row id is the child's run id with its attempt; a connector's is the message id with its edit timestamp.
...
- AE1. **Covers R6, R8, R9, R19, R21.** Given a thread with one ship unit idle at its lease and `routing.operator: on`, when the requester replies with three bullets (a personal setting, a console change, a cli change), then one decision returns two binds (`config set me …` and a plan of two units, one per repository), the setting runs with a receipt naming its line and class, the plan's receipt names both repositories before any round, and the idle unit is untouched.
...
- KTD6. **The thread session is keyed by thread; a working session by instance, unit and lane; the log gains a keyed append; one live run per unit is a store constraint.** Record 0034's isolation hangs on the unit, not the agent, so one thread per plan is no longer blocked by a shared `<thread>:coding` key; the fold's idempotency key is the child's run id with attempt, a connector's the message id with its edit timestamp; `SessionLogDO` gains a `row_id` beside `(idx, part)` and an `append` route that ignores a repeated id. The one-live-run guarantee lives in the memory worker's `live_runs` table and the ledger's claim, so per-unit admission rebuilds that table with a unit key and the memory worker deploys ahead of the bot for that unit. Governs R9, R13, R14, R15, R16.
...
- KTD13. **Admission runs after the operator when it is on, and a thread admits one live run per unit.** Today's admission folds a reply into the live run before any model reads it; under `on` the decision names the run an event steers, or none, and record 0051's owner rule survives as that sentence. The durable inbox, the boundary fold and the coordinator's `coordinator_thread_live` refusal stay; the refusal narrows to the same unit. Governs R9.
...
sequenceDiagram
  ...
  alt destructive
    ...
    A->>M: admit: steer names a live run, or new run per unit
...
U1 to U3 first: nothing a person reads changes, and every later unit's tests need authored rows, the class predicate and the replay rows. U4 ships the operator in shadow with its cap, its order and its log; U5 moves admission and rebuilds the live-run constraint (memory worker before bot); U6 wires the verifier while shadow data accrues. U7 re-keys the memory and U8 adds the briefs and wraps the two unfenced child paths. The flip to `on` follows cut three on its named gate. U9 to U11 widen the projection and the runner. U12 to U15 delete the readers in the order of least risk: directives, the typed line, the repository scan, then the attach token and the link unwrapping with the lint rule. U16 last, on its four gates.
...
- **Two units in one thread exceed what the resident's per-thread worktrees allow.** The resident already takes one worktree per thread and ref; U9 names the branch per unit so two units in one repository take two.
...
- **Depends on** the requester fix's deployment (U16's gate), record 0058's intake gate (the `on` mode's gate, not `shadow`'s), record 0054's first two units as merged, record 0044's store and buttons, the resident plane's `/residents` index, the memory worker deploying ahead of the bot for U5, and the review loop.
...
| U5 | Admission after the operator, one live run per unit | `src/core/dispatcher.ts`, `src/core/dispatch/admission.ts`, `src/core/threadAdmission.ts`, `src/core/dispatch/authorize.ts`, `src/core/runLedger/decisions.ts`, `src/core/runLedger/ledger.ts`, `deploy/cloudflare-memory/worker.ts` | U1, U4 |
...
| U9 | A repository per unit, the unit cap, preflight before the instance | `src/core/coordinator/contract.ts`, `src/core/coordinator/handOff.ts`, `src/core/ship/coordinator.ts`, `src/core/coordinator/driver.ts`, `src/channels/adminCoordinator.ts` | the `on` gate, U7 |
...
### U5. Admission after the operator, one live run per unit

- **Goal**: with the operator `on`, the decision runs before admission; a bind of `steer` folds into the run it names at that run's next boundary; a thread admits one live run per unit in the store as well as in code; a steer by someone other than the run's requester without a grant is refused; the runner's stand-in as requester holds only for steers into runs of its own plan instance.
- **Requirements**: R2, R9, R27 (thread-admission items 1, 2, 5 to 8; run-history item 29; authorization item 16).
- **Dependencies**: U1, U4.
- **Files**: `src/core/dispatcher.ts` (stage order under `on`: operator, then authorize each bind, then admission per bind); `src/core/dispatch/admission.ts` (`admit` takes a target run for a steer; one live run per unit; `coordinator_thread_live` narrows to the same unit); `src/core/threadAdmission.ts` (`decideFollowUp` reads the decision, not the directive); `src/core/dispatch/authorize.ts` (the steer owner rule: requester or grant; the coordinator tag stands in only for a steer whose target's lineage names the runner's instance); `src/core/runLedger/decisions.ts` and `src/core/runLedger/ledger.ts` (`decideClaim` and `claim` take the unit key); `deploy/cloudflare-memory/worker.ts` (`live_runs` rebuilt with `UNIQUE (thread_key, unit_key)`, a unit-less run's unit key the empty string, existing rows copied once); tests `admission.test.ts`, `threadAdmission.test.ts`, `dispatcher.test.ts`, `authorize.test.ts`, `decisions.test.ts`, the Worker's test.
- **Approach**:
  1. Tests first: two live units in one thread claim two rows; a steer names the second and folds there; a new bind while both are live starts a third run for a third unit; a second run for the same unit is refused; a member's steer into the requester's run is refused with the reason; the runner's steer into its own child passes; the runner's steer into a run outside its instance is refused; the runner's destructive bind on a person's run outside its instance is refused.
  2. Reorder the stages under the flag; widen the claim to per unit in the ledger and the store; add the owner rule.
  3. Spec rows: thread-admission items 1, 2 and 8; run-history item 29 (one live run per unit, enforced by the store); authorization item 16 (the steer owner and the runner's bound stand-in).
- **Execution note**: the memory worker deploys before the bot for this unit; the release's deploy order carries that.
- **Patterns to follow**: `admit` outcomes as a closed union; `steerRun` and the durable inbox; `canRunAgent`; the `live_runs` table and `decideClaim`.
...
- **Dependencies**: U1, U4.
- **Files**: `src/core/runLedger/sessionLog.ts` (`threadSessionKey(threadKey)`, `workingSessionKey(instance, unit, lane)`); `deploy/cloudflare-memory/worker.ts` (`turns.row_id` with a unique index; an `append` route idempotent on the id; the migration read; `sessions` registration of a working key under the unit's thread); `src/core/runLedger/writeThrough.ts` (`append(key, rowId, rows)`; the fold); `src/core/runLedger/ledger.ts` (the per-thread run listing the migration orders by); `src/core/dispatch/seed.ts` (the operator reads the thread session; the child's hand-down); `src/channels/adminCoordinator.ts` (children dispatched with the working key); `src/core/unitRuns.ts` and `web/src/pages/UnitPage.vue` (sessions from the new keys); tests `sessionLog.test.ts`, `writeThrough.test.ts`, `seed.test.ts`, `unitRuns.test.ts`, the Worker's test.
- **Approach**:
  1. Tests first: a fold written twice under one run id and attempt yields one row; an edited message appends a second row; two connectors in one second keep the object's order; a re-issued plan continues the prior instance's coding lane; a live run at cutover folds its remaining turns at its end; ten recorded threads replay through the migration in run order and a later bind finds a folded fact.
...
- U5 (memory worker deployed before the bot) and U6 merged; U7 and U8 merged with the repository row posted.
