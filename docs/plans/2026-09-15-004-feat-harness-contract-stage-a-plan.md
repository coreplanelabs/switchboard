...
*Amended 2026-09-16 (U10, from the probes above; Approach steps 1 and 2 and KTD6 read with these):* the image grep is `'opencode v2.0.3'`; the plugin reference is the directory `./plugins/switchboard`; the ask-on-all rule precedes the identity's denies; the layout adds `home/`, the tailer's files and `cmd/`.
