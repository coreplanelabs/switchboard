...
- **Dependencies:** U12.
- **Precondition:** the loop-level OpenCode relaunch scenario (a living bot, the container replaced with a call in flight, OpenCode relaunched from the record, `relaunches = 1`, the run completed — the OpenCode counterpart of `runLoop.test.ts`'s pi relaunch test, over the fake serve driving the real `OpenCodeHarness`) passes before any preset's word is set to `opencode`. U12 proved the ceiling at the harness (the verdict with its record; `open` under relaunch taking the registration over) and the loop's relaunch is harness-neutral, but the end-to-end proof needs the word itself to configure a run on OpenCode, so it lands with the word and gates it.
...
| Does v2 load a local plugin file without a package install, and does a failing `execute.before` block? | the implementer of U12 | one local `serve` with the plugin file (spike item 9) | **closed 2026-09-16** by U12 (below) |
| Does `import` accept authored assistant turns and validate `projectID`? | the implementer of U12 | one import against the binary (spike item 10) | **closed 2026-09-16** by U12 (below) |
| Effort tiers onto model variants? | the implementer of U12 | the schema and one run (spike item 11) | **closed 2026-09-16** by U12 (below) |
...
**Closed 2026-09-16 by U12 (spike items 9, 10, 11), each against a local `opencode serve` at `@opencode/cli@2.0.3` with a scripted openai-compatible model and a fake `/harness/*`:**

- Item 9 — a local plugin DIRECTORY loads with no package install and no `bun` step: `POST /api/plugin/await-activation` settled 204 and the plugin's setup ran (twice — OpenCode watches the plugin directory and re-runs setup per generation; registration is idempotent by name). A relayed tool MUST register through `ctx.tool.transform`'s editor with `options: { codemode: false }` — without it the tool is CodeMode-only and a direct model call answers `No tool named "<x>" is currently available`; with it the model calls it by its own name, and the plugin's execute ran `POST /harness/authorize` then `POST /harness/tool` with the model's call id (`context.id`) — the SAME id on both. The `execute.before` hook fires before each tool; its failure channel is a `Tool.Error` the types say blocks the call (`packages/plugin/src/effect/tool.ts`), but the gate never uses it — the gate rides `permission.asked`, decided in the bridge (KTD6). Also found live: a plugin-registered relayed tool raises NO `permission.asked` (its `session.tool.called` has no matching ask, verified in the U12 real-binary receipt), so it is gated only by the plugin's `/harness/authorize` and runs in the bot, never in the container — the bridge's bypass/refusal detection therefore skips relayed tools by name.
- Item 10 — `POST /api/session/import` accepts authored assistant turns with NO `providerState`, and resolves `projectID` from the body's `location.directory` (a body `projectID` of `global` was rewritten to the location's real project id). `GET /api/session/:id/message?order=asc` listed exactly one store message per seed message (3 seed → 3), and the next `POST …/prompt` continued from it (→ 6 messages). So the survival word is authored-session: the seed is one store message per seed `ChatMessage`, and a call in flight at a death is a completed/error tool content carrying the settlement note.
- Item 11 — the effort tier maps onto the model's declared `variants`: `providers.<p>.models.<m>.variants: [{ id, settings?/headers?/body? }]`, and a session selects one via the model ref's `variant` (`{ providerID, id, variant }` on create or on the import body's `info.model`). A `body` overlay on the variant reaches the wire — a variant carrying `body: { reasoning_effort: "high" }` put `reasoning_effort: "high"` on the model request. Stage A's configuration writer (U10) declares no reasoning variants, so there is none to select and `OpenCodeHarness.effort` returns `undefined` (OpenCode's default) until a deployment declares them — a follow-up to the configuration word, not a stage-A behaviour.
