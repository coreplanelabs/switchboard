...
- **Objective**: Unit 2 of record 0036. The route stage's one forced model call offers one tool per chat-exposed command beside the `route` tool, derived from each command's definition; a command the model picks runs through the registry as the message's user with a receipt line first; five commands are handed back as a line to paste instead of run; a command that cannot serve re-routes once over the presets; every offered command carries routing fixtures the conformance suite requires and the replay scores; the two hand-written regexes and their fast path are deleted once the replay shows the door carries their forms. When the last unit merges, "use opus for coding in this channel" lands the channel override with its receipt line and no run, and a command added to the registry is routable the day it lands or `verify` fails by name.
...
- R8. A command that answers `not_found` or `unavailable` seals its inline run with no reply and is re-routed once over the presets alone; the request runs on the preset that answer names or on `defaults.agent`; the record carries both decisions.
- R9. A `CommandDef` may carry `handBack: true`. The command stays in the menu; when the model binds it, the route stage answers `To run this: <chat form>` and invokes nothing. Five defs carry it: `repo.offboard`, `repo.rebuild`, `mcp.remove`, `runs.stop`, `memory.forget`.
...
- R14. `recognizeOperation`, `answerOperation` and the two regexes are deleted once the replay's command row holds at 100 percent on the `repo_test` and `repo_build` fixtures, including the forms that take the repository from the thread (R16); their `not_found` and `unavailable` outcomes reach a preset through R8.
...
- KTD6. **`handBack` is a `CommandDef` flag, structural like `routable` on presets.** The five defs carry it; `routeRequest` reads it after the bind and before `invoke`, so the hand-back is the one routed reply produced without the registry's authorize step, which is safe because it echoes the requester's own words and runs nothing. Governs R9.
- KTD7. **The re-route is one more `route()` call over the preset table alone.** On `not_found` or `unavailable` the stage seals the command run with no reply (the fall-through `answerOperation` performs today), then calls `route()` with the presets and no command tools, records both decisions on the `route` event (`command`, then `preset`), and the dispatch proceeds as a preset route; the trace's root rebinds to the agent run as it does now. The second call runs over the full table under the same allowlist and gates, so after the ship plan's flag swap it can land an imperative on `ship`; named in Risks. Governs R8, R14.
...
flowchart TB
  ...
  M -->|command| C{handBack?}
  C -->|yes| H[reply: To run this: chat form; nothing invoked]
  C -->|no| I[namedToInput; commands.invoke as the user, audit source route]
  ...
  I -->|invalid_input or unauthorized| U[the existing usage or restricted line]
  I -->|not_found or unavailable| S[seal the command run; route again over presets alone; run there or on defaults.agent]
```
...
- **The re-route can land an imperative on ship.** The second `route()` call runs over the full table; once the ship plan's flag swap lands, "run the tests on main in acme/api" on a repository with no resident could be answered `ship`. The record accepts a wrong route to the write preset as a pull request's worth of cost; named here so the receipt tester expects it.
...
- **The regex deletion adds a model turn to "run the tests on main".** About a second on the fast model where the regex answered at once; the fall-through is kept by KTD7, and the thread-repository form by KTD10.
...
- **Goal**: The route stage offers the derived command tools with the thread's repository as context, runs a chosen command through the registry as the message's user with the receipt line first, hands the five flagged commands back, posts a settle follow-up, and re-routes once when a command cannot serve; every record field it adds is redacted and capped.
...
- **Dependencies**: U1.
- **Files**: `src/core/dispatch/route.ts` (`routableCommands`, the tool set, the command decision in `route()`, `RouteStage` `command` kind, the command branch of `routeRequest` with the hand-back, the invoke, the follow-up, the seal and the re-route, the thread-repository line, `unwrapChatLinks` on the text, `ROUTE_RECEIPT_CAP`, the redacted `route` event); `src/core/dispatch/commandRun.ts` (new: the five functions moved from the fast path; `runInlineCommandRun` takes an optional `route`); `src/core/dispatch/fastPath.ts` (imports them); `src/core/dispatcher.ts` (hands the route stage the channel handle, the ending, the trace and the commands; answers the `command` kind before the agent gate); `src/core/commandRegistry.ts` (`CommandDef.handBack?: true`; `TraceOptions.source` and `AuditEntry.source`); `src/core/commandSurface.ts` (`chatInvocation(cmd, input)` from the promoted `toChatText`, `toArgv` and `quoteChatToken`); `src/core/testing/commandConformance.ts` (imports them from the surface); `src/core/commands/repo.ts`, `src/core/commands/mcp.ts`, `src/core/commands/runs.ts`, `src/core/commands/memory.ts` (the five flags); `src/core/runEvents.ts` (the `route` event carries `command`, `input`, `receipt` and a second decision); `src/load/routeCommandFixtures.ts` (new: one happy path per offered command, the seed U3 completes); `src/core/dispatch/route.test.ts`; `src/core/dispatch/commandRun.test.ts`; `src/core/dispatcher.test.ts`; `src/core/commandSurface.test.ts`; `src/core/commandRegistry.test.ts`; `docs/reference/specs/routing-and-config.md`; `docs/reference/specs/command-registry.md`; `docs/reference/specs/run-history.md`.
- **Approach**:
  1. Tests first, red against today: `routableCommands` over the full catalogue lists 29 tools and over the empty capabilities 12, each with the MCP name, description and schema; `route()` with a scripted `config_set` call returns a command decision with its input and no side effect; `routeRequest` binds `{ scope: "channel", models: { coding: "anthropic/claude-opus-5" } }`, invokes as the message's user with `source: route` on the audit line, and the reply's first line is `routed: config set channel --models.coding anthropic/claude-opus-5`; a wrapped Slack link binds to a bare URL; a call to an unoffered name is no route; every def with `handBack` answers `To run this: <chat form>` and invokes nothing, and the flagged set equals the five ids; a `repo_test` call whose handler answers `not_found` leaves one sealed `failed` command record and routes again over the presets, both decisions on the event; a bound input carrying a token-shaped string is redacted on the `route` event and the receipt is cut at its cap; a thread naming a repository puts it in the router's user turn and a `repo_test` call binds it; a live thread, a sticky thread and a pinned scope are not routed; `chatInvocation` round-trips every conformance variant through `parseInvocation`.
  ...
  4. Add `handBack` to `CommandDef` and to the five defs, with a one-line comment each on why a reply cannot undo the misfire; add `source` to `TraceOptions` and `AuditEntry`, copied by `invoke`.
  ...
  6. In `routeRequest`, on a command decision: `handBack` answers the hand-back and returns; otherwise `namedToInput` on `unwrapChatLinks(text)`'s bind, then `runChatCommand` as the message's user with `source: route`, the receipt line first, the follow-up posted through `postSettledOutcome` when the result settles; on `not_found` or `unavailable` seal the command run with no reply and call `route()` once more over the presets alone (KTD7); publish the `route` event through `redactAndCap` with `ROUTE_RECEIPT_CAP`.
  ...
  8. Spec rows: item 21 (the menu, the thread-repository line, the command branch, the receipt, the hand-back, the re-route, the redaction and the audit mark), item 10 (stage A stays the exact grammar; the door is the natural-language path), command-registry item 7 (the audit line's `source`), item 18 (a second path to a command, through the door, with the same `invoke`), item 20 (a `handBack` column on the five rows), run-history item 2 (the `route` event's command shape).
...
- **Patterns to follow**: `routeTool`, `parseRouteAnswer` and `tidyReason` in `src/core/dispatch/route.ts`; `invokeChatCommand` and `unwrapChatLinks` in `src/core/commandChat.ts`; the fall-through seal in today's `answerOperation`; the `attach_file` pre-check landing in the same stage; the MCP adapter's tool listing in `src/channels/mcp.ts`.
- **Test scenarios**:
  ...
  - `commandRun.test.ts`: the moved machinery records an inline run for `repo.test` and none for `config.set`, exactly as the fast path did; a `route` passed in is published after `run_meta`, redacted and capped; a fall-through leaves one sealed `failed` command record.
  ...
  - `dispatcher.test.ts`: "use opus for coding in this channel" with a scripted `config_set` call writes the channel override, replies with the receipt line first, claims no thread and starts no run; a wrapped Slack link in an `mcp_add` call binds bare; every `handBack` def is handed back and nothing is invoked; a scripted `repo_onboard` call posts the acknowledgement and then the settled outcome; a `repo_test` `not_found` seals the command run and runs the preset the scripted second call names; the same message in a live thread, a sticky thread and a pinned channel is not routed; the typed `config set channel …` still answers inline with no model call.
...
- **Goal**: The two hand-written recognizers and their fast path are gone; "run the tests on main in acme/api" and "run the tests on main" in a thread that names the repository reach `repo_test` through the door, and fall through to a preset when the repository has no resident.
...
- **Approach**:
  ...
  3. Spec rows: item 7 (the natural forms reach the command through the door with the thread's repository as context; the fall-through is the re-route), item 10 (stage B is gone; the door is the natural-language path), command-registry item 24 (the natural forms sentence), slack-commands line 138 (a plain sentence routes to the command with a receipt line).
...
| Live, human-gated: a routed write with its receipt | A new top-level message in a channel with no `agent` scope: "use opus for coding in this channel". Expect one reply whose first line is `routed: config set channel --models.coding anthropic/claude-opus-5`, the override visible in `config show`, no card and no run on the runs index | U2 |
| Live, human-gated: a hand-back | "offboard <an onboarded slug>". Expect `To run this: repo offboard <slug>`, the resident still onboarded | U2 |
| Live, human-gated: the natural op form through the door | "run the tests on main in <an onboarded repo>", then "run the tests on main" as a reply in a thread that named that repository. Expect the receipt line `routed: repo test <repo> main` and the op's result both times; the same first message in a repository with no resident re-routes to a preset, named on the card | U4 |
...
- No routed command reaches a handler except through `commands.invoke`; no `handBack` command is ever invoked by the route stage; the replay invokes nothing; no regex recognizer remains.
...
| After the ship plan's flag swap, should the re-route over presets offer read presets only, so a natural op on a repository with no resident never lands on `ship`? Decided as the full table, revisable | the maintainer | the first re-routed natural op read on the run page after both plans land | deferred |
