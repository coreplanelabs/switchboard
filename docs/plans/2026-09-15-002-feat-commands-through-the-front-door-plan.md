...
- **Amendment, 2026-09-16**: the unit ran with the exec carve-out (record 0039, amendment of the same date). `repo.test|build` are `effect: write`, so under R9 as written the door would have handed the natural forms back as the line to paste instead of running them, and deleting the regexes would have regressed "run the tests on main" from an answer to a paste. `routeRequest` now decides by `routedRunsAtOnce(def)` in `src/core/dispatch/route.ts` — a read or a `:exec`-class action runs at once, a `:write`-class write is handed back — tested on its own in `route.test.ts` and enumerated over the catalogue in `dispatcher.test.ts` (the exec-class writes are exactly `repo.test` and `repo.build`); R9's "every write is handed back" reads "every state-changing write" from here on. R14's `not_found`/`unavailable` shape is unchanged: the command's own line and the footer, no second call. `references.test.ts` swapped `recognizeOperation` for `repoFromThread` in its parsers-unaffected invariant (record 0037 names both).
