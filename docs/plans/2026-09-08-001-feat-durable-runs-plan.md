...
## Amended 2026-09-18 — the handoff is the recovery for a roll, not a licence to roll

*Re-evaluation, as an amendment after acceptance asks.* D8 made the drain a handoff and, with it, the bot deploy preflight stopped refusing for runs in flight: a deploy that lands on a live run hands the run to the next generation, which resumes it within seconds, so nobody waits on a run and a release never needs a person for one. The plan was accepted on that reasoning and on criterion (1), a run surviving a forced deploy. Checked against it: the resume is a rebuild of the run's session on the next generation, and a rebuild can be refused by the harness it is rebuilt for — one release rolled the bot under two coding children, both re-attached, both ended `failed` when their harness rejected the rebuilt session, and each cost its unit the attempt. The handoff held as a mechanism and still lost the runs: it is a recovery for a roll that happens (a hard kill, a forced deploy), not a guarantee a deploy may lean on.

**What changes.** The bot deploy preflight and `deploy restart` refuse while `inFlight > 0`; `deploy all` retries every minute up to its budget and then **fails by name** — the budget, the refusal, `NOT deployed`, the re-run recipe and what `--force` would kill — never a deploy over what refused (slack-channel item 8, release-and-deploy item 13). A bounded wait that then proceeds is a race with the run it waited on; the wait stays only as how long to hold before failing. The cost accepted: a release under a long run goes red and is re-run by hand once the run finishes — a minute — where a killed run costs its attempt.

**What does not regress.** D8's drain is unchanged: a forced deploy, a hard kill and a container the platform evicts still hand every resumable run to the next generation, and criterion (1) still holds for them. `DRAIN_DEADLINE_MS`, `COLD_START_ALLOWANCE_MS` and the catch-up window are untouched. The workflow still never re-dispatches itself and takes no wait input; the `busy` exit code stays out of `deploy all`.

## Amended 2026-09-19 — the handoff rule is restored: the gate held deploys hostage to a recovery that works

*Re-evaluation of the 2026-09-18 amendment, as a further amendment — the accepted body above stands unedited.* The gate that amendment introduced (the bot deploy preflight and `deploy restart` refusing while `inFlight > 0`, `deploy all` holding up to its budget and then failing by name) was checked against the evidence it rested on. The two coding children it cited did not die of the handoff: they died on the rebuild path — the harness refusing the rebuilt session — and that path was fixed separately (the OpenCode child that motivated the gate survives it now). Checked forward: a forced restart on 2026-09-19 rolled the bot under nine live runs and lost none — every one re-attached on the next generation and finished under its own card. Meanwhile the gate's cost was real and recurring: a release behind one long coding lease held for its whole budget and then went red with nothing deployed, for a person to re-run — the exact shape D8 was built to remove.

**What changes.** Back to D8 as accepted: the bot deploy preflight and `decideRestart` WARN for runs in flight (naming the count and the handoff) and proceed; `deploy all` waits only for a container rollout still settling, and the bot step carries no budget of its own (the 90-min `BOT_WAIT_MAX_MS`, sized for the gate's hold, goes with it). What the gate's PRs added and still holds is kept: a refusal standing at the end of the wait budget — a stuck rollout, a busy resident — fails by name (`preflightGaveUpLine`: the budget, the refusal, `NOT deployed`, the re-run recipe), and the drain and the still-draining line name the held run ids instead of a bare count.

**What does not regress.** The resident preflight still refuses for a resident's runs in flight (its swap has no handoff) with its own 30-min budget; the rollout-in-progress refusal, the fail-closed cases and the force bypass are unchanged; `DRAIN_DEADLINE_MS`, `COLD_START_ALLOWANCE_MS` and the catch-up window are untouched, and the workflow still never re-dispatches itself.
