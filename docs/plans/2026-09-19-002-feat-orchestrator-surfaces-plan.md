...
- **Scope**: R1 to R3. Tests first: `indexRow.test.ts` asserts `inheritedPace` returns the newest live child's text, `waiting on a child` with none, and nothing for a finished head; `runsIndex.test.ts` asserts the cell on a hosted head. Then the code and item 32's sentence.
...
- **Validation**: the two tests, `npm run verify:changed` on the web set; the live receipt is the index cell on a pipeline in flight, posted on #1960.
...
- Every unit: `npm run verify:changed` over its changed set, the fixtures it touches regenerated by their `gen`, `npm run hygiene:check`, `npm run check:pr-title`.
