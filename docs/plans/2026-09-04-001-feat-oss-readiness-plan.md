...
2. **Runs in 10 minutes, and nothing you do is a one-way door.** This is a product people *install*, not a repository they fork (Justin, 2026-09-07): one command (`npx switchboard init`) asks for the account, the domain and the Slack app, writes the config and the deployment profile, and deploys — to Docker on their own host, or to their Cloudflare account. Time-to-value is a design tentpole: every step is one command with a default, and every choice is reversible — config is a readable file, generated deploy files are regenerated not edited, execution backends and optional subsystems switch on and off in config, data (runs, memory) is exportable. Cloudflare, GitHub App, E2B are optional and say so. Our own production is one installation of the product, run from our infra repo's profile and config with the same commands.
...
- **PR 1 — config out of the image.** `config/config.production.yaml` leaves the tree for `coreplanelabs/infrastructure`; the Dockerfile stops copying `config/`; `SWITCHBOARD_CONFIG` (a path) stays the one runtime contract and `docker-compose.yml` mounts a file into it; `config/config.example.yaml` becomes the only config in the repo and every block documents its off-state. The bot Worker's `containerEnv` keeps pointing at the path the deploy tooling placed the file at (PR 3). `src/config.production.test.ts` becomes a golden over the example, and moves to the infra repo as the golden over the real file.
...
- **PR 3 — our production on the product's path.** The release workflow fetches our profile and config with the token it already holds and calls `deploy all`; nothing in the repository names our account, hostnames, or organization. Runtime identity from config, not code: `organization` (memory org scope; prod sets `coreplanelabs` so existing rows keep their key), the ship pipeline's PR author derived from the GitHub App at startup (`GET /app` → `<slug>[bot]` + id), self-description text built from config.
...
- **Acceptance**: `grep -r coreplane deploy src config` is empty; `npm run cli -- deploy plan` runs from the example profile; `docker compose up` with a mounted `config.yaml` against the built image answers `ask`; a generated `wrangler.jsonc` is byte-identical to today's after the profile is filled with our values (the migration proof); our production deploys from CI through `deploy all` with a fetched profile, receipted on the release PR.
