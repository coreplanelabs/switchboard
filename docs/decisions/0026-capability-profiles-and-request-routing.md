...
| Directives: `agent`, `model`, `effort`; no budget directive exists. Thread stickiness is derived by scanning the thread's user text for them; bot text is never scanned; the sticky agent enters resolution as the request's own | `src/directives.ts:23-40`, `src/core/dispatch/resolve.ts:76-89`, `src/channels/slack.ts:630` |
...
| A scope holds `agent, model, models, effort, efforts, instructions, mcpServers` and nothing quantitative; any Slack user writes their own user scope (`config set me`), channel scope writes need `config:write` | `src/config.ts:44-77`, authorization spec item 4 |
...
A **profile** is what a run may reach, who it acts as, and how long it may run: **reach** is a set of **tool bundles** (`shell`, `files`, `write-files`, `web`, `search`, `github-read`, `github-write`, `pr`, `verdict`; today's `full` is the first eight less `search` and `verdict`, `readonly` is `shell`, `files`, `web`, `github-read`, `verdict`, `web` is `web`, `search`, `github-read`, `assistant` is `web`, `github-read`, `github-write`) plus a **machine class** (`none`, `blank`, `repo-cold`, `repo-resident`); **identity** is the credential scope minted for the run (`none`, `read`, `write`); **budget** is minutes, turns and tokens, of which only minutes are ever capped by a boundary or a directive (turns and tokens stay the preset's: the first is a runaway backstop, the second a per-call limit). A **preset** is a named profile with a prompt; the five agents are the first five presets and behave as today. Two questions decide a run, and both are answered by mechanisms that already exist. The policy table answers who: `has-grant(agent:run:<preset>)`, exactly as today, its vocabulary untouched. The config layers answer how much: a **boundary** is a cap on the three axes (`maxMinutes`, `maxIdentity`, `machines`) that any scope may set (defaults, channel, user), resolved beside `model` and `effort`, and the **effective profile** is the intersection of the preset, the request's directives and every boundary on the path. A profile the boundaries reject is refused by a gate in the pipeline's authorize stage, beside the repo and PR-head gates that already live there. Identities are ordered `none < read < write`; a preset's single machine class must be in every boundary's `machines` set. A boundary never grants; a directive never widens; the executor is provisioned from the effective profile and nothing else. The **router** is a filter that picks a preset when nothing but `defaults.agent` would have; its output is a preset like any other.

The closest known shape is an IAM permissions boundary over an identity policy: the preset says what may be done, the boundary caps it, the effective permission is the intersection, and the boundary grants nothing. The one way this differs: a budget above the cap is clipped with a visible note rather than rejected, because a shorter run still ends in the runner's forced write-up, while an identity or machine class above the cap is rejected, because a preset that needs to push cannot do its job with a read token.
...
4. Still in the resolve stage, the config layers resolve the boundary and intersect it with the preset: `explore` declares 120 minutes, identity `read`, class `repo-cold`; the channel boundary caps minutes at 45; no other layer sets one. The effective profile is 45 minutes, `read`, `repo-cold`.
5. The authorize stage runs its gates: the policy table's one question, `has-grant(agent:run:explore)`, admits the caller (`explore` is unrestricted), and the profile gate finds every axis of the effective profile within the boundaries, so nothing refuses. The card says `budget 45 min (channel boundary; preset asks 120)`. The property: a routed profile cannot exceed a boundary and the clip is visible where the run is watched.
...
The second hard case behaves differently: a channel bounded to `maxIdentity: read` receives `agent:coding`. The policy table admits the caller; the profile gate finds the preset's identity `write` above the cap and refuses: `🚫 coding needs a write credential; this channel is bounded to read. Run it in a channel that allows write, or ask an admin to raise the channel's boundary.` No executor exists.

## The difficulty map

Ranked by risk of being wrong.

1. Boundaries: intersection over config layers that today override, the clip-or-refuse rule per axis, and proving an absent boundary is today. Section "Boundaries."
...
How this extends the prior art rather than adding a second model. The authorization record fixed one decision function over a closed vocabulary, asked once per request; the table keeps that one question and gains no condition kind. The layered-configuration record made effort a first-class setting resolved through the same layers as the model; a boundary is a third such setting, with intersection as its resolution rule. The pipeline records made the authorize stage a list of named gates (`agent_allowlist`, `repo_unverified`, `pr_head_unknown`); a profile the boundaries reject is one more gate in that list, refusing with a named reason before any executor exists. The authorization plan left agent tool-level actions with the resident for now, to move onto the table later through the `agent` actor kind; that move is unaffected, and when it lands identity and machine class are resource attributes the table can select on, no comparison needed since both are enums.

A boundary is `{ maxMinutes?, maxIdentity?, machines? }`. It lives where the other scope settings live, `defaults`, `channels.<id>`, `users.<id>`, set in `config.yaml` or by a new `--boundary` option on `config set` under the existing `config:write` gate for the channel scope; a boundary for a credential or a schedule actor, if one is ever needed, is a sibling `boundaries.<actorId>` block, never a fourth grants axis. A `maxMinutes` under 2 is refused at load: the bash tool keeps a 60-second reserve, so a shorter run could never execute a command. Resolution differs from every other scope setting on purpose: models and effort take the most specific layer, boundaries intersect across all of them (the smallest `maxMinutes`, the lowest `maxIdentity`, the intersection of `machines`), so a user's boundary can tighten a channel's and never loosen it, and an absent boundary caps nothing. The **effective profile** is preset ∩ directives ∩ boundaries, computed once in the resolve stage and checked by the profile gate in the authorize stage, before provision. A budget above the cap is clipped and named on the card and in the record (`boundedBy`); an identity or machine class above the cap rejects the run, naming the boundary and its scope, before any executor exists. Directives are the caller's own boundary on one run: a new `budget:<minutes>` directive joins `agent`, `model` and `effort`; `budget:30` narrows; `budget:120` above the preset or a cap changes nothing and says so. Unlike the other three it is never sticky: a budget is a property of one request, and a thread that wanted a lower budget on every turn sets a user boundary instead. Because any Slack user may write their own scope, a user boundary can only ever tighten what the channel and the defaults allow; intersection is what makes self-service safe. The ship pipeline's `ship.maxMinutes` knob becomes the ship preset's declared budget in that deployment, one number instead of a pipeline cap beside a placeholder; its child rounds run under the parent's effective profile, clipped to the remainder as today.
...
| A 4 vCPU / 12 GiB sandbox runs a large monorepo's typecheck sequentially, and a `repo-cold` clone plus install of that monorepo leaves enough of a 45-minute clipped budget to do useful work | the comparison run, timing the clone and install steps on its run page | the comparison run |
...
Slice one, before the comparison run: the four machine classes in the factory and repo resolution; the profile the runner and the ledger read; the boundary field on scopes, its intersecting resolution and the profile gate; the `explore` preset (shell, files, web fetch and search, GitHub reads; `repo-cold`; identity `read`; 120 minutes); the sandbox Worker's timeout hint corrected to `setsid -f`. No router; `agent:explore` names it. Three PRs, a few days: the classes and repo resolution; the boundary field, its resolution and the profile gate; the preset and the budget field. Slice two: the router behind `defaults.models.router`, off when unset, with the sticky label. The image manifest is its own record and slice. Each slice is its own plan through the review loop; the routing-and-config, execution, authorization and agent specs change in the same PRs.

## Open questions

Decided, revisable: `explore` runs on identity `read` whatever the caller holds; an investigation that must push is a second preset, so a read-only investigation can never be turned into a write by a directive. Decided, revisable: budgets clip, identities and machine classes reject. Decided: `budget:` is a per-message directive and never sticky; the routed preset is sticky through the bot's card label.
...
- [0002](0002-dispatcher-is-the-only-orchestrator.md), [0005](0005-layered-config-effort-first-class.md), [0007](0007-authorization-policy-table.md), [0018](0018-capabilities-computed-once-null-objects.md), [0024](0024-dispatcher-as-a-staged-pipeline.md), [0025](0025-dispatch-pipeline-as-built.md); the [authorization model plan](../plans/2026-09-03-001-feat-authorization-model-plan.md), whose scope boundary leaves agent tool-level actions to a later move onto the table.
