...
*Amended 2026-09-18, while proposed, after the reconciliation read against [record 0057](0057-the-operator-is-the-one-door-a-model-binds-every-chat-input-and-deterministic-code-authorizes-fences-and-executes.md).* Three points. (1) Vocabulary: 0057 fixes **operator** as the model turn that binds a chat input, so the person who stops an orphaned pipeline is the **maintainer** here and in the plan; nothing in this record's design names an operator. (2) The host claim is released by every exit that hands nothing off: a request refused after the claim, including a preflight question that ends a plan bind (0057's unit nine), finishes its run as today and leaves no host-keyed row, so a question never locks a thread out of ship until a reclaim. (3) Under 0057 the `agent:ship` line becomes a plan bind and the host claim moves to the bind's executor; the live receipts this record's plan types as `agent:ship` are rerun once 0057 deletes the directive syntax. Sequencing agreed with 0057: this record's units one to three land first; 0057's unit nine follows this record's units three and five. 0057 takes its own dated amendment in return: occupancy is owned by this record's host key, a unit's report is this record's `ship_unit` event projected by the thread session, and working sessions register by the run's metadata thread key.
...
## Amended 2026-09-18 — a `merge: person` unit holds its thread while its pull request is open

*Re-evaluation, as an amendment while proposed, from a production incident.* A routed ship run (a generated one-unit plan, `merge: person`) was approved after one review round and ended `merge_ready`; the requester replied fifteen minutes later correcting the design. The unit had an ending, so under record 0051's owner rule the thread had no owner, the router ran on the bare text with no thread in view, bound it to `general` on the fast tier, and stickiness carried that binding through every later reply — including a plain order for work. No coding round ever started; the pull request sat open and approved at the old head for four hours. The failure is structural, not a routing miss: `merge: person` treats the posted approval as the unit's end, while the pull request — the thing the person is iterating on — is still open, and record 0051's mailbox has a reader only while the unit is unfinished. Ownership ended one event too early.

**The decision.** A `merge: person` unit's ending still settles the walk — `merge_ready` blocks dependents and releases the next ready unit exactly as today — but the ending no longer releases the thread. The unit stays the thread's owner until its pull request is **merged or closed**, and record 0051's warm window starts at that point, not at the approval: the approval is the middle of the person's loop, the merge or close is its end. Concretely, the owner rule's unit clause splits in two: the unfinished unit owns first, and after it the merge-ready unit whose row names the thread and whose pull request is still open — decided by one bounded read of the pull request's state, a failed read leaving the row out so the thread routes fresh rather than guessing. A plain reply in that span is the unit's thread event, appended to the row's list exactly as an unfinished unit's is, never the router's; a merged or closed pull request returns the thread to the router (through the warm window, when it is above zero).

**Consequences decided here.**

- **Multi-unit plans.** The walk's settlement is unchanged: a dependent of a merge-ready unit is still told `blocked`, and a merge inside the ownership span unblocks nothing — a re-issue of the blocked units after the hand merge remains their way in, exactly as when a person merges after the walk has moved on. Holding the owner is about the thread, never the walk.
- **Lifetime.** The end state is one live instance per open pull request: the runner waits on the pull request after the approval, a reply is a steer that opens coding round N+1 → re-review → fresh approval in the same instance (the warm chain of record 0051's amendment, re-entering at the machine's existing review-round transition), and the idle limit ends an abandoned pull request's unit so a thread is not owned forever. That wait is not built by this amendment's first unit: until it lands, the dispatcher keeps the thread the unit's, records the reply on the row — the row's `merge_ready` ending stands, never overwritten `terminated` by the gone-instance rule — and acks the sender with the round that runs it (a re-issued ship on the pull request), so a correction is never misrouted and never lost.
- **Lease rule.** A post-approval coding round is not a lease expiry: nothing was cut short, so nothing continues under the grant's renewal count. It runs on a fresh lease of its own and spends no renewal — record 0046 takes the row in its own dated amendment, which this one only points at.
- **Occupancy.** The thread stays occupied for the pull request's life: an unrelated ask typed there is a steer into the unit's loop or belongs in a new thread — consistent with one thread per pull request, and with this record's host key holding one pipeline per thread.

Against the record's own criteria: nothing here touches the hosted parent, the host key or the web handle; the ownership span is the unit row's and the dispatcher's, readable from the same rows every surface projects. With the warm window at zero and the pull request merged or closed, every sentence above reduces to the record as written.
