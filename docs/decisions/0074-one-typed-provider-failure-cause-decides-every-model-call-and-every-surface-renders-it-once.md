...
The intake receipt carries `providerFailure` beside `source: error`. The first deciding process posts the rendered sentence and does not dispatch the reply. A replay reads the same typed receipt; it does not reconstruct a cause from the stored reason. Memory reflection logs the rendered cause and writes nothing.
...
- [Issue #2170](https://github.com/coreplanelabs/switchboard/issues/2170): the 21:24Z credit-limit outage, raw-payload rendering failure and generalized model-call seam.
