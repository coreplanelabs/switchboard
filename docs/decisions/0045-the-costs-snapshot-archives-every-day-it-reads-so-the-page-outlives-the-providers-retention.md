...
4. The service asks the snapshotter for rows over the range: the in-memory archive answers `2026-09-17 .. 2026-11-14` (59 days), the live snapshot answers the rest. The archive's row for 2026-11-14 `runUsage` was last written by the take of 2026-12-14, whose retention floor was 2026-11-15, so that take did not write 2026-11-14; the row is the one from 2026-12-13, which saw the day whole.
...
6. For By user the service builds one `RunUsageReport`: rows = archived `runUsage` rows for the 59 days plus the live rows; `pending` and `retentionDays` are the live report's; `earliestFinishedAt` is the earlier of the live value and the archive's first day; and `buildUserCostReport` is given `historyFrom: "2026-08-18"`, which `coverageFrom` uses in place of the retention cutoff.
7. `buildUserCostReport` prices and allocates over 90 days; its coverage line reads from 2026-09-17, not from 2026-11-15, and `coverage.retentionDays` still says 30, which is still true of the run store.
8. The same request on 2026-09-20, three days after ship: the first take on 2026-09-17 carried its 31 days, so the archive already begins on 2026-08-18; the range asks from 2026-06-23, so `coverage.from` = 2026-08-18 and the report's 56 leading days are uncovered; the page draws them as no data, the range total and the chart cover 34 days and say so, and the line says `history begins Aug 18`.
9. A user who was billed on 2026-10-01 left the workspace on 2026-11-20; run retention has dropped their runs from the store, but the archive's `runUsage` row for 2026-10-01, last written whole by the take of 2026-10-31, still names them, so the 90-day By user report still shows the row.
...
The design: the bot computes `archiveRowsOf(snapshot)`, one pure function beside the take, and the store only upserts what it is handed. For the eight Cloudflare datasets and the LLM rows a day is whole when it is closed: `range.from ≤ D < range.to` (the take day is excluded; an estimated LLM row inside that span is still written, and the next take overwrites it with the closed figure). For the per-user rows a day is whole when the store held all of it: `D ≥ retentionFloor`, where `retentionFloor` is the day after `dayOf(takenAt − retentionDays × 1 day)`, or `range.from` when `retentionDays` is 0 (the Null run store, or no retention). The Durable Object upserts each `(date, part)` row and refuses to replace a stored non-empty row with an empty one, so a provider that answers nothing for an old day on one take (a transient gap, a cost report with no bucket) cannot erase a day another take saw. That guard is decided and revisable: the one true zero it hides is a day whose earlier sighting was non-zero and whose later closed figure is exactly nothing, which no meter here produces for a live account.

Invariants: (a) an archived `(date, part)` row was written by a take that saw the day whole for that part, by the rule above; (b) for the per-user part no archived row was ever written by a take whose retention cutoff lay inside the day; (c) an archived row's rows are never replaced by an empty list; (d) the archive rows of a take are a subset of the snapshot's rows, day by day.
...
The constraint: each `(date, part)` row is one JSON value in Durable Object SQLite, whose single value limit is 2 MB; the put body is fenced at 16 MB, and the archive rows ride in it beside the snapshot, roughly doubling a body the store's own comment puts at a few hundred KB. The archive adds rows: up to 31 dates × 10 parts = 310 upserts a take, in the same transaction as the live parts, daily plus on demand. The read is the unmeasured part: `buildCostReport` is O(days × rows), so a 366-day report over a year of rows is about 140 times today's arithmetic.
...
**Why not ask Cloudflare for longer retention?** The 4w4d ceiling is what the account's GraphQL analytics answer; a plan that raised it was not priced for this, and the ceiling would still exist, only further out.

## Boundaries

The archive begins 31 days before it ships (the first take's oldest day for the billing parts, 2026-08-18 if this ships on 2026-09-17; the per-user part begins at the first take's retention floor, 2026-08-19 with 30-day retention), and nothing older will ever be shown for Cloudflare. Anthropic's cost report may reach further back and could seed `llm` rows for older days; that is the open question, not this design. The live snapshot, its take, its loop, its command and its status feed do not change. `SNAPSHOT_DAYS` becomes the literal 31 and `MAX_DAYS` the page's ceiling (366): today `SNAPSHOT_DAYS = MAX_DAYS` is a literal alias, so raising `MAX_DAYS` without breaking that alias first would make every take ask Cloudflare for 366 days and fail; unit 1 breaks the alias in the same commit and updates costs.md item 3 and criterion 5, the `resolveRange` clamp tests, and the `MAX_DAYS` comment that still calls it the snapshot's window. The page's presets and the drawing of a year are a second unit, gated on the maintainer's check-in for UI work. Compatibility: the JSON twins gain `coverage` (the daily one) and reach back further (By user); nothing is removed; `?days` over 31 stops clamping to 31 and clamps to 366.
...
| # | Criterion | Proof |
|---|---|---|
| 1 | `archiveRowsOf`: billing parts for every closed day of the range, the per-user part for every day at or after the retention floor, nothing for the take day; retention 0 → the range's start; the rows a subset of the snapshot's, day by day | `[gap]` unit 1: `src/core/costsSnapshot.test.ts::archiveRowsOf::*` |
