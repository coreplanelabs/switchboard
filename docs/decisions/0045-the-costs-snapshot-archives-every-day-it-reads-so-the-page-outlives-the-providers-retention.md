...
**Amended while parked, 2026-09-17.** The run history's rows the table below describes have moved on since `c36222a6`: the store answers one row per run (`/runs/usage`, [run-history.md](../reference/specs/run-history.md) item 56) and the bot folds them into usage cells — one per requester, thread, channel, agent and UTC day — that `costsBy.ts` (the renamed `costsByUser.ts`) reads for every cost dimension through `buildCostsByReport(dimension)`. Each cell is still keyed by `day`, so the archive's split by date and its "last whole sighting" rule hold unchanged; a pick-up re-reads costs.md items 10–10b for the shape. The table's proof links point at the renamed module; its prose stands as it was written at that sha.
