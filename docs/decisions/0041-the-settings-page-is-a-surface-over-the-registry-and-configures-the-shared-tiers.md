...
## Amended 2026-09-17 — an unlinked session's `me` is its own scope

*Re-evaluation, as an amendment after acceptance asks.* The record was accepted because the settings page is an adapter over the command registry and nothing more: every write is a command the registry already exposes, decided by the policy table with the viewer's grants, and the page carries no rule of its own about who may do what. The refusal of an unlinked session's `me` was never the page's rule; it was the config and MCP handlers', decided by the data, on one fact ("The tier the page does not offer"): a browser session requests no run, so `users["access:<sub>"]` is a scope nothing reads, and a setting nobody reads is a setting that lies. [Record 0042](0042-a-dashboard-session-is-the-person-its-email-names-identity-not-authority.md) kept the refusal for the unlinked session on the same fact.

**What changed.** [Record 0043](0043-the-home-page-is-a-chat-the-browser-is-a-channel-and-a-turn-is-a-run.md) made the browser a channel: the dashboard's chat dispatches a run as the session — `access:<sub>` unlinked, the person with the credential recorded when linked — and the resolver reads that user's scope like any other. The fact the refusal rested on is false for a browser session and still true for a service token, which requests no run. So the refusal is lifted where it is false and kept where it is true: `meIdOf` answers the linked person, else the session's own id, and only a `service` actor (`access:svc:…`) is refused, with a message that names that reason. The MCPs tab offers `me` to every session — the person's tier linked, the session's own unlinked — and marks the session's own rows as writable.

**Checked against the reasoning.** The page still carries no rule: it offers the tier and posts `scope: "me"`; the handler resolves whose scope that is and the policy table decides. An unlinked session passes exactly the rows it passed before — its own scope's `config:write` (`is-self`) and the browser baseline's `mcp:write` — and no new one. The conformance suite's one surface-bound refusal (item 25's 🚫 cell) goes with the rule, and its identical-output contract now holds for the `me` writes on every surface, which is the stronger form of criterion (3): chat, CLI and the page cannot disagree.

**What does not regress.** A linked session's `me` is unchanged, the person's; a chat caller's is unchanged; the audit line still names the session; a service token is refused as before, for the one reason that is still true of it. Nothing shared changes hands: the scope an unlinked session writes is resolved only by runs requested as that session.

**The validation rows, as they read now.** The two rows above about an unlinked session's `me` are superseded by these; their old proofs were renamed with the tests.

| Criterion | Proof |
|---|---|
| The page composes `scope: "me"` for every browser session — the linked person's tier, else the session's own; the rows it may write are its own | `[unit]` `web/src/pages/settings.test.ts::McpServersPanel::Add posts mcp.add with the tier picked and the agents joined`, `web/src/pages/settings.test.ts::McpServersPanel for a linked session (record 0042)::an unlinked session's me is its own (record 0043): offered and defaulted, Add posts scope me, its own row is mine and writable, the person's row read-only` |
| A `me` write from a browser session lands in the scope its runs read, on the config and the MCP commands; a service token's is refused by the data; a chat caller's `me` write is unchanged | `[unit]` `src/core/commands/config.test.ts::config set::a credential needs config:write for any scope; a chat person always has their own scope; a browser session writes its own \`me\` (the scope its chat runs read) and not \`channel\` (no grant); a service token has no \`me\``, `src/core/commands/mcp.test.ts::mcp.* commands::a browser session's \`me\` write lands in its own tier (record 0043); a service token's is refused by the data; org and channel writes and every read are unchanged for both` |

## Amended 2026-09-17 — channel names, not ids

*Re-evaluation.* The record was accepted on the evidence that the page is an adapter over the registry and carries no rule of its own. Its second open question — channel names in place of ids — stayed open because no directory exposed a channel's name. It closed with the names series (#1562, #1566, #1569) in the same shape the acceptance reasoned from: a core name directory (`src/core/names.ts`) the adapters fill and every seed reads, so the Channels and MCPs tabs show `#name` with the id in the tooltip, and a channel is picked from a list a new registry command (`config channels`) answers — the CLI and chat get the same list, the page grew no logic. Nothing the acceptance relied on regresses: the page still renders commands and posts commands.

## Amended 2026-09-17 — the tabs navigate in place

*Re-evaluation, as an amendment after acceptance asks.* The record was accepted for three things: the page is an adapter over the command registry with no second write path, personal settings stay where the identity is, and the installation tab is an allow-list projection. It also stated the web app's shape at the time — "navigation is full page loads over a server-rendered seed" in the delta, and "three tabs, each a full page load over a seed" in the shape — as a fact about the house, not as a ground of the decision. [Record 0056](0056-the-dashboard-navigates-in-place-a-page-is-one-view-one-seed-two-encodings.md) retires that fact: every page route answers its seed alone as JSON to a request that accepts `application/json`, the router loads it before the route resolves, and the tabs, the channel index rows and the cog are `RouterLink`s. Checked against the acceptance: the seed each tab paints from is still the registry's answer invoked as the viewer, built by the same view, only encoded twice; every write is still one `POST /api/<group>.<verb>`; nothing about who may do what moves into the page. What could regress is a stale page after a write — the page still reloads the document after a save, so it is seeded anew as the new state, and the amendment leaves that as it was. Nothing else in this record changes.
