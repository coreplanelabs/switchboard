...
2. With `review.rereviewOnPush: on` — the per-repository default — a substantive head change after a verdict creates exactly one replacement review run for the newest observed head. If the head moves while that automatic review is active, the subscription supersedes it, tells it to end without posting a verdict, advances the same claimed transition and starts one replacement at the newest head only after the obsolete run ends. Redelivery or a burst of pushes never leaves two automatic reviews live for one pull request.
3. The claimed transition posts exactly one first update in the originating review thread and one pull-request comment: “the head moved from `<old>` to `<new>`, re-running the review — `<run link>`”. If that transition advances again before its verdict, those same publications are reconciled to the newest head and replacement run rather than posting another pair. When the final replacement run ends, exactly one second update on each surface states its verdict at `<new>`.
...
The bet is that the bot owns that renewal. Every posted pull-request verdict leaves one durable subscription. A changed patch invalidates the verdict and bot-produced approval, emits one old-head → new-head update in the originating thread and on the pull request, and starts one review at the new head. Further pushes supersede that run and move the same update and review slot to the newest head, so only the final replacement emits a verdict and only its `LGTM` can re-mint approval. An unchanged rebase carries the decision under record 0071. Repository branch protection may additionally require the **Switchboard verdict at head** check, but it is a belt over the bot's event-driven re-review, not the mechanism that starts it.
...
The signed App webhook intake admits `pull_request.synchronize`, resolves the repository setting, refreshes the pull request and looks up its subscription. A substantive move from subscribed head `A` to current head `B` opens one durable active transition for `(repository, pull request)`, with `A` as its source and `B` as its target. The claim creates or adopts one review run pinned to `B` and records its run link before either surface is updated. A redelivery sees the same target and can finish missing publications, but cannot create another run. A newer target advances this same active transition until it settles; completed target claims remain as audit and deduplication receipts.

The transition invalidates the current verdict and bot-produced approval before dispatch. The old review remains immutable audit evidence at `A`; it is no longer merge evidence for `B`. The auto-approval path dismisses or supersedes its own stale approval and records whether GitHub accepted that write. Failure to clear the GitHub artifact leaves the product state stale and the optional check red; it never silently treats the approval as current. If GitHub refuses the dismissal and the repository does not require **Switchboard verdict at head**, issue #2145's motivating failure remains reachable — GitHub can still merge `B` on the approval minted for `A` — and that is the residual hole the optional check exists to close.
...
For a substantive move, the first publication is deterministic and includes the full old and new heads plus the replacement run link. It is posted once in the originating review thread and once as a pull-request comment. Both publications carry the transition id and remote message identity in their durable receipt (and an invisible marker where the remote surface needs reconciliation), so a timeout after GitHub or the channel accepted the write is read back rather than posted twice. If the transition advances before its verdict, those same publications are edited or reconciled to the newest target and run link.

The replacement review runs through the ordinary review path at `B`: the same read-only agent, reviewed-head guard, typed verdict, severity gate and deterministic GitHub review body. While `B` remains the target, its end records one second publication per surface: `LGTM at B`, `Changes requested at B`, or a named no-verdict/refusal state. Only the first two are verdicts; only `LGTM at B` can re-mint the auto-approval. If the head moves again to `C` before that transition settles, the subscription atomically marks the `B` run superseded, tells it to end without posting a verdict, advances the transition target to `C`, and waits for the obsolete run to end before starting one replacement at `C`. The same first publication is reconciled from `A → B` and `rB` to `A → C` and `rC`; there is no `B` second publication. If `rB` crossed the posting fence and put a verdict on GitHub just before the supersede arrived, that immutable review remains only as audit evidence at `B`: it is immediately stale, emits no second update, cannot become the subscription's current verdict, cannot mint approval and cannot turn the `C` check green. Thus there is at most one live automatic review for the pull request, and a burst of pushes yields one review and one verdict at its final head rather than a queue.
...
2. A contributor pushes a substantive commit, producing `B`. GitHub delivers `pull_request.synchronize` twice around a process restart. The first delivery opens the active transition at target `B`, invalidates the current verdict and approval, creates run `rB`, and records its link. The second delivery adopts that target. Between them, exactly one thread update and one pull-request comment say that the head moved from `A` to `B` and link `rB`; the check at `B` is red with “verdict is at A; review rB is running”.
3. While `rB` is reviewing, another substantive push produces `C`. The `C` delivery advances the same transition to `C`, marks `rB` superseded and tells it to end without posting a verdict. Only after `rB` ends does the subscription create `rC`. The existing thread update and pull-request comment are reconciled in place to say `A → C` and link `rC`; no second head-moved update or `B` verdict update is posted, and a redelivery at `C` adopts `rC`. If `rB` posted its immutable GitHub verdict just before the supersede signal crossed, that review remains visible as stale audit evidence at `B` but is not copied to the update surfaces and cannot become current, mint approval or turn `C` green.
4. `rC` requests changes at `C`. The transition's one second update per surface states that final-head verdict, no approval is minted, and the check remains red with the blocker. A repeated finish event repairs a missing publication receipt but posts no duplicate. Across the `B → C` burst, the requester saw one reconciled head-moved update and one verdict update, and only one automatic review was live at a time.
5. After that transition settles, the contributor fixes the blocker at `D`. One new transition creates `rD`, posts its one head-moved update per surface and waits for the verdict. `rD` returns `LGTM at D`, posts the transition's one second update per surface, mints an approval pinned to `D` and turns only the `D` check green. GitHub can merge under the same verdict the reader sees.
...
The invariant is: one pull request has at most one live automatic review; one active transition has at most one head-moved update and one final verdict update per surface even when its target advances; and no current approval exists without an `LGTM` that applies to the same patch and is pinned to that head.

## A second hard-case trace: a ship fix pushes its owned pull request

1. A ship unit owns pull request 42 and its subscription at head `A`. Its review round requests changes, so the unit dispatches one fix round in its existing thread.
2. The fix round pushes `B` to that pull request and records the push before returning. The owning ship unit claims the transition to its next review round at `B`; the subscription and unit row point to the same transition identity.
3. A `pull_request.synchronize` delivery races that ship transition. Webhook intake finds the active ship owner, records `deferred to ship` on the transition and creates no generic review run. A redelivery reads the same deferral receipt. If the ship round already created its review, the webhook adopts that fact rather than dispatching.
4. The ship-owned review is the sole replacement review at `B`. The transition receipts publish one first update per surface with its run link; its completion publishes one second update per surface and advances the unit from the verdict. An `LGTM at B` can move the unit toward checks and mint approval at `B`; changes requested returns the same unit to its next fix round.

The ownership rule is: while a live ship unit owns the pull request, its durable round transition performs the re-review and webhook intake only records that it deferred. The generic watcher acts only when no live ship owner exists.
...
5. **A head that moves while its replacement review runs.** Advancing the active transition must stop `rB` before dispatching `rC`, reconcile the existing first update and suppress `rB`'s verdict and second update. The posting fence handles the narrow race: a `B` verdict already written remains immutable audit evidence, but the subscription compare-and-swap keeps it stale and powerless at `C`.
...
**The update protocol is product behavior, not logging.** People need to see the gap open and close in the place they asked and on the pull request they may merge. The first update names old head, newest head and live run; a supersede reconciles that same message instead of adding another. The second names only the final verdict and reviewed head. Hidden transition ids and stored remote message identities make those two human messages safe to retry or revise.
...
- **Unit one — re-review on push, update messages and setting.** Add the durable review subscription and claim ledger; subscribe direct reviews when their verdict posts and connect ship's existing pull-request ownership to the same identity; admit `pull_request.synchronize`; classify unchanged rebases through record 0071's proof; invalidate changed-patch verdicts and bot-produced approvals; keep one automatic review slot per pull request by stopping a superseded run before starting its newest-head replacement; post and reconcile the first update once per surface and post only the final verdict update; add `review.rereviewOnPush` at repository scope, default on, to settings and `config set repo`. Fixture tests replay all four issue #2145 merges, webhook redelivery, a second push during re-review, the posting-fence race, an off repository and an unchanged rebase.
...
Before acceptance, give a fresh reader only this record and ask for four things: restate the bet; explain why branch protection is optional rather than the owner; trace `A → B → C` through verdict, approval, messages and runs; and name the rebase-only exception. The gate passes only if the answer says that a changed patch immediately invalidates both verdict and bot-produced approval, one active transition advances to the newest head with no overlapping automatic reviews and only one reconciled first update plus one final verdict update per surface, only a current-head `LGTM` restores approval, and an unchanged-patch proof carries the decision without a model run.
...
Implementation acceptance for unit one additionally replays the four issue #2145 fixtures and proves under webhook redelivery and a push burst that at most one automatic review is live, obsolete runs post no verdict update, only the final head is reviewed to a verdict, and each surface receives one reconciled first update and one final update. Unit two acceptance proves the check red at a stale, blocked or in-flight head and green only at a current-head `LGTM`, both when GitHub requires it and when it is informational.
