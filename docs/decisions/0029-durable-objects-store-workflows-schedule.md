...
1. The refresh instance for `repo:acme/monorepo` is on its `install` step, 4 minutes into a 10-minute budget, awaiting `ResidentDO.installDeps(key)`, the engine's install step exposed as a method, through the resident Worker binding.
