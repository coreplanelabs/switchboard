...
## Amended 2026-09-20 — the hand-back clauses are replaced; the operator becomes one loop (record 0069)

*Re-evaluation, as record 0069’s amendment of 2026-09-20 decides.* Record 0069, as amended, replaces this record's hand-back clauses ("a bind at or after the path's confirm class is handed back as the line to paste"; the verifier's disagreement "hands the line back to type"): *a bind at or after the path's confirm class is offered as one click through record 0044's store on every chat surface, and a verifier disagreement or failure floors to the route; the operator outranks no guard, deterministic code still authorizes, fences and executes — and it also never interprets a failure into a sentence: no path after the operator's decision renders a line to retype.* With the one-execution-path plan's loop unit the operator is one agent loop whose only way to act is a typed tool call — `bind_preset`, a registry command's own typed tool, `ask`, or ending the turn — the verifier retires (`verifyOperatorBind` and `renderVerifierHandBack` deleted), and every turn outcome executes through `decideExecution`'s cell. The split this record decided — judgement in the model, determinism at the boundaries — is unchanged.

## Amended 2026-09-21 — repository context is a typed door fact

The repository token scan named in this record is retired from the operator path as a decider. `thread_state` exposes the newest finished run's agent, repository and pull request plus the channel's configured default repository; `bind_preset` carries an optional typed `repo: owner/name`; and target resolution accepts that slot below an explicit current-message target. The existing URL/directive extraction remains only as compatibility for typed requests and the `routing.operator: off` rollback path, not as facts hidden from the door. A repo-less ship write with one available repository becomes record 0054's one Yes/No question carrying a runnable ship bind, never the old syntax example.

The question cell is tightened at the same boundary: an `ask` proposal must parse through the projected registry grammar or as a projected preset with task and repository facts before it renders. The exact chat form `mcp list` is permanently the `mcp.list` registry read, including when its backend is unavailable; it never falls through to another interpretation.
