...
There is no repository-plan migration path. The proposed bootstrap plan at `docs/plans/2026-09-21-002-feat-the-ship-pipeline-dissolves-into-the-orchestrator-plan.md` is imported once as the bootstrap work record, then the repository-path input is deleted. Any other repository plan someone wants to reuse must be supplied as an ordinary file to `plan`, just like a document uploaded from elsewhere.
...
- [Record 0073](0073-the-ship-pipeline-dissolves-into-the-orchestrator-the-unit-machine-is-the-deterministic-atom-and-judgement-composes-units.md) and its [proposed implementation plan](../plans/2026-09-21-002-feat-the-ship-pipeline-dissolves-into-the-orchestrator-plan.md) — plans as context, the deterministic unit atom, durable adoption and the future graph-path collapse.
...
## Amended 2026-09-25 — retain a non-executable migration bridge until retirement

Status: proposed append-only amendment for maintainer acceptance. It changes no accepted record or runtime behavior. Re-evaluated against `main` at `b7f8f15f4bb351a30811697a93d6bac4d67a5db9`, accepted record 0073, and current #2290/#2292/#2291.

**Re-evaluation.** Record 0077 was accepted because every source form converges on one validated canonical pin, a provider URL supplies the human address, and execution starts only from a complete `PlanRef`. This removed repository paths and generated requests as executable inputs while preserving durable identity, budgets, current initiator authority and guarded merge behavior. The rationale and five acceptance answers stand.

The delivery order nevertheless deletes readers before a deployed migration can read legacy repository-seeded and generated source material. That couples recovery to the possibly broken new runtime. The minimum non-regressing change is a retained, build-stamped migration/recovery bridge, followed by reader deletion in a later release.

**Delivery-order text replaced.** In *Delivery order* item 1, replace deletion of the generated-plan execution kind, repository-path request grammar and readers with: remove the execution kind and request grammar from every admission, dispatch, resume, planning and unit-machine path; confine both readers to the bridge command. Replace “migration fails before deleting either reader unless every resumable pipeline has a pin” with: no build deletes either reader until the deletion gate below passes. The bridge ships across S1 and initial S2 while admission and resume are closed; deletion follows record 0073 cut three. No checked-in plan executes beside a pin, and no flag selects a legacy loader.

`PlanRef{planId,digest,providerUrl}` remains the sole executable loader. While 0073 fallback exists, its composer loads content only through `PlanRef` and may use verified retained graph/cursor state belonging to already-resumable migrated units. It never rebuilds executable edges from artifact sequencing reasons or gives a new admission legacy graph state. Unknown or inconsistent retained state parks; verified rows may continue under ordinary authorization after the fleet reopens resume, until fenced composer transfer. The bridge itself never resumes them. Rollback never re-enables an execution reader.

The independently invocable bridge reads only the admission-closure inventory: resumable generated/repository-seeded rows already in the coordinator store plus 0077’s named bootstrap plan. It accepts no path, ref or task text and admits no plan. It constructs the host-owned `PlanArtifact` from each row's admitted source through the same RFC 8785/JCS validator, canonicalizer and SHA-256 implementation as the pure three-file contract; no migration-specific encoding or artifact framework. It never rewrites a pin or changes an existing digest; invalid or ambiguous rows park with a named cause.

Its separate operator credential permits bounded inventory reads, immutable `PlanStore` writes, idempotent parent create-or-adopt and publication attachment through the same destination authority/marker adapter, admission closure, and owner parking. Before create/adopt or attachment, deterministic code compares every source's authorized-reader boundary with the destination audience, refuses unknown or broader visibility, and emits only redacted citations/receipts. Snapshots and receipts use the configured run-history/artifact retention, deletion and authorized-read policies; referenced pins remain while any work is resumable. Missing policy parks the operation, not a wider audience or indefinite retention default. The bridge cannot open admission; start/resume code; write branches; publish reviews/approvals; merge; release; or change grants. The operator appears in the receipt but never replaces `ExecutionInitiator`; a row without a resolvable initiator parks. Invocation depends on neither chat nor the new bot/composer runtime.

Migration preserves instance and unit ID/key, idempotency namespace, source, initiator, request context, operation target/base, publication destination/version, evidence references, source reader/completeness boundaries, client-requested merge policy, thread/branch/PR, events, rounds/waits/counts, lease/recovery claim, heads, ending cause, retained work, active composer/epoch, pending-question key, tagged `{child|wait|ending}` relation and sequence, salvage receipts, owner generation and parent/child budgets. A required binding that is missing or contradictory parks; no prose reconstruction or authority-widening default fills it. Versioned legacy-field mappings must prove the same facts; explicitly optional absence stays absent. Current grants are freshly resolved, never copied as authority. No replacement unit, budget or authority is created.

The pure #2292 three-file artifact/ref groundwork (types, validation, canonicalization/digest and `PlanRef`) need not await bridge completion; it has no store, loader, owner, migration or runtime effect and remains behind its own drain/source gates. Acceptance is necessary but insufficient before implementing or deploying any legacy-row read, pin/publication write, loader-routing change or reader deletion. Each still needs reviewed planning and fleet-owned code, merge and release authority. Acceptance permits no implementation, admission, resume, merge, release or deployment.

**Safety and deletion gate.** Admission closure and owner parking are durable compare-and-swap facts under expected sequence, composer epoch and owner generation; every capable runtime must already honor their fence or be drained before the first pin. The bridge never owns/composes and advances an epoch only to park. After the first pin, rollback targets only the bridge build or later. Provider publication may be irreversible; legacy-encoding removal, pin-only admission and reader deletion are explicit later boundaries.

Keep the exact bridge artifact deployable and schema-compatible until every resumable row has a verified pin and complete `PlanRef`, no unknown/dual owner or graph-dependent row remains, 0073 fallback parity passes, and forward recovery succeeds. Newer schema versions must remain readable/fenceable by the bridge or be refused before rollout. Before pin-only admission, prove recovery after every crossed irreversible boundary with bot/composer unavailable, first against a production-shaped snapshot and then within the authorized production admission-closed migration window: close admission, inventory, repair/attach a reference or park an owner, and emit a receipt. Do not create an extra production outage solely for proof. Only a later build deletes readers and bridge.

Proof covers: mixed binaries refusing legacy execution; same-unit mid-round continuation with budgets; bridge recovery from broken new runtime; unavailable publication leaving a retryable non-executable pin; independent credential/runtime support with no code/merge power; visibility incompatibility refusing attachment with a redacted receipt; and rollback stopping at each named irreversible boundary.

**Five-answer recheck.** The canonical pin plus provider URL remains the only executable reference; projection remains non-canonical; admitted execution survives provider outage through its pin; merge still requires fresh initiator authority; and legacy inputs collapse once into the pinned path, never a compatibility execution path.
