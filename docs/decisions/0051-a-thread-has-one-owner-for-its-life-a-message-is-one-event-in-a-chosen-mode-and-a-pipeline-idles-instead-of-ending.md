...
status: proposed
date: 2026-09-17
pattern: Thread as session with a single durable owner (Anthropic Managed Agents' idle/running session, Capy's one agent per thread); the owner outlives its runs, a pipeline unit idles at its lease or its cap like a session at budget_reached, and a plain message is delivered in a sender-chosen mode (steer, queue, interrupt) uniform across agents; the router is a session-start concern only
---
...
3. A plain reply during a live run is steered, queued or interrupts, at the sender's choice, on pi and OpenCode alike; no call in flight is aborted.
...
A ship pipeline unit that stops at its lease leaves its thread with no owner, so the next reply runs the router as if the thread were new and the only continuation is a by-hand re-issue; 12 of the 59 ship coding children the budget analysis read before this record ended at the clip. The bet is that a thread has **one owner for its life**, the pipeline unit while it is unfinished or else the newest continuable session, that every plain message is **one event** stored on the owner and delivered in a **mode** the sender picks, and that a unit **idles** instead of ending. It costs a wait on the Workflow runner that parks the plan's later units while a person is needed, a mailbox on the unit row, the instance id written on the live ship run, and an amendment to record 0046. Decided: the owner rule, the modes, the idle unit, the requester's word at the lease; open: whether a waiting instance is billed. Doing nothing keeps "re-issue `agent:ship` in this thread with the same text" on every stop card.
...
stateDiagram-v2
    ...
    Live --> Live: thread event · steer / queue / interrupt
...
2. **Delivery modes on two harnesses**: `queue` and `interrupt` as one header both harnesses honor without cutting a call. Section: delivery modes.
...
The mode lives on the thread event and nowhere else. The dispatcher parses a leading `!steer`, `!queue` or `!interrupt` (Capy's spelling, steer the default so a bare reply behaves as today) and writes it on the event. The harness reads it when it drains. `steer` is today's path. `queue` is held until the loop's current turn ends and delivered as the next user turn; on pi the turn boundary and the turn's end are one moment, so `queue` and `steer` coincide there and differ only on OpenCode, which can deliver mid-turn. `interrupt` aborts nothing: the call in flight finishes, the remaining tool calls of that turn are not executed and their results say the sender cut the turn short, and the message is the next turn. No new abort primitive is needed, item 2 holds unchanged, and the mode is the harness's to honor, not the preset's.

Invariants. A queued message is delivered before any later steer. An interrupt cuts at most one turn; a second interrupt in the same turn is queued. Every mode records the `input` event and the `follow_up` note a steer records today, plus the mode. On an idle owner every mode delivers as the next turn.

Failure modes. An interrupt lands during the write-up: the write-up is the loop's last turn and is never cut, so the message runs as the fresh turn item 4 defines. An interrupt from a sender the allowlist excludes is refused as a steer is refused today.

The alternative it beat: interrupt as the default, Capy's choice. A filed outside fact (record 0047) will be a frequent sender, and a bot must never cut a person's turn.

## The owner rule

The constraint is that the owner must be computable where the dispatcher already skips the router for a live thread, from the runs page it already read, with at most one more read. The rule, in order: a live run in the thread owns it; else, when the page's ship run names its instance or a child names it by `parentInstanceId`, the unit of that instance whose row names this thread owns it if the row has no ending; else the newest continuable run a person addressed, today's sticky agent; else no owner. The one read is the instance's unit rows. The instance id is on a child's view today, and on the ship run's record only when the instance finishes, so unit one writes it on the live ship run's `run_meta` at the hand-off and exposes it on the view. A coordinator's child never owns a thread, so an ended unit's thread routes fresh. A request directive keeps today's meaning: `agent:review <pr>` in a pipeline's thread starts a review when no run is live and is refused as an agent mismatch when one is.
...
| Does `!interrupt` need a permission distinct from steer? | the maintainer | a decision; default the steer gate, revisable | unit three |
...
| The live ship run's `run_meta` names its instance and the view exposes it | `[gap]` unit one: `src/core/dispatch/ship.test.ts`, `src/core/runsService.test.ts` |
...
| `!queue` is delivered at the turn's end on OpenCode and coincides with steer on pi; `!interrupt` lets the call in flight finish, drops the turn's remaining tool calls with the cut-short result and delivers the message as the next turn; every mode is recorded; nothing is aborted | `[gap]` unit three: `src/core/harness/conformance.test.ts` (both drivers) |
