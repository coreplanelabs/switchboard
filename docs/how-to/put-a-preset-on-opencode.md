...
- **The record names its harness and whose word chose it.** The run's metadata carries `harness: opencode` and `harnessScope: user | channel | defaults` (`runs get <id>` shows both; the facts bar reads `opencode harness (user scope)`), and the row's facts carry the server's pid, port and session, which is how a restart finds or rebuilds the process.
