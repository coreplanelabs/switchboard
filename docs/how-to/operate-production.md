...
- The profile and the bot's config live in a private infrastructure repository. The release workflow reads them with `SWITCHBOARD_DEPLOY_PROFILE=github://…@main` and a read-only App token it mints as `CONFIG_REPO_TOKEN`; from a laptop, export the same two variables.
