...
| `coordinator` | A `coordinator` entry in `SWITCHBOARD_INGRESS_TOKENS` with `grants.http:coordinator: { actions: [coordinator:step] }`, plus run history on the state Worker | `POST /admin/coordinator/instances|spawn|read-record|pr-check` answer that bearer — the steps a ship coordinator instance calls into the bot | Every coordinator route refuses; nothing else changes (no instance runs until the coordinator's steps exist) | The bot Worker's `ShipCoordinator` Workflow (no instances yet); a table on the state Worker's `RunHistoryDO` |
