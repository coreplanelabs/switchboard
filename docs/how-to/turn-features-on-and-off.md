...
| `routing.operator` | **On by default** — nothing to add. `shadow` records the door's typed decision without executing it; `off` is the rollback lever. The retired readers' classifier has no `model`, `effort`, `auto` or `answer` key | One operator loop binds every plain request to a preset or registry command. A no-call turn is re-asked once; a second binds `general` with reason `no_decision` ([routing-and-config item 21](../reference/specs/routing-and-config.md)) | With `off`, explicit directives and scope settings still resolve normally; an otherwise plain request uses `defaults.agent` | One operator model loop per admitted chat event; `npm run load -- route` replays that same loop over the historical fixture corpus ([load-harness item 17](../reference/specs/load-harness.md)) |
