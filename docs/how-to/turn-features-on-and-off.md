...
| `harness` | Retired — nothing to set. Every preset runs on pi ([harness-pi.md](../reference/specs/harness-pi.md) item 1): a preset with a workspace in the run's container, one without (general, research, conductor) as a child of the bot over loopback with none of pi's own tools (item 12). The process needs `PUBLIC_BASE_URL` for a preset with a workspace (the run's container reaches the model proxy and the harness routes through it) and `PORT` for one without (loopback), and pi in the images (every image carries it). A block of `<preset>: pi` written during the series still loads; `native` fails the load by name | Every run is driven by pi inside the run's container — `pi --mode rpc`, the run bearer as its only key, our tools relayed to the bot, every event on the run's own record; the run page and the card show pi's `read`/`edit`/`write` beside `bash` | — (there is no other loop) | Nothing beyond the model calls the run makes anyway; pi's process in the container (one Node process per live run, under the image's heap cap) |
...
The other 29 commands are on in every installation.
