...
| `coordinator` | A `coordinator` entry in `SWITCHBOARD_INGRESS_TOKENS` with `grants.http:coordinator: { actions: [coordinator:step] }`, plus run history on the state Worker | `POST /admin/coordinator/*` answer that bearer — the steps a ship coordinator instance calls into the bot (`plan`, `unit-start`, `branch`, `spawn`, `read-record`, `pr-check`, `round`, `unit-end`, `finish`) and the shim's `instances` route | Every coordinator route refuses; `agent:ship` runs in-process as before | The bot Worker's `ShipCoordinator` Workflow; two tables on the state Worker's `RunHistoryDO` |
| `ship.coordinator` | `ship: { coordinator: true }` in the config, with the `coordinator` feature above on and `PUBLIC_BASE_URL` set (the bot addresses its own shim) | Every `agent:ship` request (a task, or `plan <path>.md [units U<n>, …]`) becomes a plan runner instance: one thread and one branch per unit, the card following the plan, its summary in the requesting thread; a resume at review (a ship pull request's URL) still runs in-process | `agent:ship` runs the in-process round loop (the default) | The Workflow instance per plan; the instance's records on the state Worker |
