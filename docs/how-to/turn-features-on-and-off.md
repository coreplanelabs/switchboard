...
|---|---|---|
| **minimal** | Slack and one provider. Tools run on the bot host. No Access, so the dashboards serve loopback callers only (`dashboardAuth: none`). Nothing optional. | [Run it locally](../tutorials/run-it-locally.md) — this is that tutorial's configuration |
