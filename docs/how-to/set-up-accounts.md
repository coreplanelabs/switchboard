...
What you need: an account, a domain in it (every hostname in the deployment profile must be under a zone in the account), `npx wrangler login` once, and a Cloudflare API token for the account in `CLOUDFLARE_API_TOKEN` with *Workers Scripts: Edit*, *Containers: Edit*, *Workers R2 Storage: Edit* and *Account Settings: Read* at the account, and *Workers Routes: Edit* and *DNS: Edit* on the zone. `deploy all` checks the token's capabilities before it touches a Worker, because a token that can deploy the state Worker but not list containers would strand the bot half-deployed. *Containers: Edit* is also what the image copy needs: an installation that deploys the release's published images (`"images": "registry"`, what `init` writes from the npm package) has `deploy all` mint a short-lived registry credential from the token and copy the images into the account's registry over HTTPS — no Docker anywhere. Containers Edit is the permission Cloudflare documents for `containers`; it documents none for the registry credential endpoint itself, and a token with Containers Edit has been refused there where a `wrangler login` was accepted — the refusal names the endpoint, and [Deploy](deploy.md#before-you-start) says what it means. Docker is needed only where images are built: a checkout whose profile says `"images": "build"`, which is how this project's own production deploys. The Access application, if you want one, is created in the Zero Trust dashboard by hand: its team domain and AUD tag go in the profile's `access` block and reach the bot as `ACCESS_TEAM_DOMAIN` and `ACCESS_AUD`. The whole sequence is [Deploy](deploy.md).
