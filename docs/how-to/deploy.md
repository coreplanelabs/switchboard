...
- A Cloudflare account and a domain (a *zone*) in it: every Worker gets a hostname under it. `npx wrangler login` done once for that account. Docker running only where images are built: a profile with `images: "build"` (the bot's, resident's and sandbox's images build where `deploy all` runs). With `images: "registry"` nothing is built anywhere — the release's published images are copied into your account's registry once per version, by `deploy all` itself ([step 5](#5-deploy-images--copy-the-releases-images-registry-mode)).
- `CLOUDFLARE_API_TOKEN` in your shell only when it is a token for this account: wrangler prefers a token over your login, and a token for another account is refused, never silently swapped. In `registry` mode `deploy images` and `deploy all` need it, with **Containers Edit**: they mint a short-lived registry credential from it (`POST …/containers/registries/registry.cloudflare.com/credentials`, the call `wrangler containers` makes) to list what the account registry holds and to copy the release's images in. A missing token is refused by name; a token the endpoint refuses is refused naming the endpoint and the permission. `deploy plan` reads what it can: without the token (or with one the endpoint refuses) its `Images:` line says `not probed` and why, and it refuses nothing. Containers Edit is the permission Cloudflare documents for `containers` (wrangler gates its `containers` commands on it); Cloudflare documents no permission for the registry endpoint itself, and a token with Containers Edit has been refused there where the `wrangler login` (whose scopes include `containers:write` and `cloudchamber:write`) was accepted — if yours is, the refusal says so.
...
- `images` — where the bot's, resident's and sandbox's container images come from. `"registry"` (what the example says, and what `init` writes from the npm package — with all four Workers): the images every release publishes, copied once per version into your account's Cloudflare registry by `deploy all` itself (or by [`deploy images`](#5-deploy-images--copy-the-releases-images-registry-mode) ahead of it) and referenced from there — no Docker anywhere, and every container start pulls from Cloudflare's own cached registry. `"build"` (the default when the field is absent, and what `init` writes in a checkout): each Worker's Dockerfile, built by wrangler where `deploy all` runs — a checkout's shape, and how this project's own production deploys. Why the copy rather than a direct pull from GitHub's registry, and why `deploy all` makes it: [record 0027](../decisions/0027-images-copied-into-the-account-registry.md) and [record 0030](../decisions/0030-deploy-all-copies-the-images-it-needs.md).
...
## 5. `deploy images` — copy the release's images (registry mode, optional)

With `"images": "registry"` in the profile, the Workers deploy the images the release published rather than building them. Cloudflare's Containers pull from Cloudflare's own registry (cached, pre-fetched), not from GitHub's, so the images are copied into your account once per version. `deploy all` does that copy itself when it finds an image missing ([step 6](#6-deploy-all--the-whole-plan-in-order)); this command is the same copy on its own, to pre-warm the registry before a deploy window:

```bash
npx tsx src/cli.ts deploy images --dry-run   # which of the three the account registry already holds
npx tsx src/cli.ts deploy images             # copy the missing ones, registry to registry
```

For each of the bot, resident and sandbox images at this CLI's version — the only version the rendered configs reference, so there is no flag to copy another release; run that release's CLI — the command asks the account registry whether it already holds `registry.cloudflare.com/<account>/<name>:<version>` and skips it if so; otherwise it copies `ghcr.io/<owner>/<repo>[-resident|-sandbox]:<version>` registry to registry over HTTPS: a push credential minted from `CLOUDFLARE_API_TOKEN` (why the token needs Containers Edit — a token without it is refused naming the endpoint, before anything moves), the image's `linux/amd64` manifest and every layer the account registry lacks, streamed in 64 MiB parts with each layer's digest verified on the way, the manifest pushed byte-identical and its digest read back; then it lists the registry again and refuses unless every copy appears. Nothing is installed for it and no daemon runs. A second run finds everything `present` and copies nothing. On a profile that builds (`"images": "build"`) there is nothing to copy: the command says so and reads nothing, which is why [the reusable deploy workflow](#deploy-from-your-ci) runs it before every plan and lets the profile decide.
...
`deploy all` first checks: wrangler's login is the profile's account (or `CLOUDFLARE_API_TOKEN` verifies against it — a token for another account is refused with wrangler's own words, never silently swapped for your login), from a checkout that the tree is clean and `HEAD` is `origin/main` (`--allow-branch` relaxes only that; from an operator directory there is no tree — the Worker sources are the package's, at its version), and the credential can do what the selected Workers need (`wrangler containers list`, `wrangler r2 bucket list`). In `registry` mode it then **copies the release's images its Workers lack** into the account registry — the same copy as [step 5](#5-deploy-images--copy-the-releases-images-registry-mode), once per version, before anything deploys — and says what it copied. It reads and validates the config, deploys the state Worker, pushes the config document, deploys the bot and **waits until the bot is live** — `/healthz` answered by a container that is not draining and reports the deployed commit — then the resident and sandbox Workers with their own preflights and live gates. The plan's `Images:` line says where each Worker's container comes from: in `build` mode Docker must be running where the command runs (the bot's image, and the resident's and sandbox's, are built there); in `registry` mode nothing is built and the line counts the images present in the account registry and says `deploy all` copies the rest. `deploy all --dry-run` prints the plan and what it would copy, copying and deploying nothing.

Bearers the run needs in its own environment: `MEMORY_TOKEN` when the bot is a step (the config push); one of `RESIDENT_ADMIN_TOKEN` / `RESIDENT_OPERATOR_TOKEN` / `RESIDENT_READ_TOKEN` when the resident is (its preflight reads the fleet); `SANDBOX_TOKEN` when the sandbox is (its live gate probes `/exec`); `CLOUDFLARE_API_TOKEN` (Containers Edit) in `registry` mode, for the registry read and the copy.
...
The images are where the profile's `images` mode matters most. With `"images": "registry"` (the example's shape, and what `init` writes from the package — with all four Workers) nothing is built from the package: `deploy all` copies the release's images into your account registry once per version, over HTTPS, and every Worker, the bot included, deploys the copy. With `"images": "build"` the resident's and sandbox's images are built where `deploy all` runs, from the Dockerfiles in their materialised directories, exactly as from a checkout — but the bot's image is the repository's root `Dockerfile`, built from `src/`, `web/` and the toolchain, which the package does not carry, so `deploy all` refuses the bot step up front, naming this and the `registry` mode as the way out, rather than rolling the state Worker and then failing.
