...
4. **Permission gates** run post-resolution: agent allowlists admit listed users + admins; absent allowlist = open. `config set channel` is gated by `channelConfig` (empty list = admins only).
