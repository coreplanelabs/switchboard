...
| Live end-to-end structured output on Slack (structuring pass renders correctly in a real thread) | `[gap]` ([#252](https://github.com/coreplanelabs/switchboard/issues/252)) Enable `output.structured` on a deployment and mention the bot; confirm the reply renders correctly with no raw JSON and no double-converted markup. |
