...
| [run-history.md](run-history.md) | Durable run records (#157): the node-free record contract + retention, the `RunStore` seam (in-memory / file / `RunHistoryDO`), the dispatcher write path, `RunsService` read merge, friction ledger served from the store |
| [command-registry.md](command-registry.md) | One channel-agnostic command registry (#157): register once (`runs.*`), scopes + chat gates resolved against the adapter-resolved caller, auth→parse→handle→map, audit line, untrusted-content wrapper, shared compact renderer; no command starts a run (KTD16) |
