...
| [command-registry.md](command-registry.md) | One channel-agnostic command registry (#157): register once (`runs.*`, `friction.report\|propose`, `repo.list`), generic HTTP `/api/<group>.<verb>` / MCP `<group>_<verb>` / CLI / chat adapters with no per-command code, scopes + chat gates resolved against the adapter-resolved caller, auth→parse→handle→map, audit line, untrusted-content wrapper, one shared text renderer; no command starts a run (KTD16) |
