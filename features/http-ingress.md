...
| The token map has ONE parser shared with the Worker shim: `parseIngressTokens` delegates to `src/core/ingressTokens.ts` (`parseIngressTokenMap`, `tokenForSubject` — ambiguous subject → undefined) | `[unit]` `src/core/ingressTokens.test.ts::*`, `::parseIngressTokens (env → config, fail-closed)::*` |
