...
2. **Acceptance receipt**: the moment a message is accepted for handling, the bot reacts ☎️ (`telephone_receiver`) on it — before any model/tool work. Fire-and-forget: missing `reactions:write` scope or a duplicate reaction logs and never blocks handling.
...
| ☎️ reaction lands on acceptance, before the status card | `[agent]` Requires the `reactions:write` scope on the Slack app (**pending — human-gated**). Mention the bot; the ☎️ reaction must appear on your message before/with the status card. Until the scope exists, expect a `[ack] missing_scope` log line and otherwise unchanged behavior. |
