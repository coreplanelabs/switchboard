...
Container application ids: `GET /accounts/{id}/containers/applications`. A group's LLM spend is the cost report filtered to `anthropicWorkspaceId`; the bot's API key must live in that workspace for the attribution to be right (the org default workspace reports as `null` and is never attributed to a group).

## Behavior

1. **Pricing model is Cloudflare's.** vCPU bills on **active** seconds (`cpuTimeSec`); memory and disk bill on the **provisioned** size for every awake second (`allocatedMemory` / `allocatedDisk` byte-seconds). Durable Objects bill duration at 128 MB × active wall time plus per-million requests. Constants in `CLOUDFLARE_PRICES` (list, 2026-08-29); the page states them. Gross — plan fees and included allowances are not subtracted.
2. **Group scoping.** Only rows whose `applicationId` is in `containerApps` or whose `scriptName` is in `workers` count; everything else in the account (e.g. `terrateam`) is dropped before any arithmetic. LLM rows count only when `workspaceId` equals the group's `anthropicWorkspaceId`.
...
7. **Routes.** `/costs` and `/costs.json` → the first configured group; `/costs/<group>` and `/costs/<group>.json`; group slugs are `[a-z0-9-]{1,40}`. GET-only (405 otherwise). Unknown group → 404. Not configured → 503 naming the config keys. Upstream failure → 502 with a capped reason, never a 500.
...
| # | Criterion | Proof |
|---|---|---|
| 1 | Container pricing reproduces Cloudflare's billing for a real row (2026-08-28 resident → $1.066) and DO pricing is duration + requests | `[unit]` costs.test.ts `containerCostUsd`, `durableObjectCostUsd` |
| 2 | Rows outside the group (other apps, other workers, other/default workspaces) never reach the report | `[unit]` `buildCostReport › keeps only…` (×2) |
...
| 7 | Cloudflare source: bearer header, account-scoped variables, exclusive end date, row mapping; non-200 and GraphQL `errors` both throw; token never in URL/error | `[unit]` `CloudflareGraphqlUsageSource` (×3) |
