...
// `costs`, Metrics `metrics`, Delivery `github`, Plane `runLedger`; Threads and Runs are always there, and so is the section the viewer is on. The
...
describe("navSections — which sections exist", () => {
  it("every capability on → Threads, Runs, Plane, Residents, Costs, Metrics, Delivery in fixed order; settings is chrome, not a section", () => {
  ...
  it("a section is listed only when its capability is on; Runs needs none", () => {
    ...
    expect(navSections({ ...ALL_ON, metrics: false }, "runs").map((s) => s.id)).toEqual([
      "home",
      "runs",
      "plane",
      "residents",
  ...
  it("the section the viewer is on is always listed, even with its capability off", () => {
    ...
    expect(navSections({ ...ALL_ON, ...MINIMAL }, "metrics").map((s) => s.id)).toEqual(["home", "runs", "metrics"]);
...
describe("AppNav", () => {
  it("renders the seven sections in fixed order with clean hrefs (no tokens, no query strings), each a RouterLink — navigated in place", () => {
  ...
  it("marks exactly the current section with aria-current=page", () => {
    for (const current of ["home", "runs", "plane", "residents", "costs", "metrics", "delivery"] as const) {
  ...
  it("drops Residents when residents is off and Costs when costs is off; the minimal installation is Threads and Runs", () => {
    expect(
      mountApp(AppNav, { props: { current: "runs" }, seed: island({ residents: false }) })
        .findAll("nav.site a")
        .map((a) => a.text()),
    ).toEqual(["Threads", "Runs", "Plane", "Costs", "Metrics", "Delivery"]);
    expect(
...
describe("AppShell", () => {
  ...
  it("links to the docs at /docs in a new tab — the project's published site, on every installation — without adding an entry to the section nav", () => {
    ...
    expect(wrapper.findAll("nav.site a")).toHaveLength(7);
  });
  ...
  it("puts the docs in the phone menu too, as its own group above the sections", () => {
    ...
    expect(items[1].map((i) => i.label)).toEqual([
      "Threads",
      "Runs",
      "Plane",
      "Residents",
