...
const INDEX_ROWS: RunIndexRowSeed[] = [
  ...
  // A ship pipeline's group (live-view item 33): the hosted parent carries the
  // instance it hosts, its unit runs name it — the index nests them under it.
  row({
    id: "ship-1",
    token: "tok-ship-1",
    label: 'ship · acme/api · "ship the retry-queue plan"',
    agent: "ship",
    userName: "alice",
    ...
    hosted: true,
    instanceId: "wf-preview-1",
  }),
  row({
    id: "ship-1-c0",
    label: 'coding · acme/api · "U16 — the retry queue drains in order"',
    finished: true,
    persisted: true,
    startedAt: NOW - 30 * 60_000,
    finishedAt: NOW - 30 * 60_000 + 690_000,
    status: "completed",
    eventCount: 84,
    userName: "alice",
    parentInstanceId: "wf-preview-1",
    idempotencyKey: "wf-preview-1:U16/0/coding",
  }),
  row({
    id: "ship-1-r1",
    token: "tok-ship-1-r1",
    label: 'review · acme/api · "U16 — the retry queue drains in order"',
    activity: "reading the diff",
    startedAt: NOW - 9 * 60_000,
    eventCount: 12,
    eventsLast5m: 4,
    lastToolCallAt: NOW - 15_000,
    userName: "alice",
    parentInstanceId: "wf-preview-1",
    idempotencyKey: "wf-preview-1:U16/1/review",
  }),
  row({
    id: "hist-1",
