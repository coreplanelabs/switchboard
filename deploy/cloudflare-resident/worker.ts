...
export class ResidentDO extends Sandbox<Env> {
   ...
   *  starts and again whenever the runtime is replaced under it — every lease
   *  this object writes carries it, and a lease from another incarnation is a
   *  holder whose process context is gone. */
  ...
  async installDeps(input: {
    ...
    for (;;) {
      ...
      if (decision.dead?.tree) {
        console.log(
          `deps: ${decision.why} — sweeping ${decision.dead.tree} left by ${decision.dead.holder} before installing ${key.slice(0, 8)}`,
        );
        const swept = await this.runOk(
          killStaleBuildProcessesCommand(BUILD_USER, decision.dead.tree),
          "deps-install-stale-sweep",
        );
  ...
  private async watchdogCheckLifecycle(): Promise<{
    ...
    if (status.state === "refreshing" || status.state === "restoring") {
      ...
      if (!inFlight && systemClock() - updatedAt > STALE_MIDFLIGHT_MS) {
        ...
        const liveAgain = liveInFlight(await this.readInFlight(), systemClock(), this.incarnation);
        ...
        await this.ctx.storage.delete([inFlightKey("refresh"), inFlightKey("hydration")]);
        if (!chained) {
...
async function createRefreshInstance(
  ...
  try {
    ...
    if (isDuplicateInstanceError(message)) {
