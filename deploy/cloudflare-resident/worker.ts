...
export class ResidentDO extends Sandbox<Env> {
  ...
  /** Item 25: the held /await-restore requests, answered by the lifecycle
  ...
  async setResidentState(state: ResidentState, reason = ""): Promise<void> {
    ...
    // Item 25: the transition OUT of `restoring` is the restore event — it
  ...
  /** POST /await-restore (docs/reference/specs/execution.md item 25): answer at
  ...
  async awaitRestore(): Promise<{ state: string; reason: string }> {
    const status = await this.getStatus();
    if (status.state !== "restoring") return { state: status.state, reason: status.reason };
    const outcome = await this.restoreWaiters.wait();
...
/** POST /await-restore (docs/reference/specs/execution.md item 25): the bot's
